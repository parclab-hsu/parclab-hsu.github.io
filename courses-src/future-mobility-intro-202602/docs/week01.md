...
!!! abstract "심화 자료 · 자율주행 안전 아키텍처"
    1주차의 개관을 자율주행 쪽으로 한 단계 더 파고든 심화 자료를 별도 페이지로 정리했다. **L3 상용화 지연과 L2++ 전환**, 그리고 그 선택이 요구하는 **ASIL-D 이중화·SOTIF 설계**를 다룬다. 2주차 이후의 내용을 먼저 훑어보는 성격이므로, 지금은 전체 그림만 잡고 넘어가도 좋다.

    → [🛡️ 심화 · 자율주행 안전 아키텍처 — L2++와 ASIL-D](safety-architecture.md)
