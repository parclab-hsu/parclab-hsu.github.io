...
!!! warning "레벨 3을 가리키는 표현들"
    "주행 중에는 자동차가 책임지지만 시스템의 요청이 있을 때는 사용자가 개입해 제어권을 넘겨받아야 하는 단계", 그리고 SAE의 **조건부 자율주행(Conditional Automation)** — 두 표현 모두 **레벨 3**을 가리킨다.
...
!!! tip "L2+ · L2++는 어디에 놓이는가"
    업계가 말하는 **L2+/L2++는 SAE 공식 분류가 아니다.** 기능은 위 표의 레벨 3에 가깝게 끌어올리되 **최종 책임은 레벨 2에 그대로 두는** 과도기 모델이며, 이 모순 때문에 오히려 더 엄격한 이중화 설계가 요구된다. → [🛡️ 심화 · 자율주행 안전 아키텍처](safety-architecture.md)
...
!!! info "ASIL과 기능안전"
    차량 소프트웨어를 논할 때 함께 따라오는 것이 **ISO 26262 기능안전과 ASIL 등급(A~D, D가 최상위)**이다. 같은 ECU 안에서도 응용 연산은 ASIL-B, 이를 감시하는 실시간 코어는 ASIL-D로 분리 설계하는 방식이 쓰인다. → [🛡️ 심화 · 자율주행 안전 아키텍처 §9](safety-architecture.md)
...
!!! warning "군집주행 — 통신 방식과 효과를 짝지어 기억한다"
    군집주행의 통신 방식은 **V2V**이며, 대표 효과는 **연료 효율 향상과 운전자 부담 완화**다.
    V2G(전력망)·V2P(보행자)·V2I(신호등)와 혼동하지 않는다. 군집주행은 운전자의 부담을 **줄이는** 기술이다.
