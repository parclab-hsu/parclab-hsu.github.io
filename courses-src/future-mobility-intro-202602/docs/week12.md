...
!!! success "스마트시티의 정의 — 기말 서술형 1번 직결"
...
!!! danger "시험 포인트"
    **"모빌리티 서비스에서 사용자 중심의 모빌리티 서비스인 'MaaS'의 약자는?"** → **Mobility as a Service**
...
!!! danger "시험 포인트 (기말 2번)"
    **"드론 생태계 관리를 위한 UTM 시스템의 주요 기능으로 적절하지 않은 것은?"**
    → ① 비행계획 제출 및 허가 ② 실시간 비행 중 충돌 회피 ③ **지자체별 연료 보급소 위치 전송** ← 정답 ④ 드론 위치·고도 정보 통합 제공
...
!!! danger "시험 포인트 (기말 9번)"
    **"차세대 산업성장 동력 확보와 관련된 설명으로 사실과 가장 거리가 먼 것"**
    → **"제조업 비중은 계속 증가하고 고도 성장을 지속"** ← 정답(사실과 다름)
    (✅ 글로벌 수준의 제조·생산 기반 보유 / ✅ 공급 과잉 구조와 확장성 한계 직면 / ✅ 휴머노이드 로봇 시장은 서비스·의료 중심으로 성장)
...
!!! danger "시험 포인트 (기말 10번)"
    **"지능형 로봇 기반 유연 생산 시스템의 핵심 전략으로 가장 적절한 것은?"**
    → **인간-로봇 협업 중심의 유연한 작업 흐름 구축**
    (✕ 정형화된 반복 작업의 전면 자동화 / ✕ 전 공정 무인화를 통한 고정 생산라인 설계 / ✕ 모든 직업에 대한 인간 개입을 최소화한 구조)

## 🤔 생각해 봅시다

1. 2020년 현대자동차가 그린 가상 스마트시티 시나리오에서, **어떤 부분이 "인간 중심의 스마트시티"**라고 여겨지는가?
2. Whim의 대중교통 이용률 48%→74% 증가는 정말 MaaS 때문인가? 다른 설명 요인은 없는가?
