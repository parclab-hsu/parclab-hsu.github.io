...
**공역**은 항공기의 활동에 이용되는 3차원의 공중 영역으로, 안전한 운항을 위해 법·제도·관행으로 통제된다. 사용 목적에 따라 네 가지로 나뉜다.

| 구분 | 종류 |
|---|---|
| **관제공역** | 관제권(CTR), 관제구, 비행장교통구역 |
| **비관제공역** | 조언구역(미지정), 정보구역 |
| **통제공역** | 비행금지구역, 비행제한구역, **초경량비행장치 비행제한구역** |
| **주의공역** | 훈련구역, 군작전구역, 위험구역, 경계구역, **초경량비행장치 비행구역** |

- **관제권(CTR)** — 계기비행 항공기가 이착륙하는 공항 주위에 설정되는 공역. 공항 기준점(ARP)으로부터 **반경 5NM의 원통 구역**이며 통상 **5,000ft 이하**다.
- 드론 운용과 직접 맞닿는 것은 **통제공역**과 **주의공역**이다. 초경량비행장치 비행제한구역·비행구역이 여기에 속한다.

!!! tip "UTM과 어떻게 이어지는가"
    UTM은 **400ft 이하 저고도**를 자동화된 방식으로 관리하는 체계다. 그런데 그 저고도 공간도 이미 위와 같은 **공역 구분 위에 놓여 있다.** 비행금지구역·관제권 안에서는 UTM이 있다 해도 별도의 허가 절차를 밟아야 한다.
...
- 국토교통부, 『항공안전법』 제2조 · 공역 관련 규정 · 항공안전기술원 **K-드론 시스템**
