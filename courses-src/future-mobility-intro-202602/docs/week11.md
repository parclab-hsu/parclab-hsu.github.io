...
> 🔗 **강의 흐름** — 이 주차는 개론 과목에서 유일하게 **정량 설계**를 다룬다. 그래서 **기말고사 계산 문제 3문항이 전부 여기서 나온다.** 강성·안전계수·동력원·공진 — 네 가지 개념을 숫자로 다룰 수 있어야 한다.
...
!!! danger "기말 서술형 필수 — 30분 이상 체공을 위한 동력원 선정"
...
## 9. 레이아웃 및 로터 선정에 따른 성능 비교 — 기말 필수
...
!!! danger "기말 서술형 — 왜 더 무거운 옥토콥터가 더 오래 나는가?"
...
!!! example "기말 계산 예제 — 공진 위험 정량 분석 (실제 출제)"
    **문제**: 쿼드콥터 충격시험 결과 주요 고유진동수가 **27 Hz, 38 Hz, 151 Hz**에서 측정. **18×6.1 프로펠러** 장착, 호버링 시 약 **6,000 RPM** 회전. 프로펠러 통과주파수(BPF)를 계산하고(블레이드 **2매** 가정) 공진 위험을 정량 분석하시오.
...
1. 헥사콥터와 옥토콥터 중 **택배 드론**에 적합한 것은? 9주차의 "옥토콥터는 체공시간이 줄어든다"와 이번 주 계산표(옥토가 더 오래 난다)가 모순처럼 보이는데, 어떻게 화해시킬 수 있나? *(힌트: 같은 배터리 vs 같은 페이로드, 동축 반전 여부)*
2. 안전계수를 2.5로 잡은 설계와 1.4로 잡은 설계 중, **사람이 타지 않는 드론**에는 어느 쪽이 합리적인가? 판단 기준은?
3. 디지털 트윈으로 Virtual Flight Test를 하면 실물 시험을 얼마나 줄일 수 있을까? 줄일 수 **없는** 시험은 무엇인가?
