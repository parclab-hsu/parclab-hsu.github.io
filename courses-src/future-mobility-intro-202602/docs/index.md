...
!!! warning "중간고사 비중이 가장 크다"
    40%가 중간고사 한 번에 걸려 있다. 1~7주차(자율주행 파트)를 **개념 → 구조 → 기술 → 제도** 순서로 정리해 두는 것이 이 과목의 최대 승부처다. 시험 유형은 [시험 유형과 핵심 정리](exams.md) 참고.
...
!!! warning "자료의 기준 학기"
    주차별 강의 자료·커리큘럼·기출 분석은 **직전 운영분(2026-1학기)을 기준으로 정리**한 것이다. 주제 구성과 시험 유형은 그대로 이어지지만, **주차별 강의 일자와 최신 산업 동향 수치는 2026-2학기 개강 시점에 갱신**한다.
    이전 학기(2026-1학기) 목록에서도 이 페이지로 연결된다.

!!! note "이 사이트의 자료에 대하여"
    본 사이트의 주차별 페이지는 강의 슬라이드의 **핵심 개념·수치·용어를 강의자가 재정리한 학습 노트**다. 원본 슬라이드 이미지와 외부 기관 자료(보고서·기사·데이터)는 **포함하지 않으며**, 필요한 경우 출처만 표기했다. 수업 중 배포되는 원본 PDF는 학내 LMS를 통해 확인한다.
