...
!!! tip "자율주행 파트(1~7주차)"
    `2주차 개념·제도` → `5주차 레벨·규제` → `3주차 인지–판단–제어` → `6주차 센서 비교` → `7주차 V2X` 순으로 읽으면, 정의에서 시작해 시스템·센서·통신으로 이어지는 흐름이 자연스럽게 잡힌다.

!!! tip "드론·도시·로봇 파트(9~14주차)"
    `12주차 스마트시티 정의` → `11주차 구조설계(강성·FoS·동력원)` → `14주차 액추에이터 4종·모션 플래닝` → `10주차 UTM·법률` 순. 11주차의 정량 설계는 직접 계산해 보며 읽는다.
