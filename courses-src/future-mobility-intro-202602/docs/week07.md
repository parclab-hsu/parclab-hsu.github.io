...
> 💡 **기초 다지기** — **"센서는 보이는 것만 본다."** 카메라는 앞차 너머를 못 보고, 라이다는 코너 뒤를 못 본다. 그런데 앞차가 "여기 결빙 구간이야"라고 **말해 주면** 보지 않고도 알 수 있다. 이것이 V2X다. 자율주행이 "보는" 단계에서 **"데이터로 이해하는"** 단계로 넘어가는 것이다.
...
- **Handover and communication** — 제어권 넘김. 차가 운전하다 사람에게 넘길 수도, 다시 받아올 수도 있는데 이 과정에서 통신이 중요하다. 시스템 → 운전자: "지금 개입하세요". 차량 ↔ 클라우드 상태 공유
...
1. **중간고사 서술형 5번 연습** — 도심 자율주행 확산 과정에서 발생할 수 있는 **기술적·윤리적·법제도적 도전 과제**를 각각 1개 이상 제시하고 해결 방안을 논리적으로 서술해 보자.
...
2. Shift 10처럼 **신호등이 없는 도시**가 가능하려면 무엇이 전제되어야 하는가? 그 도시에서 정전이 나면?
3. 카카오모빌리티 사례처럼, 우리 학교 주변의 어떤 불편을 **모빌리티 데이터**로 설명할 수 있을까?
