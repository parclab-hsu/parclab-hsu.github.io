...
> 🔗 **강의 흐름** — 6주차 마지막 시나리오(안개 낀 야간 고속도로)에서 센서만으로는 부족하다는 것을 확인했다. 이번 주는 그 빈칸을 **통신**이 어떻게 메우는지 본다. V2X와 도심 자율주행의 도전과제, 그리고 **7단계 공학 설계 프로세스**가 이 주차의 내용이다.
...
!!! warning "V2X 유형별 대표 용도"
    **V2I**의 대표 목적은 **교통 사고 예방**이고, 대형 트럭 군집주행의 통신 방식은 **V2V**다.
    네 유형(V2V·V2I·V2P·V2G)의 정의와 대표 용도를 짝지어 정리해 둔다.
...
이 주차의 후반부는 **미래 자동차산업 아이디어 공모전** 준비와 연결된다. 도심 자율주행이 직면한 도전 과제와 해결 방안을 **7단계 공학 설계 프로세스 모델링 기법**에 따라 구조화하는 훈련이다.
...
!!! tip "설계 프로세스와 안전 표준"
    7단계 공학 설계 프로세스는 안전이 걸린 시스템에서 **ISO 26262의 V-사이클**과 **SOTIF의 검증 루프**라는 더 엄격한 형태로 나타난다. 요구사항 도출 → 검증 → 잔여 위험 평가 → 사양 확정으로 이어지는 흐름을 심화 자료에서 확인할 수 있다. → [🛡️ 심화 · 자율주행 안전 아키텍처 §11](safety-architecture.md)

## 🤔 생각해 봅시다

1. 도심 자율주행 확산 과정에서 발생할 수 있는 **기술적·윤리적·법제도적 도전 과제**를 각각 1개 이상 제시하고 해결 방안을 논리적으로 서술하시오.
...
▶️ **다음 주** — 8주차는 **중간고사 기간**이다. → [8주차](week08.md)
