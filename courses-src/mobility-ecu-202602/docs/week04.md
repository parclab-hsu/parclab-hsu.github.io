...
!!! tip "교수자용 — 4주차 운영 자료"
    [🗓️ 운영 키트](week04-kit.md) (180분 진행 대본·준비물·판서·시간 조절) · [🖨️ 배포용 워크시트](week04-worksheets.md) (인쇄해서 나눠 주는 양식)
...
> **로버 구동부 연결** — 벅으로 배터리 전압을 12V/5V로, 이후 LDO로 3.3V. 상세 부품값은 12주차.
...
> 📚 이번 주 용어의 자세한 설명과 그림은 **[주차별 용어 정리 — 4주차](terms-by-week.md#week04)** 에 있다.
...
- 전원회로의 중심 질문은 "전원 레일, 효율, 발열이 최종 로버 ECU에서 어떤 문제를 해결하는가"이다.
...
!!! tip "📌 실전 팁·주의점 (현장에서 자주 하는 실수)"
    벅 인덕터는 **포화전류를 부하보다 크게** 잡고, 스위칭 노드 배선은 **짧게**(노이즈원). 다이오드는 쇼트키를 쓴다.
...
4. 멀티미터로 무부하 전압과 부하 연결 후 전압강하를 비교한다. 결과를 다음 주차 산출물에 어떻게 반영할 것인가?
...
> 이 사례노트는 `전원 레일, 효율, 발열`를 실제 로버, 산업용 AMR, 차량 ECU 문제로 연결한다.
...
- "본 실험은 전원 레일, 효율, 발열을 로버 ECU 관점에서 검증하기 위해 수행하였다."
...
### 배터리 내부저항과 부하 시 전압 강하

로버는 경사에서 전류를 가장 많이 쓰는데, 바로 그때 배터리 전압이 가장 많이 떨어진다. 배터리를 이상적인 전압원으로 두면 등판 시험에서 예상 못 한 저전압 차단을 만난다.

```text
Vbat(부하) ≈ Vocv(SoC) - Ipeak × (Rcell + Rwire + Rconnector + Rfuse)
```

- 방전이 진행되면 `Vocv`가 내려가고 `Rcell`은 오히려 올라간다. 저온에서는 둘 다 더 나빠진다.
- 설계 기준은 만충 전압이 아니라 **"방전 말기 + 피크 전류" 조건의 최저 전압**이다. 이 값에서 12V·5V·3.3V regulator가 모두 dropout을 넘기는지 확인한다.
- 저전압 보호(UVLO·BMS 차단) 임계값은 이 순간 전압 강하보다 낮게 잡는다. 그렇지 않으면 경사 진입 순간마다 로버가 스스로 꺼진다.
- 측정으로 확인한다. 무부하 전압과 피크 부하 전압을 같은 지점에서 재고, 차이를 전류로 나누면 실효 내부저항이 나온다.
...
| 자료 | 수업 중 쓰는 장면 |
|---|---|
| [practical-circuits.pdf](attachments/practical-circuits.pdf) **(원본 자료)** | 전원회로와 컨버터 기본 설명 |
