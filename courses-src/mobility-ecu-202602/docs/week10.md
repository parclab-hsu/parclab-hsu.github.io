...
!!! tip "교수자용 — 공식 10주차 · STM32 펌웨어 2(EXTI·ADC·Timer·UART) 운영 자료"
    [🗓️ 운영 키트](week10-kit.md) · [🖨️ 배포용 워크시트](week10-worksheets.md)
...
> 📚 이번 주 용어의 자세한 설명과 그림은 **[주차별 용어 정리 — 10주차](terms-by-week.md#week10)** 에 있다.
...
- ADC·Timer·UART의 중심 질문은 "ADC, Timer, UART 통합이 최종 로버 ECU에서 어떤 문제를 해결하는가"이다.
...
> 이 사례노트는 `ADC, Timer, UART 통합`를 실제 로버, 산업용 AMR, 차량 ECU 문제로 연결한다.
...
- "본 실험은 ADC, Timer, UART 통합을 로버 ECU 관점에서 검증하기 위해 수행하였다."
...
### CubeMX 주변장치 설정과 생성 코드

ADC·Timer·UART는 CubeMX에서 몇 번의 클릭으로 설정되지만, 그 값이 어느 레지스터로 내려가는지 알아야 시험과 디버깅이 가능하다.

| CubeMX 설정 | 확인할 레지스터 |
|---|---|
| ADC 채널·sampling time | `ADC->SMPRx` |
| ADC trigger = TIM1 TRGO | `ADC->CR2` EXTSEL/EXTEN |
| PWM 주기·duty | `TIM->PSC`, `ARR`, `CCRx` |
| 상보 출력·dead time | `TIM1->CCER`, `BDTR` DTG |
| USART 115200 8-N-1 | `USART->BRR`, `CR1` |

- dead time은 ns로 입력하지만 실제로는 `BDTR`의 DTG 필드 값으로 양자화된다. 입력값과 생성값을 반드시 비교한다.
- baud rate와 타이머 주기는 CubeMX가 계산해 주더라도 앞 절의 식으로 검산하고 파형·터미널로 확인한다.
- Project Manager의 Code Generator에서 주변장치별 `.c/.h` 분리 생성을 켜면 생성 코드와 직접 작성한 코드의 경계가 분명해진다.
...
4. CubeMX에 dead time을 1µs로 입력해도 생성된 DTG 값이 정확히 1µs가 아닐 수 있는 이유를 설명한다.
...
- [ ] CubeMX로 ADC를 TIM1 TRGO 트리거로 설정하고 실제 샘플링 시점을 debug pin으로 확인
