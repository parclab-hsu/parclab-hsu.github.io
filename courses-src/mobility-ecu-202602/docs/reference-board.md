...
이 수업에는 담당 교수가 직접 설계한 **2축 BLDC 모터 드라이버 보드**가 레퍼런스로 따라온다. 회로도·BOM·CPL·레이어 이미지를 모두 공개하므로, 학생은 "완성된 실물"을 역으로 읽으며 각 주차의 이론이 어디에 쓰였는지 확인할 수 있다.
...
1. `Quantity` 내림차순으로 보면 보드의 성격이 보인다. 이 보드는 **10Ω 36개, 100nF 30개**가 최상위다. 게이트 저항과 디커플링이 압도적으로 많다는 뜻이고, 이는 스위칭 회로의 특징이다.
2. `Designator`를 세어 `Quantity`와 맞는지 확인한다. 여기서는 546개로 일치한다. 불일치는 곧 조립 사고다.
