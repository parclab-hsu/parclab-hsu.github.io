...
!!! info "두 보드의 역할"
    **레지스터 직접 제어 실습은 STM32F767 학습보드**로 한다. 이 강의의 코드·측정·과제는 모두 F767 기준이다.

    담당 교수가 설계한 레퍼런스 보드 [FM_SmartDriver V1](reference-board.md)은 **STM32H723ZGT6**를 쓰지만, 이 수업에서는 **회로도·BOM·PCB를 읽고 설계 근거를 설명하는 대상**이며 여기에 코드를 올리지는 않는다.
