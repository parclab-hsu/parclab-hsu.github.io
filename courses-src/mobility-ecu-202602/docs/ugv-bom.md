# 로버(AMR) 부품표(BOM) & 회로 예시

차동구동(2륜) AMR 기준. 교육용으로 입수·이해가 쉬운 **브러시 DC 기어모터 + 듀얼 H-브리지** 구성. H-브리지=6주차 풀브리지, 엔코더 속도=5·7주차, 벅·션트·BLE=과정과 그대로 연결.
...
고출력·무소음이 필요하면 브러시 DC 대신 **BLDC 모터 + 3상 인버터**(5·6·12·15주차 그대로)로 교체. 홀센서 6-step·유니폴라 PWM·션트 센싱이 AMR 구동부에 그대로 적용된다.
