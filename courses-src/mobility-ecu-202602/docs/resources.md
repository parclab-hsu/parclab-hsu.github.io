...
!!! danger "🔴 (변경 필요) — 저작권"
    아래 원본 첨부자료(PDF·슬라이드 미리보기)는 chcbaram 원본으로, 공개 배포 전 자작 자료 교체 또는 인용 허가가 필요하다.



> 공개 강의 페이지에서 바로 열 수 있도록 핵심 PDF/XLSX 자료를 선별해 연결했다. 원본 전동킥보드 BLDC 구동 자료는 로버·우주 로버 구동 ECU 관점으로 재해석해 사용한다.
