...
이 사이트는 단순 주차 목록이 아니라 **메인 강의자료 + 확장 강의자료 + 실습 코드 + 첨부자료**가 한 세트로 움직이도록 구성했다. 교수자는 메인 페이지를 강의 슬라이드처럼 사용하고, 확장 자료는 수업 중간 활동과 과제·평가로 연결한다.

| 자료 묶음 | 수업에서의 역할 | 학생이 남길 증거 |
|---|---|---|
| 주차별 메인 자료 | 3시간 수업의 기본 슬라이드, 핵심 이론, 용어 박스, 실습 전환 포인트 | 필기, 핵심 공식, 이해 확인 답안 |
| 심화 강의노트 | 첨부 PDF 이미지와 도해를 확대해 설명하는 교수자용 판서 자료 | 그림 위 신호 흐름 표시, 계산 근거 |
| 실습 코칭노트 | 팀별 실습 절차, 코칭 질문, 오류 분류 기준 | 사진, 측정값, UART/Teleplot 로그 |
| 평가·문제팩 | 10분 퀴즈, 계산·해석 문제, 구두 발표 질문 | 풀이 과정, 단위, 판단 근거 |
| 현장 사례노트 | 실제 고장처럼 생각하는 진단 시나리오 | 원인 가설, 측정 순서, 재발 방지안 |
| 실습 코드 | 8~10주차 STM32 기초와 5~7·15주차 구동 통합의 코드 골격 | 코드 수정점, 빌드/로그 캡처 |

!!! info "한 주차를 강의하는 권장 순서"
    `메인 자료의 개념도 → 첨부자료 이미지 → 핵심 이론 보강 → 통합 강의 슬라이드 → 실습 코칭노트 → 평가·사례노트` 순서로 진행하면, 3학년 학생이 이론·회로·펌웨어·최종 프로젝트의 연결을 놓치지 않는다.
