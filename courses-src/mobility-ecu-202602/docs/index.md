...
| 도구 | STM32H7 계열(레지스터 직접제어, HAL 미사용), 로버 구동 모터 + 한성대 보드, EasyEDA(회로/PCB), STM32CubeIDE, 로버 섀시 키트, UART/Teleplot |
...
!!! success "개편 핵심"
    ...
    - **캡스톤 고도화**: 로버(AMR) 2륜 구동을 우주 로버 미션형 설계·디버깅·검증 프로젝트로 확장한다.
...
- **캡스톤(심화)**: [로버(AMR) 개발 프로젝트](ugv-project.md) — WIZnet 사례를 각색해 직접 만들며 마이크로프로세서를 이해하고 개발 자신감을 키운다.
