...
> **우주 로버 대회 참가 프로젝트**로서, **월면(月面) 험지주행·경사극복 미션**을 수행하는 자작 로버의 **구동 제어유닛(ECU)**을 전자부품 기초부터 회로·PCB·STM32 펌웨어까지 **팀 기반으로 설계·제작·통합**하고, 최종적으로 대회 미션(등판각·완주)을 수행·시연한다. 상세는 [강의계획서](syllabus.md).
...
| 항목 | 내용 |
|------|------|
| 교과목 | 모빌리티 전자제어유닛 실습 (3학년 중심) |
| 학과 | 미래모빌리티학과 |
| 운영 | 15주 · 주당 3시간 · 총 45시간 |
| 구성 | 이론 강의 1.5h + 실습/프로젝트 1.5h (주 3시간) · 팀 기반 운영 |
| 도구 | **실습**: STM32F767 학습보드(레지스터 직접제어) · **읽기용 레퍼런스**: FM_SmartDriver V1(STM32H723ZGT6, 2축 BLDC, 6층), 로버 구동 모터, EasyEDA(회로/PCB), STM32CubeIDE, 로버 섀시 키트, UART/Teleplot |
...
상세는 [강의계획서(Syllabus)](syllabus.md) 참고.
...
!!! success "개편 핵심"
    ...
    - **캡스톤 고도화**: 로버 2륜 구동을 우주 로버 미션형 설계·디버깅·검증 프로젝트로 확장한다.
...
| 8주차 | 요구사항 추적표, HSI, 시스템 블록도, 중간 설계 리뷰 결함 목록 | 하드웨어-소프트웨어 계약 확정 |
| 9~11주차 | GPIO/EXTI/ADC/Timer/UART 코드와 로그, 6-step 구동·보호 로직 분석 | 펌웨어 모듈과 진단 채널 |
| 12~14주차 | 게이트드라이버·보호 회로 리뷰, 회로도, PCB 배치와 노이즈 검토 | 제작 가능한 하드웨어 설계 |
| 15주차 | 통합 펌웨어, 안전 상태, 실패 시나리오 로그, 발표 자료 | 로버 구동 ECU 검증 |
...
- **팀 프로젝트(최종)**: 로버 제어유닛 완성·통합 시험 + 월면 험지주행 대회 미션 수행·시연 (등판각·완주 채점)
- **캡스톤(심화)**: [로버 개발 프로젝트](ugv-project.md) — WIZnet 사례를 각색해 직접 만들며 마이크로프로세서를 이해하고 개발 자신감을 키운다.
