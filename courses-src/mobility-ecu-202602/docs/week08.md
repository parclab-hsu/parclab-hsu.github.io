...
> **학습목표** — ARM/Cortex-M 구조와 STM32H7 계열 메모리맵을 이해하고, 포인터 기반 레지스터 직접 접근으로 GPIO 출력(LED)을 구현한다. 첨부 F767 자료는 레지스터 직접 제어 원리를 비교 설명하는 예시로 사용한다.
...
> 🧭 **하드웨어 자료 구분** — 수업 기준 MCU는 **STM32H7 계열(레지스터 직접 제어, HAL 미사용)**이다. 일부 첨부자료는 F767(레지스터 직접접근), F103(HAL), ESP32(Arduino) 예시이므로, 교수자는 “원리 비교 자료”와 “이번 학기 구현 기준”을 분리해 설명한다.
...
!!! note "H7 구현 전 확인"
    아래 레지스터 시퀀스와 코드는 **F767 비교 예시**다. 이번 학기 구현은 STM32H7 보드의 레퍼런스 매뉴얼과 헤더 파일 기준으로 포트, 버스, 클럭 Enable 비트, 핀 번호를 다시 확인해 적용한다.
...
!!! note "슬라이드 4 · 레지스터 직접접근과 HAL을 비교한다"
    직접접근은 원리를 투명하게 보여주고 HAL은 개발 생산성을 높인다. 수업 구현 기준은 STM32H7 레지스터 직접 제어이며, F767/F103 자료는 추상화 수준을 비교할 때 사용한다.
...
> 이 사례노트는 `레지스터와 GPIO 초기화`를 실제 로버, 전동킥보드, 로버, 차량 ECU 문제로 연결한다.
...
HAL 없이 레지스터에 직접 값을 써서 LED를 켜는 **F767 비교 예제**다. 클럭→모드→출력 순서가 핵심이며, H7 실습에서는 보드 회로도와 레퍼런스 매뉴얼에 맞게 포트·핀·클럭 레지스터를 바꾼다. [⬇ 전체 코드](code/week08_gpio_led.c)
