...
> 🧭 **하드웨어 자료 구분** — 수업 기준 MCU는 **STM32F767(레지스터 직접 제어, HAL은 비교 학습용)**이다. 첨부자료 중 F103은 HAL, ESP32는 Arduino 예시이므로, 교수자는 “원리 비교 자료”와 “이번 학기 구현 기준”을 분리해 설명한다.
...
!!! note "H7 구현 전 확인"
    아래 레지스터 시퀀스와 코드는 **STM32F767 기준**이다. 보드가 바뀌면 레퍼런스 매뉴얼과 헤더 파일 기준으로 포트, 버스, 클럭 Enable 비트, 핀 번호를 다시 확인해 적용한다.
