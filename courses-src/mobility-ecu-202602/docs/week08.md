# 8주차 · STM32 GPIO·레지스터 기초

> 🔗 **강의 흐름** — 이론(1~7주차)을 이제 **STM32 펌웨어**로 옮긴다. 이번 주는 레지스터 직접 접근(GPIO). 다음 주는 입력·인터럽트.

!!! tip "교수자용 — 공식 9주차 · STM32 펌웨어 1(레지스터·GPIO·클럭) 운영 자료"
    [🗓️ 운영 키트](week09-kit.md) · [🖨️ 배포용 워크시트](week09-worksheets.md)
...
> 🧭 **하드웨어 자료 구분** — 실습 기준 MCU는 **STM32F767(레지스터 직접 제어, HAL은 비교 학습용)**이다. 첨부자료 중 F103은 HAL, ESP32는 Arduino 예시이므로, 교수자는 “원리 비교 자료”와 “이번 학기 구현 기준”을 분리해 설명한다. 레퍼런스 보드(STM32H723)는 회로를 읽는 대상이며 실습 대상이 아니다.
...
!!! note "구현 전 확인"
    아래 레지스터 시퀀스와 코드는 **이번 학기 실습 기준인 STM32F767**에 맞춘 것이다. 다른 보드로 옮길 때는 레퍼런스 매뉴얼과 헤더 파일 기준으로 포트, 버스, 클럭 Enable 비트, 핀 번호를 다시 확인해야 한다.
...
!!! note "슬라이드 4 · 레지스터 직접접근과 HAL을 비교한다"
    직접접근은 원리를 투명하게 보여주고 HAL은 개발 생산성을 높인다. 수업 구현 기준은 STM32F767 레지스터 직접 제어이며, F103 HAL 자료는 추상화 수준을 비교할 때 사용한다.
...
- STM32 GPIO·메모리맵·레지스터의 중심 질문은 "레지스터와 GPIO 초기화가 최종 로버 ECU에서 어떤 문제를 해결하는가"이다.
...
4. HAL 코드 한 줄이 어떤 레지스터 비트와 대응되는지 주석으로 남긴다. 결과를 9주차 입력·클럭·인터럽트 실습에서 어떻게 확장할 것인가?
...
> 이 사례노트는 `레지스터와 GPIO 초기화`를 실제 로버, 산업용 AMR, 차량 ECU 문제로 연결한다.
...
- "본 실험은 레지스터와 GPIO 초기화를 로버 ECU 관점에서 검증하기 위해 수행하였다."
...
| 시간 | 활동 | 학생 산출물 |
|---|---|---|
| 0:00-0:20 | 7주차 PI 제어 로그와 이번 주 GPIO/레지스터 목표 연결 | 펌웨어 입출력 목록 |
...
| 자료 | 수업 중 쓰는 장면 |
|---|---|
| [stm32-lecture-v0.2.pdf](attachments/stm32-lecture-v0.2.pdf) **(원본 자료)** | STM32 레지스터 직접접근 흐름의 주교재 |
| [stm32-gpio-exti.pdf](attachments/stm32-gpio-exti.pdf) **(원본 자료)** | GPIO 설정과 EXTI 선행 자료 |
| [stm32-orientation-dev-env.pdf](attachments/stm32-orientation-dev-env.pdf) **(원본 자료)** | 개발환경 점검 자료 |
...
HAL 없이 레지스터에 직접 값을 써서 LED를 켜는 **STM32F767 실습 예제**다. 클럭→모드→출력 순서가 핵심이며, 보드가 바뀌면 회로도와 레퍼런스 매뉴얼에 맞게 포트·핀·클럭 레지스터를 확인해 적용한다. [⬇ 전체 코드](code/week08_gpio_led.c)

```c
/*
 * [8주차] GPIO 출력 — 레지스터 직접 접근 (STM32F767 실습 예제)
 * ---------------------------------------------------------------
