...
!!! tip "교수자용 — 공식 11주차 · BLDC 6-step 구동 펌웨어 — 이 페이지의 Hall 정류·상태기계 부분이 해당한다 운영 자료"
    [🗓️ 운영 키트](week11-kit.md) · [🖨️ 배포용 워크시트](week11-worksheets.md)

!!! tip "교수자용 — 공식 15주차(통합 검증·최종 발표) 운영 자료"
    이 페이지는 **공식 15주차 · 통합 검증과 최종 발표**에도 해당한다.
    [🗓️ 운영 키트](week15-kit.md) (pre-flight gate·시연 6단계·평가표 45%) · [🖨️ 배포용 워크시트](week15-worksheets.md) (요구사항 추적표·시연 기록)
...
> 이 사례노트는 `통합 펌웨어와 최종 검증`를 실제 로버, 산업용 AMR, 차량 ECU 문제로 연결한다.
...
Hall·전류·온도 외에 **시간 축의 고장**도 진단 목록에 넣는다. 통합 단계에서 실제로 자주 나오는 것은 이쪽이다.

- **제어 주기 초과(overrun)**: 주기 태스크가 정해진 시간 안에 끝나지 못하면 정류와 속도제어가 함께 어긋난다. 태스크 시작·종료를 GPIO toggle로 찍어 실제 실행 시간을 측정한다.
- **상위 명령 timeout**: UART·블루투스 명령이 정해진 시간 이상 오지 않으면 스스로 감속·정지한다. 통신이 끊긴 로버가 마지막 속도로 계속 가는 것을 막는다.
- **watchdog reset 이력**: IWDG/WWDG refresh는 주기 태스크가 한 바퀴를 정상적으로 돌았을 때만 한다. reset 후에는 `RCC->CSR` 플래그로 원인을 기록하고 PWM이 꺼진 상태에서 시작한다.
- **채널 간 불일치**: 좌우 RPM이 같은 명령에서 크게 벌어지면 한쪽 Hall·배선·기구 문제다. 양쪽을 함께 정지시킨다.
...
로버는 정류 상태기계를 좌우 각각 하나씩 돌린다. 명령은 `v, ω → vL, vR → RpmRefL/R → 채널별 PI → 채널별 6-step 정류` 순으로 내려간다.

- 역방향은 duty만 줄이는 것이 아니라 정류 순서를 반대로 적용한다. 명령 방향과 측정된 Hall 진행 방향이 다르면 fault로 처리한다.
- 좌우 모터는 마주 보게 장착되는 경우가 많다. "로버 전진"과 "모터 CW"의 관계를 채널마다 HSI에 정의한다.
- 한 채널에서 과전류·Hall 오류·stall이 나면 그 채널만 끄지 말고 양쪽을 함께 정지시킨다. 한쪽만 멈추면 로버가 제자리에서 돈다.

섀시에 올린 뒤에는 단계적으로 확장하고, 실패하면 이전 단계로 돌아간다.

| 단계 | 조건 | 통과 기준 |
|---|---|---|
| 벤치 | 모터 분리 | 파형·deadtime, fault 차단 |
| 무부하 회전 | 바퀴 공중 부양 | 방향 일치, RPM 추종 |
| 차동 동작 | 바퀴 공중 부양 | v/ω가 좌우 RPM으로 분배 |
| 저속 직진 | 지면, duty 제한 | 5m 경로 이탈·좌우 편차 기록 |
| 선회·제자리 회전 | 지면 | 최소 회전반경, 전류 상승 확인 |
| 경사·험지 | 목표각부터 단계 상승 | 실패 지점 각도와 원인 기록 |

튜닝 결과는 직진 편차(5m당 m), 최소 회전반경(m), 등판 실패각(°), 최대 전류(A)로 기록한다.
...
4. 바퀴를 공중에 띄운 시험만으로 평지 주행 미션 통과를 주장할 수 없는 이유를 설명한다.
...
- [ ] 지면 저속 직진 5m·선회 시험 후 직진 편차와 최소 회전반경 기록
