...
## 🔧 OT 소개: 우리가 15주 뒤에 읽어낼 실물

담당 교수가 직접 설계한 **2축 BLDC 드라이버 `FM_SmartDriver V1`** 이 이 수업의 레퍼런스 보드다. 오늘 배우는 요구사항부터 15주차 통합 검증까지, 모든 주차가 이 한 장의 어딘가를 설명한다.
...
세계 최고 수준의 공대도 "제품을 만든다"를 정규 과목으로 가르친다. 그 방식이 이 수업의 15주 구조와 상당 부분 같다.
...
**우리 수업과의 관계** — ME218의 주제 목록은 이 수업의 주차 구성과 거의 그대로 겹친다. 트랜지스터·전원(2~4주차), PWM(6주차), 레지스터·타이머·인터럽트(9~10주차), 상태기계(11주차), 노이즈(14주차)가 모두 ME218 A·B의 항목이다. 차이는 **스탠퍼드가 3개 과목·9개월에 나눠 하는 것을 15주 한 과목에 압축**한다는 점이다. 그래서 이 수업은 "완성품 제작"이 아니라 **"요구사항–회로–펌웨어–검증이 연결된 설계 설명 능력"** 을 평가한다.
