...
!!! tip "교수자용 — 1주차 OT 운영 자료"
    [🗓️ 운영 키트](ot-kit.md) (180분 진행 대본·준비물·판서·시간 조절) · [🖨️ 배포용 워크시트](ot-worksheets.md) (인쇄해서 나눠 주는 양식)
...
> 📚 이번 주 용어의 자세한 설명과 그림은 **[주차별 용어 정리 — 1주차](terms-by-week.md#week01)** 에 있다.
...
- 이번 주 설명은 선수지식 진단, 실습 안전, 15주 프로젝트 구조를 하나의 로버 ECU 개발 흐름으로 묶는 데 초점을 둔다.
...
##### 0:00-0:20 · 사전지식 진단
오리엔테이션 및 로버 제어유닛 구조 수업은 학생이 이미 알고 있는 전원, 모터, MCU, 센서 개념을 짧게 확인하며 시작한다. 오늘의 `ECU 전체 구조와 2륜 차동구동`에서 낯선 용어를 표시하게 하고, 이후 15주 동안 채워 넣을 학습 지도를 만든다.
...
4. 최종 로버 블록도를 종이에 그리고 각 선을 전원선과 신호선으로 구분한다. 결과를 2주차 전자부품 학습에서 어떤 회로 블록으로 이어 볼 것인가?
...
> 이 사례노트는 `ECU 전체 구조와 2륜 차동구동`를 실제 로버, 산업용 AMR, 차량 ECU 문제로 연결한다.
...
- **기능 요구사항**: 무엇을 해야 하는가. 예: 스로틀 입력에 따라 모터 속도를 제어한다.
- **성능 요구사항**: 어느 수준까지 해야 하는가. 예: 정상상태 속도 오차를 ±5% 이내로 유지한다.
- **안전 요구사항**: 고장 때 허용할 상태는 무엇인가. 예: Hall 신호가 100ms 이상 갱신되지 않으면 PWM을 차단한다.

요구사항 한 문장에는 주어, 조건, 행동, 수치, 단위를 넣고 시험 방법을 함께 정한다. “잘 동작한다”처럼 시험자가 PASS/FAIL을 결정할 수 없는 문장은 요구사항으로 사용하지 않는다.
...
설계 단계마다 대응 시험을 미리 연결한다. 멀티미터 값, 오실로스코프 파형, UART 로그, 주행 거리처럼 재현&nbsp;가능한 증거를 요구사항 ID와 함께 보관한다.

### ECU 시간 예산

<div class="table-scroll" markdown tabindex="0" role="region" aria-label="ECU 시간 예산 표">

| 기능 | 실행 방식 | 허용 주기·지연 예 | 지연 시 영향 |
|---|---|---:|---|
| 과전류 차단 | 하드웨어 비교기·고우선 ISR | 수 µs 이하 | MOSFET 손상 |
| Hall 정류 | EXTI | edge 직후 | 토크 저하·역회전 |
| 속도 PI | 주기 태스크 | 1~10ms | 응답 불안정 |
| 온도 감시 | 저속 태스크 | 100ms | 열 보호 지연 |
| 상태 통신 | 백그라운드·DMA | 100~500ms | 로그 지연 |

</div>

### 미니 FMEA 실습

<div class="table-scroll" markdown tabindex="0" role="region" aria-label="미니 FMEA 표">

| 고장 형태 | 시스템 영향 | 탐지 방법 | 안전 상태 |
|---|---|---|---|
| 스로틀 단선 | 입력 부동·최대값 | ADC 정상범위 검사 | PWM 차단 |
| Hall 오류 | 정류 순서 오류 | 000/111·불법 전이 검사 | PWM 차단 |
| 저전압 | MCU 반복 reset | Vdc ADC·UVLO | 출력 비활성 |

</div>

### 요구사항을 검증 문장으로 바꾸기

팀에서 쓴 요구사항을 아래 표로 옮긴다. 검증 방법 칸을 채우지 못하는 요구사항은 아직 요구사항이 아니라 희망이다. 이 표는 뒤쪽 설계 리뷰의 추적표와 최종 인수시험 표의 첫 행이 된다.

<div class="table-scroll" markdown tabindex="0" role="region" aria-label="요구사항 검증 변환 표">

| ID | 요구사항 문장 | 구분 | 측정량과 판정 기준 | 검증 방법 | 검증 주차 |
|---|---|---|---|---|---|
| R-01 | 스로틀 입력에 비례해 좌우 바퀴 속도를 제어한다 | 기능 | 명령 대비 정상상태 오차 ±5% 이내 | Teleplot 속도 응답 기록 | 7 |
| R-02 | Hall 신호가 100ms 이상 갱신되지 않으면 PWM을 차단한다 | 안전 | 차단까지 지연 ≤ 120ms | Hall 배선 분리 시험 | 15 |
| R-03 | | | | | |

</div>

- 한 문장에 동작 하나만 쓰고, 주어·조건·행동·수치·단위를 모두 넣는다.
- “빠르게”, “안정적으로”, “충분히”는 수치로 바꾸거나 지운다.
- 안전 요구사항은 정상 동작이 아니라 **고장 났을 때 무엇이 되어야 하는가**를 쓴다.
- 검증 주차를 적어 두면 그 주차 실습이 왜 필요한지 학생이 스스로 연결한다.
...
| 자료 | 수업 중 쓰는 장면 |
|---|---|
| [kit-guide.pdf](attachments/kit-guide.pdf) **(원본 자료)** | 키트 전원·커넥터·구성품 확인 실습의 기준 자료 |
| [stm32-orientation-dev-env.pdf](attachments/stm32-orientation-dev-env.pdf) **(원본 자료)** | 개발환경 설치와 보드 연결 점검 자료 |
...
담당 교수가 직접 설계한 **2축 BLDC 드라이버 `FM_SmartDriver V1`** 이 본 강의의 레퍼런스 보드다. 오늘 배우는 요구사항부터 15주차 통합 검증까지, 모든 주차가 이 한 장의 어딘가를 설명한다.

![FM_SmartDriver V1 3D 렌더 — 상단 BLDC Motor-1, 하단 BLDC Motor-2의 3상 인버터가 좌우 대칭으로 배치되고 가운데 STM32H723ZGT6가 두 축을 함께 제어한다](figures/board/3d.jpg)

| 항목 | 값 |
|---|---|
| 용도 | 좌/우 2축 BLDC 구동 제어유닛 (로버 차동 구동) |
| MCU | STM32H723ZGT6 (LQFP-144) — **읽기용**. 레지스터 실습은 F767 학습보드로 한다 |
| 층수 · 크기 | 6층 · 약 220 × 220 mm |
| 부품 | 146종 546개, 실장 558점 전량 Top면 |
| 전력단 | 100V MOSFET 12개 = 2축 × 3상 × (하이/로우) |

상단 절반이 Motor-1, 하단 절반이 Motor-2다. **같은 3상 인버터를 두 벌 놓은 좌우 대칭 구조**가 곧 로버의 좌우 바퀴다. 5주차의 차동 구동 운동학, 7주차의 좌우 2채널 PI, 11주차의 채널별 정류 상태기계가 모두 이 대칭에서 나온다.

회로도 9시트·BOM·CPL·6층 레이어 이미지는 [레퍼런스 보드 페이지](reference-board.md)에서 모두 내려받을 수 있다.

## 🎓 OT 소개: 스탠퍼드는 "제품 개발"을 어떻게 가르치는가

세계 최고 수준의 공대도 "제품을 만든다"를 정규 과목으로 가르친다. 그 방식이 본 강의의 15주 구조와 상당 부분 같다.

| 과목 | 무엇을 훈련시키는가 | 운영 방식 |
|---|---|---|
| **ME218 A·B·C** — Smart Product Design | "스마트 제품 = 마이크로프로세서로 기능이 확장된 제품". 트랜지스터 스위칭, 상태기계, C 레지스터 프로그래밍, 타이머·PWM·인터럽트, OP앰프, 센서/액추에이터 인터페이싱, 노이즈, 전원 | 3개 분기 연속 수강. 매 분기 공통 주제로 팀별 경진 |
| **ME103 / ME203** — Design and Manufacturing | 의미 있는 제품을 골라 관점을 세우고 직접 만든다. 래피드 프로토타입·CAD·제조 시험 모델, 양산 관점 재설계 | Product Realization Lab에서 제작, 공개 발표 "Meet the Makers" |
| **ME310 A·B·C** — Design Innovation | 기업이 낸 실제 과제를 국제 협업으로 푼다 | 9개월. 스탠퍼드 팀 3~4명 + 해외 파트너 대학 팀, 기업 후원. 6월 쇼케이스 "EXPE" |

![스탠퍼드 제품 개발 교육 3갈래와 본 강의의 위치 — ME218(임베디드 구현)·ME203(제조)·ME310(문제 정의)이 각각 본 강의의 어느 주차로 들어오는지 보여주는 자작 도식](figures/stanford_product_courses.svg)

**본 강의와의 관계** — ME218의 주제 목록은 이 수업의 주차 구성과 거의 그대로 겹친다. 트랜지스터·전원(2~4주차), PWM(6주차), 레지스터·타이머·인터럽트(9~10주차), 상태기계(11주차), 노이즈(14주차)가 모두 ME218 A·B의 항목이다. 차이는 **스탠퍼드가 3개 과목·9개월에 나눠 하는 것을 15주 한 과목에 압축**한다는 점이다. 그래서 본 강의는 "완성품 제작"이 아니라 **"요구사항–회로–펌웨어–검증이 연결된 설계 설명 능력"** 을 평가한다.

!!! tip "수업 중 활동 — 실제 프로젝트 구경하기"
    [ME218 프로젝트 아카이브](https://me218archive.stanford.edu/)에서 최근 한 해를 열어 함께 본다. 2024–2025년 주제는 ME218A "Interactive Elaborate Ethereal Entertainment", ME218B "CARRY Us to Victory: Build Me a Forklift", ME218C "Jailbreak!! Navigate the Flood to Escape the Warehouse"였다.

    1. 저 프로젝트들과 우리 로버 ECU는 기술적으로 무엇이 같고 무엇이 다른가?
    2. 한 학기에 저 수준까지 가려면 15주 중 어디를 덜어내야 하는가?
    3. 우리 팀은 15주 뒤에 무엇을 "보여줄" 것인가? 한 문장으로 적어 본다.

**출처** — [Smart Product Design Lab (ME218)](http://design.stanford.edu/spdl/spdlvisitor.html) · [ME218 아카이브](https://me218archive.stanford.edu/) · [ME310 Global Engineering Design Innovation](https://me310.stanford.edu/) · [Product Realization Lab (ME103/ME203)](https://productrealization.stanford.edu/courses/me) · 더 넓은 비교는 [국내외 대학·대회 동향](global-benchmark.md) 참고. (조사 시점 2026-09)
