...
> 근거: UAV개발/STM32_강의자료_Ver0.2.pdf(F767 레지스터), ST마이크로프로세서/(F103 HAL), 모빌리티센서/(ESP32 Arduino)

## ⚠️ 자료가 3개 하드웨어 계열로 분리됨 (핵심 함정)
| 폴더 | 하드웨어 | 방식 |
|---|---|---|
| ST마이크로프로세서/ | STM32F103RB (Cortex-M3, 72MHz) NUCLEO+EVB | CubeIDE+CubeMX+**HAL** |
| UAV개발/STM32_강의자료 | STM32F767 (Cortex-M7, 216MHz) 교육보드 | **포인터 레지스터 직접접근** |
...
- 교육흐름 권장: 레지스터로 원리(F767) → HAL로 생산성(F103).
