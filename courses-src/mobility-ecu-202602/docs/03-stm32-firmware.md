...
!!! note "이번 학기 구현 기준"
    실습 기준 MCU는 **STM32F767**이며, HAL 없이 레지스터 직접 제어로 원리를 확인한다. F103(HAL)·ESP32(Arduino) 첨부자료는 주변장치 구조와 추상화 수준을 비교하기 위한 참고 예시로만 사용한다. 레퍼런스 보드 FM_SmartDriver V1(STM32H723)은 회로도·PCB를 읽는 대상이며, 여기에 실습 코드를 올리지는 않는다.
