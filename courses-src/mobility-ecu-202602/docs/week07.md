...
!!! tip "교수자용 — 7주차 운영 자료"
    [🗓️ 운영 키트](week07-kit.md) (180분 진행 대본·준비물·판서·튜닝 절차) · [🖨️ 배포용 워크시트](week07-worksheets.md) (인쇄해서 나눠 주는 양식)
...
> 📚 이번 주 용어의 자세한 설명과 그림은 **[주차별 용어 정리 — 7주차](terms-by-week.md#week07)** 에 있다.
...
- 제어공학·PI 속도제어의 중심 질문은 "PI 제어와 로그 튜닝이 최종 로버 ECU에서 어떤 문제를 해결하는가"이다.
...
4. Ki 추가 후 정상상태오차가 줄었는지 그래프 위에서 확인한다. 결과를 8주차 STM32 펌웨어 구현에서 어떤 변수와 로그 항목으로 옮길 것인가?
...
> 이 사례노트는 `PI 제어와 로그 튜닝`를 실제 로버, 산업용 AMR, 차량 ECU 문제로 연결한다.
...
- "본 실험은 PI 제어와 로그 튜닝을 로버 ECU 관점에서 검증하기 위해 수행하였다."
...
차동 구동 로버는 속도 제어기가 두 개다. 같은 명령을 주어도 마찰·무게 배분·모터 편차 때문에 좌우 속도가 달라져 로버가 휜다.

- 좌우 각각 독립 PI로 바퀴 속도를 맞춘다. 적분항과 anti-windup은 채널마다 따로 관리한다.
- 남는 직진 편차는 좌우 속도 오차나 yaw 값으로 보정한다. 보정량은 `vL - Δ/2`, `vR + Δ/2`처럼 두 명령에 나눠 넣는다.
- 한 채널이 duty·전류 한계에 걸리면 그쪽만 뒤처져 로버가 돈다. 포화 시 양쪽 명령을 함께 낮춰 좌우 비율을 유지한다.
- 로그에는 `RpmRefL/R`, `RpmFdbL/R`, `DutyL/R`을 함께 남긴다. 한쪽만 보면 편차 원인을 구분할 수 없다.
...
| 2:25-3:00 | 8주차 STM32 구현을 위한 제어 변수·로그 항목 정리 | 펌웨어 입력/출력 목록 |

## 📎 수업자료 활용

| 자료 | 수업 중 쓰는 장면 |
|---|---|
| [motor-control-inverter-theory.pdf](attachments/motor-control-inverter-theory.pdf) **(원본 자료)** | 제어공학과 PI 속도제어 주교재 |
| [speed-control-teleplot.pdf](attachments/speed-control-teleplot.pdf) **(원본 자료)** | 속도 로그 시각화와 Teleplot 사용 자료 |
...
4. 좌우 중 한 채널만 포화될 때 로버에 생기는 현상과 대응을 설명한다.
...
- [ ] 로버 직진 속도 PI 제어 응답 그래프 제출
- [ ] 좌우 2채널 PI 로그를 동시에 기록해 직진 편차 원인 분석
