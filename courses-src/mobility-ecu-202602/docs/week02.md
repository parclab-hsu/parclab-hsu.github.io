...
!!! tip "교수자용 — 2주차 운영 자료"
    [🗓️ 운영 키트](week02-kit.md) (180분 진행 대본·준비물·판서·시간 조절) · [🖨️ 배포용 워크시트](week02-worksheets.md) (인쇄해서 나눠 주는 양식)
...
> 📚 이번 주 용어의 자세한 설명과 그림은 **[주차별 용어 정리 — 2주차](terms-by-week.md#week02)** 에 있다.
...
4. 커패시터 용량을 바꿨을 때 노이즈가 줄어드는 위치와 줄어들지 않는 위치를 구분한다. 결과를 다음 주차 산출물에 어떻게 반영할 것인가?
...
> 이 사례노트는 `수동소자와 ADC 입력 보호`를 실제 로버, 산업용 AMR, 차량 ECU 문제로 연결한다.
...
R=10kΩ, C=100nF이면 `fc ≈ 159Hz`, `τ = 1ms`이다. 필터를 강하게 하면 노이즈는 줄지만 스로틀 응답 지연은 커진다. STM32 ADC의 sample-and-hold 커패시터는 입력에서 순간적으로 전하를 가져간다. 분압 저항이 너무 크면 sampling time 안에 안정화가 끝나지 않아 ADC 전압 오차가 생긴다.

### 42V 배터리 분압 예제

`Rtop=150kΩ`, `Rbottom=10kΩ`일 때:

```text
Vadc,max = 42V × 10kΩ / 160kΩ = 2.625V
Idivider = 42V / 160kΩ = 262.5µA
Ptop ≈ I² × 150kΩ = 10.3mW
```
...
| 자료 | 수업 중 쓰는 장면 |
|---|---|
| [practical-circuits.pdf](attachments/practical-circuits.pdf) **(원본 자료)** | 수동소자 정격, 전압분배, 디커플링 설명의 주교재 |
