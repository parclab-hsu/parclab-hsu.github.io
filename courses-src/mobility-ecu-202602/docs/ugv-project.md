# 로버(AMR) 개발 프로젝트 — 직접 만들며 배우는 마이크로프로세서 (캡스톤)

자율이동로봇(AMR, Autonomous Mobile Robot)을 직접 기획·설계·제작하며 마이크로프로세서를 체득하는 통합 프로젝트. WIZnet IoT 스피커 프로젝트(chcbaram)의 "처음부터 끝까지 과정을 공개하며 완주"하는 개발 방식을 지상 로버로 각색한다.
