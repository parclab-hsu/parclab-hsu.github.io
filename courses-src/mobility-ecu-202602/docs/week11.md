...
!!! tip "교수자용 — 공식 8주차(중간 설계 리뷰) 운영 자료"
    이 페이지는 **공식 강의계획서의 8주차 · 중간 설계 리뷰**에 해당한다.
    [🗓️ 운영 키트](week08-kit.md) (발표 시간 배분·평가표 20%·결함 등급 판정) · [🖨️ 배포용 워크시트](week08-worksheets.md) (자가 점검·HSI 계약 양식·상호 리뷰표)
...
> 📚 이번 주 용어의 자세한 설명과 그림은 **[주차별 용어 정리 — 11주차](terms-by-week.md#week11)** 에 있다.
...
> 이 사례노트는 `요구사항과 HSI`를 실제 로버, 산업용 AMR, 차량 ECU 문제로 연결한다.
...
### CubeMX Pinout으로 HSI를 검증한다

HSI 표를 손으로만 작성하면 핀 충돌과 alternate function 오류가 발표 직전에 발견된다. Pinout & Configuration에서 다음을 확인하고 그 결과를 HSI 표의 근거로 남긴다.

- 같은 핀을 두 기능이 요구하는 충돌과 대체 가능한 핀 후보
- TIM1 상보 출력(CHx/CHxN)처럼 반드시 쌍으로 배정해야 하는 신호
- SWD(PA13/PA14), NRST, BOOT0, 오실레이터 핀처럼 침범하면 안 되는 예약 핀
- ADC 채널이 해당 ADC 인스턴스에 실제로 존재하는지 (`ADC1_IN0`, `ADC2_IN7` 등)
- 5V 신호가 들어올 수 있는 핀의 5V tolerant 여부

핀 하나를 바꾸면 회로도 net, HSI 표, 펌웨어 심볼, 시험 포인트가 함께 바뀐다. `.ioc`·회로도·HSI 표를 같은 버전 이름으로 묶고, 리뷰에서는 세 문서를 나란히 놓고 대조한다.
...
4. CubeMX Pinout에 핀 충돌 경고가 없는데도 HSI 검토가 여전히 필요한 이유를 설명한다.
...
- [ ] CubeMX `.ioc` pinout과 HSI 표를 대조해 불일치 항목 목록 작성
