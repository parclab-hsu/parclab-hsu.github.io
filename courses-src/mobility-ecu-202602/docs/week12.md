...
!!! tip "교수자용 — 공식 12주차 · 인버터 하드웨어 설계 운영 자료"
    [🗓️ 운영 키트](week12-kit.md) · [🖨️ 배포용 워크시트](week12-worksheets.md)
...
> 📚 이번 주 용어의 자세한 설명과 그림은 **[주차별 용어 정리 — 12주차](terms-by-week.md#week12)** 에 있다.
...
- 인버터 하드웨어 설계의 중심 질문은 "전력부 부품 선정과 보호가 최종 로버 ECU에서 어떤 문제를 해결하는가"이다.
...
4. 과전류 비교기 임계값을 션트 전압 기준으로 계산한다. 결과를 다음 주차 산출물에 어떻게 반영할 것인가?
...
> 이 사례노트는 `전력부 부품 선정과 보호`를 실제 로버, 산업용 AMR, 차량 ECU 문제로 연결한다.
...
- "본 실험은 전력부 부품 선정과 보호를 로버 ECU 관점에서 검증하기 위해 수행하였다."
...
| 자료 | 수업 중 쓰는 장면 |
|---|---|
| [inverter-hardware-design-v0.1.pdf](attachments/inverter-hardware-design-v0.1.pdf) **(원본 자료)** | 인버터 하드웨어 설계의 주교재 |
| [motor-control-inverter-theory.pdf](attachments/motor-control-inverter-theory.pdf) **(원본 자료)** | 6~7주차 이론을 회로 설계로 연결 |
