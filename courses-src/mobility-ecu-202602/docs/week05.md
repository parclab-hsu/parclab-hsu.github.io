...
!!! tip "교수자용 — 5주차 운영 자료"
    [🗓️ 운영 키트](week05-kit.md) (180분 진행 대본·준비물·판서·시간 조절) · [🖨️ 배포용 워크시트](week05-worksheets.md) (인쇄해서 나눠 주는 양식)
...
> 📚 이번 주 용어의 자세한 설명과 그림은 **[주차별 용어 정리 — 5주차](terms-by-week.md#week05)** 에 있다.
...
4. 부하가 증가할 때 전류 증가와 속도 감소를 Teleplot 로그로 남긴다. 결과를 다음 주차 산출물에 어떻게 반영할 것인가?
...
> 이 사례노트는 `토크, 역기전력, 홀 위치`를 실제 로버, 산업용 AMR, 차량 ECU 문제로 연결한다.
...
미션은 직진뿐 아니라 경로 추종을 요구한다. 소형 로버의 조향은 좌우 속도차로 도는 차동 구동(skid-steer)과 앞바퀴 각도를 바꾸는 서보 조향(Ackermann)으로 나뉜다. 이 수업의 기준은 좌우 독립 구동 2모터의 차동 구동이다.
...
vR = v + ω × W/2
vL = v - ω × W/2
rpm_wheel = 60 × v_wheel / (2π × r)
```

`W=0.30m`, `r=0.1m` 기준 계산 예다.

| 동작 | 좌/우 바퀴 RPM |
|---|---|
| 직진 0.5m/s | 47.7 / 47.7 |
| 반경 1m 선회, 0.5m/s | 40.6 / 54.9 |
| 제자리 회전 1rad/s | -14.3 / 14.3 |

- 제자리 회전은 타이어가 옆으로 끌리므로 직진보다 큰 토크를 요구한다. `M ≈ μ × m × g × L / 4`(L은 축거)는 출발점일 뿐, 최종 값은 지면에서 측정해 확정한다.
- 모터 사양은 직진 연속 토크와 회전 피크 토크를 함께 만족해야 한다.
- 요구사항은 최고속도, 최소 회전반경, 제자리 회전 가능 여부, 좌우 속도 오차 허용치로 나누어 수치화한다.
...
| 자료 | 수업 중 쓰는 장면 |
|---|---|
| [motor-control-inverter-theory.pdf](attachments/motor-control-inverter-theory.pdf) **(원본 자료)** | 모터 원리, 홀센서, 속도계산의 주교재 |
...
- [ ] 전기각/기계각 변환 계산
- [ ] M/T 방식 RPM 계산 비교
- [ ] 트랙 폭·바퀴 반지름으로 직진·선회·제자리 회전의 좌우 바퀴 RPM 계산표 작성
