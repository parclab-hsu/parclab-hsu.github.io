...
!!! note "파일 앞 번호와 주차 번호가 다른 경우가 있다"
    파일 이름의 앞 번호는 **원본 강의자료의 회차**를 따르고, 주차 번호는 **교재 챕터**를 따른다. 그래서 `09_face_main`은 7주차, `15_struct_packet`은 14주차에 쓴다. 나머지 여섯 개는 번호와 주차가 같다. **위 표의 주차 열을 기준으로 삼는다.**
