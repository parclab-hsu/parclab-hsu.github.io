...
![포인터 배열 vs 배열 포인터](img/30_ptr_array_vs_array_ptr.png)

*괄호 하나로 뜻이 완전히 달라진다. `int *p[3]`은 포인터가 3개, `int (*p)[3]`은 포인터가 1개다. 헷갈리면 괄호부터 본다.*
...
![함수 포인터 = 실행할 함수를 표로 골라 부르기](img/12_func_pointer.png)

*명령이 늘어날 때 `switch`를 길게 늘이는 대신 표에 한 줄을 더한다. 15주차 ROS2 콜백이 같은 생각이다.*
