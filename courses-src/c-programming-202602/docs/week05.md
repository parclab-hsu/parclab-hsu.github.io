...
### 개념을 더 풀어 설명하기
조건문은 프로그램이 상황에 따라 다른 길을 선택하게 만드는 구조다. 앞 주차까지의 코드는 값을 계산하고 출력하는 데 집중했다면, 조건문부터는 값에 의미를 부여한다. 거리값이 10cm이면 위험, 25cm이면 주의, 50cm이면 정상처럼 같은 숫자라도 기준을 통과하면서 상태가 된다.

모빌리티 분야에서 조건문은 안전 기준과 직접 연결된다. `distance < 15`라는 조건은 단순 비교가 아니라 "15cm 안쪽이면 멈춘다"는 제어 정책이다. 따라서 조건문을 작성할 때는 비교 연산자 하나하나가 실제 동작을 바꾼다는 점을 강조해야 한다. 경계값 테스트는 이 주차의 핵심 활동이다.

### 라이브 코딩 흐름
거리값을 입력받아 `STOP`, `SLOW`, `RUN`을 출력하는 프로그램을 먼저 만든다. 이후 상태 문자열 대신 LED 표정을 바꾸는 함수 호출로 바꾼다. 마지막에는 문자 명령 `h`, `a`, `o`, `n`을 `switch`로 처리해 표정을 전환한다. 이 흐름은 "조건 판단 → 함수 호출 → 하드웨어 반응"으로 이어진다.

### 학생 활동
- 14.9, 15.0, 15.1, 29.9, 30.0을 넣고 상태가 어떻게 바뀌는지 표로 만든다.
- `if` 순서를 바꾸었을 때 결과가 왜 달라지는지 설명한다.
- `switch`에서 `break`를 지우고 다음 case까지 실행되는 현상을 확인한다.
- LED 표정에 새로운 상태 `WARNING` 또는 `REVERSE`를 추가한다.

### 자주 막히는 지점
- 범위 조건은 작은 범위부터 검사할지 큰 범위부터 검사할지 순서가 중요하다.
- `else if`는 앞 조건이 거짓일 때만 검사된다.
- `switch`는 정수형/문자형 분기에는 좋지만 복잡한 범위 비교에는 `if`가 더 자연스럽다.
- 하드웨어 실습에서는 코드가 맞아도 보드 업로드나 라이브러리 문제로 실패할 수 있다.
