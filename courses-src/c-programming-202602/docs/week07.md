...
### 개념을 더 풀어 설명하기
함수는 코드를 짧게 만드는 도구가 아니라 책임을 나누는 도구다. `main` 함수 안에 입력, 계산, 판단, 출력, 하드웨어 제어가 모두 섞이면 프로그램이 조금만 길어져도 읽기 어렵다. 함수를 만들면 "거리로 상태를 판단하는 일", "상태를 출력하는 일", "LED 표정을 바꾸는 일"처럼 역할을 나눌 수 있다.

좋은 함수는 이름만 보아도 무엇을 하는지 알 수 있다. `f1`, `abc` 같은 이름보다 `decide_state`, `show_face`, `handle_command` 같은 이름이 훨씬 낫다. 함수 이름은 코드 안에 들어 있는 작은 설명문이다. 함수의 입력은 매개변수, 결과는 반환값으로 정리하면 테스트도 쉬워진다.

### 라이브 코딩 흐름
처음에는 `main` 안에 있는 거리 판단 코드를 그대로 보여 준다. 그 다음 판단 부분을 `decide_state(double distance)` 함수로 빼고, 출력 부분을 `print_state(int state)`로 분리한다. 마지막에는 시리얼 명령 문자를 받아 `show_happy`, `show_angry` 같은 함수 호출로 연결한다.

### 학생 활동
- 길어진 `main` 코드에서 함수로 뺄 수 있는 부분을 표시한다.
- 함수 하나의 책임을 한 문장으로 적고, 그 책임에 맞는 이름을 붙인다.
- 매개변수와 반환값이 있는 함수, 반환값이 없는 `void` 함수를 각각 만든다.
- 시리얼 명령 하나를 추가하고 그 명령을 처리하는 함수를 작성한다.

### 자주 막히는 지점
- 함수 선언과 정의의 매개변수 목록이 다르면 컴파일 오류가 난다.
- 반환형이 `int`인데 `return`을 빠뜨리면 의미가 불명확해진다.
- 함수 안의 지역변수는 함수가 끝나면 사라진다.
- 함수가 전역변수를 너무 많이 직접 바꾸면 나중에 원인을 추적하기 어렵다.
