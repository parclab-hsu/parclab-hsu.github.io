...
- **0~25분**: 1차원 배열을 행과 열로 확장하는 그림을 그린다. LED Matrix, 지도, 점유격자 예시를 한 화면에 놓고 공통 구조를 찾게 한다.
- **25~80분**: 2차원 배열 선언과 중첩 반복을 실습한다. 행과 열 순서를 바꾸면 출력이 어떻게 달라지는지 직접 확인한다.
- **80~125분**: 문자열이 `char` 배열이라는 점과 `'\0'`의 의미를 설명한다. 짧은 명령 문자열을 읽고 비교하는 예제를 작성한다.
- **125~180분**: WiFi 컨트롤러 실습을 진행한다. 브라우저 요청이 문자열로 들어오고, 그 문자열이 조건/함수 호출로 이어지는 흐름을 ROS2 토픽의 전 단계로 정리한다.

## 강의 본문 보강

### 개념을 더 풀어 설명하기
2차원 배열은 행과 열을 가진 데이터 구조다. 1차원 배열이 시간 순서의 센서값을 저장한다면, 2차원 배열은 공간을 저장할 수 있다. LED Matrix의 행과 열, 주차장 격자, 로봇 지도, 점유격자는 모두 2차원 배열로 생각할 수 있다.

문자열은 C에서 별도의 마법 같은 자료형이 아니라 `char` 배열이다. 문자열의 끝에는 `'\0'`이라는 널 문자가 들어가며, C 함수들은 이 문자를 보고 문자열이 어디서 끝나는지 판단한다. 이 개념을 이해하지 못하면 문자열 버퍼 크기, 복사, 비교에서 오류가 자주 발생한다.

### 통신으로 연결하기
WiFi 컨트롤러 실습은 문자열과 통신이 만나는 지점이다. 브라우저에서 `/run`을 누르면 보드는 문자열 형태의 요청을 받고, 프로그램은 그 문자열을 해석해 상태를 바꾼다. 후반부 ROS2에서도 토픽 이름과 메시지 내용을 해석해 콜백 함수가 실행된다. 즉 11주차는 웹 버튼과 ROS2 토픽 사이의 다리 역할을 한다.

### 학생 활동
- 8행 12열 배열에서 특정 좌표를 켜고 끄는 코드를 작성한다.
- 중첩 반복문에서 바깥 반복이 행인지 열인지 바꾸어 출력 결과를 비교한다.
- `"RUN"`, `"STOP"` 문자열을 비교할 때 `==`와 `strcmp`의 차이를 확인한다.
- WiFi 명령에 `/left`, `/right`를 추가하고 상태 출력을 바꾼다.

### 자주 막히는 지점
- `map[row][col]`에서 행과 열 순서를 자주 바꿔 쓴다.
- 문자열 비교는 `==`가 아니라 `strcmp`를 사용한다.
- 문자열 배열 크기는 실제 문자 수보다 1칸 더 필요하다. 마지막 `'\0'` 때문이다.
- WiFi 실습 실패는 코드 외에도 SSID, 비밀번호, 같은 네트워크 접속 여부가 원인일 수 있다.
