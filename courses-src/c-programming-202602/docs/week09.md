...
- **0~25분**: 중간고사 오답을 스코프 관점으로 연결한다. "변수가 어디에서 살아 있는가"를 묻는 질문으로 후반부를 시작한다.
- **25~80분**: 지역변수, 전역변수, `static` 지역변수를 같은 예제로 비교한다. 함수가 끝난 뒤 값이 사라지는지 남는지 실행 결과로 확인한다.
- **80~135분**: 차량 상태 카운터와 상태머신 예제를 작성한다. 전역변수로 만든 버전과 `static`으로 숨긴 버전을 비교해 설계 차이를 느끼게 한다.
- **135~180분**: 스코프 오류 찾기 실습을 진행한다. 마지막에는 "편해서 전역을 쓰는 경우"와 "책임이 분명해서 전역이 필요한 경우"를 구분하게 한다.

## 강의 본문 보강

### 개념을 더 풀어 설명하기
스코프는 이름이 보이는 범위이고, 수명은 값이 살아 있는 시간이다. 변수는 단순히 값을 담는 그릇이지만, 그 그릇을 어디에 두느냐에 따라 프로그램의 안정성이 달라진다. 함수 안에 있는 지역변수는 밖에서 볼 수 없어 안전하지만, 함수가 끝나면 사라진다. 전역변수는 여러 함수가 공유할 수 있어 편하지만, 누가 값을 바꾸었는지 추적하기 어렵다.

`static` 지역변수는 학생들이 특히 헷갈리는 개념이다. 함수 안에서 선언되므로 밖에서는 보이지 않지만, 함수가 끝나도 값은 사라지지 않는다. 버튼 누른 횟수, 상태 전환 횟수, 필터 내부 상태처럼 "기억은 해야 하지만 외부에는 숨기고 싶은 값"에 적합하다.

### 라이브 코딩 흐름
먼저 같은 이름의 지역변수가 서로 다른 함수에 있을 때 충돌하지 않는 예제를 보여 준다. 다음에는 전역변수 `state`를 여러 함수가 바꾸는 예제를 만들고, 값이 어디에서 바뀌었는지 추적하기 어려운 상황을 보여 준다. 마지막에는 `static int count`로 함수 호출 횟수를 기억하는 카운터를 만든다.

### 학생 활동
- 코드 속 변수마다 지역, 전역, static 중 어디에 속하는지 표시한다.
- 전역변수를 쓰는 버전과 매개변수/반환값을 쓰는 버전을 비교한다.
- 함수 호출 횟수를 기억하는 `static` 예제를 작성한다.
- 로봇의 현재 상태를 어디에 저장하는 것이 좋은지 토론한다.

### 자주 막히는 지점
- 지역변수는 같은 이름이어도 함수가 다르면 다른 변수다.
- 전역변수는 편하지만 디버깅 비용이 커질 수 있다.
- `static` 지역변수는 한 번만 초기화되고 이후 값을 유지한다.
- 변수의 수명과 스코프는 같은 말이 아니다.
