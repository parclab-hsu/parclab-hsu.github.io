...
![포인터 = 다른 변수의 주소를 담는 변수](img/02_pointer_concept.png)

*`&a`로 주소를 구해 `p`에 넣고, `*p`로 그 주소가 가리키는 값을 읽는다. 3주차 `scanf_s("%d", &n)`의 `&`가 바로 왼쪽 화살표다.*
...
![포인터 연산 — p + 1 은 '한 칸' 이동이다](img/29_pointer_arith.png)

*`p + 1`을 "주소가 1 늘어난다"로 외우면 배열에서 반드시 틀린다. 가리키는 자료형의 크기만큼 움직인다.*
...
![값 전달(복사) vs 주소 전달(원본 수정)](img/21_callby.png)

*함수에 값을 넘기면 복사본이 가므로 원본이 안 바뀐다. 원본을 고치거나 결과를 여러 개 돌려받아야 하면 주소를 넘긴다.*
