...
|---|---|---|
| 프로그램은 명령의 순서다 | `Hello.c`를 만들고 문자열을 바꿔 실행한다 | 내가 바꾼 문장이 어느 줄에서 출력되는가? |
| 컴파일은 번역 과정이다 | 세미콜론, `#include`, 괄호를 하나씩 지워 오류를 본다 | 오류가 전처리, 컴파일, 링크 중 어디에 가까운가? |
| 개발환경은 역할이 다르다 | Visual Studio, Arduino IDE, GitHub 화면을 각각 캡처한다 | PC C 실습과 보드 업로드는 어떤 점이 다른가? |
| GitHub는 제출 기록이다 | `week01` 폴더에 코드와 실행 화면을 커밋한다 | 나중에 다시 봐도 실행 과정을 설명할 수 있는가? |

### 수업 중 바로 해볼 것

1. Visual Studio에서 `week01_hello.c`를 만들고 `Hello Hansung C`를 출력한다.
2. Arduino IDE에서 보드와 포트를 선택한 뒤 기본 예제를 컴파일한다.
3. GitHub 저장소에 `week01/README.md`를 만들고 오늘 설치 성공/실패 내용을 기록한다.

### 막히면 확인

- 빌드 오류는 코드 창의 빨간 줄보다 **오류 목록의 첫 번째 메시지**를 먼저 읽는다.
- Arduino 업로드가 실패하면 보드, 포트, USB 케이블, 보드 매니저 설치를 코드보다 먼저 확인한다.
- Git push가 안 되면 저장소 주소, 로그인 계정, 현재 브랜치 이름을 확인한다.
...
- `Hello.c`에서 문자열을 자기 이름이나 학과명으로 바꾸어 실행한다.
- Visual Studio와 Arduino IDE에서 같은 문장을 출력해 보고, PC 프로그램과 보드 프로그램의 차이를 적는다.
- 설치 중 막힌 화면은 캡처해 GitHub 저장소의 `week01` 폴더에 남긴다.

### 자주 막히는 지점
- C 수업인데 Visual Studio 설치에서 C++ 워크로드를 골라야 한다는 점을 헷갈린다.
- 파일 이름이 `Hello.c`가 아니라 `Hello.cpp`가 되어 C 문법 실습과 다르게 동작할 수 있다.
- Arduino 업로드 실패를 코드 문제로만 생각하지만, 실제로는 보드 선택, 포트 선택, USB 케이블 문제인 경우가 많다.
- GitHub push는 평가 제출이면서 동시에 학기말 포트폴리오가 된다는 점을 초반에 강조해야 한다.

## 1. 이론

### 1.1 프로그램과 프로그래밍
- **프로그램**은 컴퓨터가 특정 작업을 수행하도록 차례대로 작성한 명령어의 모음이다.
- **프로그래밍**은 프로그래밍 언어로 프로그램을 만드는 과정이다.
- 프로그래머는 문제를 해결하기 위해 처리 순서와 방법을 명령문으로 표현한다.
- ChatGPT 같은 인공지능 서비스도 결국 사람이 설계한 프로그램과 모델이 컴퓨터 위에서 실행되는 사례로 볼 수 있다.

![프로그래밍은 하드웨어, 소프트웨어, 프로그래머가 함께 만드는 작업이라는 원본 강의자료 그림](img/pdf-crops/week01_programming_components.png)

*원본 강의자료 크롭: 프로그래밍은 언어 문법만의 문제가 아니라 하드웨어, 소프트웨어, 프로그래머의 협업으로 이해한다.*
...
![프로그래밍 언어의 종류와 사람/컴퓨터가 이해하기 쉬운 수준을 설명한 원본 강의자료](img/pdf-crops/week01_language_levels.png)

*원본 강의자료 크롭: C는 사람이 읽을 수 있는 고급 언어 형식을 가지면서도 하드웨어와 가까운 제어를 가능하게 한다.*

![여러 프로그래밍 언어 이름이 모여 있는 원본 강의자료 워드클라우드](img/pdf-crops/week01_language_wordcloud.png)

*언어는 목적에 따라 선택한다. 이 과목에서는 모빌리티 하드웨어 제어와 로봇 연계를 위해 C를 중심 언어로 둔다.*
...
![C 언어 유래와 데니스 리치, UNIX 개발 배경을 설명한 원본 강의자료](img/pdf-crops/week01_c_origin.png)

*C는 UNIX 개발 과정에서 등장했고, 이후 운영체제와 임베디드 소프트웨어의 핵심 언어로 자리 잡았다.*
...
![C 언어 장점을 목록으로 정리한 원본 강의자료](img/pdf-crops/week01_c_features.png)

*위 장점 중 이 수업에서 특히 중요한 것은 함수 기반 모듈화, 포인터, 하드웨어 제어, 라이브러리 기반 입출력이다.*
...
자세한 설치 순서는 [환경 설정 상세 가이드](setup-guide.md)를 따른다.

![Visual Studio, Arduino IDE, Tinkercad를 함께 사용하는 수업 환경을 보여 주는 원본 강의자료](img/pdf-crops/week01_course_overview.png)

*1주차에는 PC C 개발 환경과 Arduino 실습 환경을 함께 준비한다. 이후 이 흐름이 ROS2 로봇 실습까지 확장된다.*
...
!!! tip "Visual Studio에서 꼭 확인할 것"
    C 수업이어도 설치 워크로드는 **C++를 사용한 데스크톱 개발**을 선택한다. 새 파일을 만들 때 항목은 C++ 파일을 골라도, 파일 이름은 반드시 `.c` 확장자로 저장한다.
...
### 실습 1-5 · 설치 체크리스트
[환경 설정 상세 가이드](setup-guide.md)의 1주차 제출 체크리스트를 확인하고, Visual Studio 실행 화면, Arduino 보드/포트 선택 화면, `Hello` 실행 결과를 캡처한다.

---

## 4. 과제
- 설치 스크린샷·보드 인식·`Hello` 출력 → **GitHub에 push**.
- Visual Studio에서 줄 번호 표시, 프로젝트 저장 위치, 콘솔 실행 방식(`Ctrl + F5`)을 설정한다.

## 5. 참조
- 교재 Ch01–02
- C 레퍼런스 <https://en.cppreference.com/w/c> · Arduino <https://docs.arduino.cc/> · Git <https://docs.github.com/get-started>
- 설치: [Visual Studio Community](https://visualstudio.microsoft.com/ko/vs/community/) · [Arduino IDE](https://www.arduino.cc/en/software/) · [Tinkercad](https://www.tinkercad.com/)
...
5. Visual Studio에서 C 수업을 위해 선택해야 하는 워크로드는 무엇인가?
...
??? success "정답 및 해설"
    ...
    5. **C++를 사용한 데스크톱 개발** — C 컴파일 도구가 이 워크로드에 포함되어 있다.
