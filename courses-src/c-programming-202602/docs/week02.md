...
![요구사항을 적었는가 — 세 번 고치기 vs 세 단계로 쌓기](img/25_srs_vs_no_srs.png)

*위: 요구사항이 없으면 만들고 나서야 아닌 걸 알게 되어 같은 일을 반복한다. 아래: 먼저 적어 두면 같은 작업이 세 단계로 쌓이고, 마지막에 적어 둔 방법으로 검증한다.*
...
![모듈 — 한 가지 일만 맡는 덩어리](img/26_module_boundary.png)

*세 덩어리는 각각 다른 주차에 채워진다. 지금 배우는 `printf()`는 맨 오른쪽 출력 모듈이다.*
...
![%-10s %7.1f 가 화면에서 차지하는 칸](img/27_format_to_columns.png)

*형식 지정자는 화면의 "칸"을 정한다. 칸 수를 적어 두면 다 됐는지 눈으로 셀 수 있다.*
...
![AI에게 시킬 때 — 적지 않으면 AI가 대신 정한다](img/28_ai_prompt_srs.png)

*왼쪽은 지시가 애매해 결과가 매번 다르고 채점 기준도 없다. 오른쪽은 요구사항과 제약을 적어 결과가 수렴하고 검증할 수 있다.*
