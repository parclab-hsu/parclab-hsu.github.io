...
| 8~9주 | (중간고사) 스코프, static | 호출 사이에 값이 남는 카운터 | 상태를 어디에 둘지 정하는 감각 |
...
1. **학습 목표 확인**: 오늘 끝나기 전에 설명하거나 실행할 수 있어야 하는 항목을 먼저 표시한다.
2. **이론-실습 연결표 확인**: 이론 개념, 바로 해볼 실습, 확인 질문을 한 줄씩 연결해서 읽는다.
3. **문법 먼저**: 해당 주차의 핵심 문법을 10줄 이하 예제로 이해한다.
4. **보드로 확인**: 같은 개념이 Arduino R4에서 어떻게 보이는지 확인한다.
5. **로봇으로 확장**: 그 개념이 ROS2 토픽, 센서 배열, 주행 명령으로 어떻게 바뀌는지 본다.
6. **체크포인트**: 형성평가 3문제를 스스로 답하고 설명할 수 있으면 다음 주차로 넘어간다.
...
1. 학생은 `학습 목표`에서 오늘 끝나기 전에 할 수 있어야 하는 일을 체크한다.
   강의자는 목표가 이론, C 실습, Arduino/ROS2 연결을 모두 포함하는지 먼저 짚어 준다.
2. 학생은 `이론-실습 연결표`의 첫 행부터 읽고 바로 실행할 실습을 표시한다.
   강의자는 설명 직후 같은 행의 실습으로 넘어가 학생의 손이 멈추지 않게 한다.
3. 학생은 코드 예제를 실행하기 전에 예상 출력 또는 상태 변화를 먼저 적는다.
   강의자는 실행 결과만 보지 말고 왜 그렇게 나왔는지 질문한다.
4. 학생은 `자주 막히는 지점`과 `형성평가 체크포인트`로 제출 전 점검한다.
   강의자는 막힌 지점을 다음 주 도입 퀴즈 또는 보충 설명으로 회수한다.

!!! tip "학습 메모"
    2026-07-04 후속 보강에서 모든 주차에 `이론-실습 연결표`를 추가했다. 학생은 이 표를 "오늘 읽을 이론"과 "오늘 실행할 코드"를 연결하는 출발점으로 사용하고, 강의자는 3시간 수업의 실습 전환 지점으로 사용한다.
