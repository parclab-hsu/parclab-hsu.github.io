...
| 단계 | 학생 활동 | 성공 기준 |
|---|---|---|
| C 로직 확인 | `scan_logic.c`에서 최솟값 탐색과 정지 판단을 읽는다 | ROS2 없이도 함수의 입력과 출력을 설명한다 |
| 브리지 확인 | `stella_n2_bridge.cpp`에서 C 결과가 `/cmd_vel`로 바뀌는 줄을 찾는다 | 선속도와 각속도 값이 어디서 정해지는지 표시한다 |
| 토픽 테스트 | 안전/주의/정지 3가지 `/scan` 메시지를 발행한다 | `/cmd_vel` 출력이 상황에 따라 달라진다 |
| 발표 정리 | C 함수, Arduino 표시, ROS2 토픽, 안전 절차를 한 장에 묶는다 | 기능보다 판단 근거와 재현 절차를 설명한다 |

!!! warning "실습 안전 게이트"
    실제 로봇을 움직이기 전에는 테스트 토픽으로 `/cmd_vel` 출력만 먼저 확인한다. 바퀴를 띄운 상태에서 첫 실행을 하고, 속도 제한과 수동 정지 방법을 팀원이 모두 알고 있을 때만 바닥 주행으로 넘어간다.

## 강의 본문 보강

### 개념을 더 풀어 설명하기
마지막 주차의 핵심은 ROS2라는 새 기술을 많이 외우는 것이 아니다. 한 학기 동안 배운 C 문법이 실제 로봇 소프트웨어 안에서 다시 등장한다는 사실을 확인하는 것이다. `/scan`의 `ranges[]`는 배열이고, 가장 가까운 장애물을 찾는 과정은 반복문이며, 정지와 회피 판단은 조건문이다. 판단 결과를 묶으면 구조체가 되고, 큰 배열을 함수에 넘길 때는 포인터가 쓰인다.

ROS2 토픽은 프로그램 사이의 데이터 통로다. LiDAR 노드는 `/scan`으로 거리 배열을 보내고, 주행 제어 노드는 `/cmd_vel`로 속도 명령을 보낸다. 이 수업에서는 ROS2 전체를 깊게 배우기보다, C로 작성한 판단 함수가 ROS2 메시지와 어떻게 연결되는지 보는 데 집중한다.

### 실습 흐름
먼저 PC에서 `scan_logic.c`만 따로 읽고 테스트한다. 이 파일은 ROS2를 몰라도 이해할 수 있는 순수 C 로직이어야 한다. 그 다음 `stella_n2_bridge.cpp`를 보며 C 함수의 결과가 `/cmd_vel` 메시지로 바뀌는 과정을 확인한다. 마지막으로 테스트용 `/scan` 데이터를 발행하고 `/cmd_vel` 출력이 어떻게 바뀌는지 관찰한다.

### 학생 활동
- `ranges[]` 배열에서 가장 가까운 거리와 위치를 찾는 코드를 손으로 추적한다.
- 정지거리 기준을 바꾸었을 때 주행 명령이 어떻게 달라지는지 비교한다.
- `/scan` 테스트 메시지를 3가지 상황으로 만들어 본다. 안전, 주의, 정지 상황을 각각 포함한다.
- 발표 자료에는 "어떤 C 함수가 어떤 판단을 했는가"를 반드시 넣는다.

### 안전과 평가 포인트
- 실제 로봇 연결 전에는 반드시 테스트 토픽과 시뮬레이션 흐름으로 확인한다.
- 첫 실행은 바퀴를 띄운 상태에서 진행한다.
- 속도는 작게, 정지거리는 크게 시작한다.
- 기능이 많아도 설명할 수 없는 코드는 좋은 프로젝트가 아니다. C 코드 품질, 안전성, 재현 가능한 실행 절차가 함께 평가된다.
