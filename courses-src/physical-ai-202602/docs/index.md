...
!!! info "강의자료는 이 사이트에 있습니다"
    주차별 강의자료·과제·실습 코드는 **모두 이 사이트**에 있습니다. [Notion 안내 페이지](https://app.notion.com/p/38f495ed919b8106ad14e27bb9888996)에는 교과목 정보와 수업 전 준비 사항, 주차별 바로가기 링크만 있습니다.
...
### 🖥️ 실습 환경 — 내 경우는 어디에 해당하나

Isaac Sim은 **RTX 계열 NVIDIA GPU**를 전제로 동작합니다. GPU가 없다고 수강을 못 하는 것은 아니며, 아래 세 경로 중 하나를 고르면 됩니다.

| 내 상황 | 경로 | 알아둘 점 |
| --- | --- | --- |
| **RTX GPU 노트북·PC 보유** | 본인 PC에 직접 설치 | 가장 원활. 1주차 절차를 그대로 따라가면 됩니다 |
| **GPU가 없거나 사양 미달** | **AWS 클라우드**(EC2 `g6e` 인스턴스 권장) | **유료**입니다. RDP로 원격 접속해 사용합니다 |
| **실습실 PC 사용** | 실습실 환경 이용 | 사용 가능 여부·시간은 수업 중 별도 안내 |

!!! warning "AWS를 쓰는 경우 — 요금 관리가 중요합니다"
    강화학습(4·5주차)을 클라우드에서 처음부터 돌리면 **요금이 크게 나올 수 있습니다.** 이럴 때는 이미 학습된 Policy 모델을 받아 쓰는 방법이 있습니다. 인스턴스를 **쓰지 않을 때 반드시 중지(stop)** 하세요.

### 📋 수업 전 준비 (1주차 전까지)

- [ ] 위 표에서 **내 실습 경로를 정하기** — GPU 유무는 `nvidia-smi`(Linux) 또는 작업관리자 → 성능 → GPU(Windows)로 확인
- [ ] 본인 PC 사양을 권장 사양과 비교해 정리 — **과제 1의 제출물**입니다
- [ ] Ubuntu 22.04 LTS 설치 준비 — **8GB 이상 빈 USB** 한 개 (부팅 USB 제작용)
- [ ] 선수 지식 점검: 파이썬 기초 문법, 선형대수(행렬·좌표 변환), 인공지능 기초

!!! danger "듀얼부팅으로 설치한다면 — 반드시 먼저 백업"
    Windows와 Ubuntu를 같은 PC에 쓰려면 디스크 파티션을 나눠야 합니다. 이 과정에서 **기존 데이터가 삭제될 수 있습니다.** 중요한 자료는 설치 전에 반드시 다른 저장장치에 백업하세요.

!!! note "학과 노트북을 쓰는 경우"
    Windows와 리눅스가 이미 듀얼부팅으로 깔려 있습니다. 기존 리눅스가 Ubuntu 22.04가 아니거나 환경이 꼬였다면, 리눅스만 지우고 USB로 재설치하는 절차를 [2주차 페이지 맨 위](week02.md)에 정리해 두었습니다. **파티션을 건드리므로 반드시 순서대로 따라가세요.**

설치 절차는 [1주차 페이지](week01.md)에 단계별로 있습니다. 미리 혼자 설치하다 막혀도 괜찮습니다 — 막힌 지점을 기록해 두면 그대로 과제 1의 트러블슈팅 항목이 됩니다.

### 📦 학기 중 쓰게 될 것들 (미리 알아두기)
...
3. 공식 문서 확인 — 각 주차 하단 **🔗 참고자료** / **📘 Isaac Sim 5.1.0 공식 문서**
