...
이번 주차 본문에 등장한 핵심 용어를 처음 보는 학부생도 이해할 수 있게 정리했습니다.

### System-2 / System-1

- **정의**: 로봇 지능을 **추론(생각) 계층**과 **실행(행동) 계층**으로 나눈 2단 구조입니다. System-2는 자연어 명령을 받아 "무엇을 어떤 순서로 할지" 계획을 세우는 두뇌이고, System-1은 그 계획을 받아 실제 모터·센서를 제어하는 신체입니다.
- **역할/왜 중요한가**: 느리지만 똑똑한 LLM(System-2)과 빠르고 정확한 제어기(System-1)를 분리하면, LLM이 멈추거나 느려도 로봇의 실시간 동작이 끊기지 않습니다. 인간 인지심리학의 "느린 사고(System 2) / 빠른 사고(System 1)" 개념에서 따온 이름입니다.
- **맥락·예시**: 본 강의의 System-2(`models.py`·`llm_planner.py`·`system2_node.py`)는 생성한 플랜을 `/system2/plan_cmd` 토픽으로 발행해 System-1에 전달하며, 15주차에서 둘을 연동·검증합니다.

### Pydantic (파이단틱) · BaseModel

- **정의**: 파이썬에서 데이터의 **타입·범위·구조가 규칙에 맞는지 자동으로 검사**해 주는 라이브러리입니다. `BaseModel` 을 상속한 클래스로 "데이터 규격(스키마)"을 선언합니다.
- **역할/왜 중요한가**: LLM이 만든 결과물은 형식이 틀리거나 없는 값을 만들 수 있는데, Pydantic이 **거름망** 역할을 해 규격을 벗어난 데이터를 처음부터 거부합니다. 이것이 LLM 환각을 막는 공학적 장치입니다.
- **맥락·예시**: `models.py` 의 `Step`, `HighLevelPlan`, `ReplanRules`, `System1State` 가 모두 `BaseModel` 로 정의되어 LLM과 로봇 사이의 "계약서" 역할을 합니다.

### Literal 타입 · 환각(Hallucination) 방지

- **정의**: `Literal["move_to", "scan", ...]` 은 **미리 정한 특정 문자열만 값으로 허용**하는 타입 제약입니다. 환각(Hallucination)은 LLM이 그럴듯하지만 실제로는 없는 정보·기능을 지어내는 현상을 말합니다.
- **역할/왜 중요한가**: 로봇이 실제로 수행 가능한 단위 액션은 정해져 있습니다. `Literal` 로 5종(`move_to`, `scan`, `report_and_wait`, `track`, `return_to_home`)만 인정하면, LLM이 존재하지 않는 동작을 만들어내도 스키마 검사 단계에서 즉시 에러로 걸러집니다.
- **맥락·예시**: `Step.task` 에 적용되어 "환각 차단"의 핵심 방어선이 되며, 과제의 "환각 차단 테스트"가 바로 이 동작을 검증하는 과정입니다.

### Field 제약 (`ge` / `le`)

- **정의**: Pydantic의 `Field` 로 필드에 **기본값과 값의 범위**를 지정하는 기능입니다. `ge`(greater than or equal, ≥)·`le`(less than or equal, ≤)로 허용 최소·최대값을 강제합니다.
- **역할/왜 중요한가**: 물리적으로 말이 안 되는 값(음수 재시도, 100%를 넘는 배터리 등)을 데이터 단계에서 막아 시스템의 안전성을 높입니다.
- **맥락·예시**: `retry: Field(0, ge=0)` 은 재시도 횟수를 0 이상으로, `battery_rtb: Field(0.18, ge=0.0, le=1.0)` 은 배터리 임계값을 0.0~1.0 범위로 제한합니다.

### LangChain · 실행 체인 (`prompt | llm | parser`)

- **정의**: LangChain은 LLM 애플리케이션을 **부품(프롬프트·모델·파서)을 파이프(`|`)로 연결**해 만드는 프레임워크입니다. `chain = prompt | llm | parser` 는 입력이 프롬프트 → LLM → 파서를 차례로 통과하는 처리 흐름을 뜻합니다.
- **역할/왜 중요한가**: 복잡한 "자연어 입력 → 검증된 객체 출력" 과정을 한 줄로 선언적으로 표현할 수 있어 코드가 간결하고 재사용하기 쉽습니다.
- **맥락·예시**: `llm_planner.py` 의 `build_plan()` 에서 `chain.invoke(...)` 한 번으로 모호한 운영자 명령이 검증된 `HighLevelPlan` 파이썬 객체로 변환됩니다.

### 그 외 용어 빠른 정리

| 용어 | 설명 |
| --- | --- |
| **HighLevelPlan (고수준 계획)** | System-2가 만들어내는 결과물. `intent`(의도) + `steps`(단위 액션 목록) + `replan_rules`(안전 규칙)로 구성된 1개의 미션 계획 단위. |
| **temperature (온도)** | LLM 출력의 무작위성·창의성을 조절하는 값. `0.1` 처럼 낮추면 매번 비슷하고 보수적인 답을 내어 "경로 흔들림"을 막음. |
| **PydanticOutputParser** | LLM의 문자열 출력을 JSON으로 파싱해 Pydantic 스키마로 검증·객체화하는 LangChain 유틸리티. `get_format_instructions()` 로 "이 JSON 형식으로 답하라"는 지시문도 자동 생성. |
| **프롬프트 4단계 (Action·Map·Logic·Format)** | 시스템 프롬프트에 LLM의 행동 범위(Action)·공간 추론(Map)·상황 판단(Logic)·출력 형식(Format)을 한 번에 주입하는 설계 규격. |
| **ROS 2 / rclpy** | 로봇 소프트웨어를 노드(프로그램)들로 나누고 토픽으로 메시지를 주고받게 하는 미들웨어. `rclpy` 는 그 파이썬 클라이언트 라이브러리. |
| **토픽 (Topic) · 발행/구독** | ROS 2의 비동기 통신 방식. 발행(publish)자가 보낸 메시지를 구독(subscribe)자가 받음. 예: `/ats_state`(구독), `/system2/plan_cmd`(발행). |
| **데몬 스레드 (Daemon Thread)** | 메인 작업과 별도로 도는 보조 스레드. 운영자 키보드 입력 루프를 데몬 스레드로 돌려 ROS `spin` 루프가 막히지 않게 함(블로킹 방지). |
| **Human-in-the-loop (사람 개입)** | 자율 실행 중간에 사람이 승인·지시로 끼어드는 협업 방식. `report_and_wait` 상태에서 운영자 명령을 받아 즉시 재계획(Re-planning)함. |
| **SNR (신호 대 잡음비)** | 의미 있는 정보(신호) 대비 불필요한 데이터(잡음)의 비율. `state_to_text` 가 좌표를 소수점 2자리로만 요약해 토큰 낭비를 줄이고 LLM 판단을 돕는 것이 SNR 개선의 예. |
...
| 용어 | 한 줄로 외우기 | 헷갈리는 지점 |
| --- | --- | --- |
| **System-2** | 자연어를 **검증된 계획(JSON)** 으로 바꾸는 층 | 로봇을 직접 움직이지 않는다 — 계획만 만든다 |
| **Pydantic / BaseModel** | 파이썬 클래스로 **데이터 형식을 강제**하는 도구 | 형식이 틀리면 아예 객체가 안 만들어진다 |
| **스키마 = 계약서** | LLM과 로봇 사이의 **약속된 데이터 형식** | 스키마가 있어야 LLM 출력을 믿고 실행할 수 있다 |
| **`Literal` 타입** | 허용된 **값만** 쓰도록 못 박기 | `task`를 5종으로 제한 → 없는 동작을 지어내지 못한다 |
| **환각(Hallucination)** | LLM이 **그럴듯하지만 틀린 것**을 지어냄 | 스키마·Literal·검증으로 차단한다 |
| **`Field(ge=, le=)`** | 숫자의 **하한·상한** 제약 | 각도·시간이 말이 안 되는 값으로 나오는 것을 막는다 |
| **LangChain** | 프롬프트·모델·파서를 **엮는 프레임워크** | `prompt | llm | parser` 파이프로 연결 |
| **`PydanticOutputParser`** | LLM 출력을 **스키마 객체로 파싱·검증** | 파싱 실패 = 계획 거부. 실행 전에 걸러진다 |
| **`temperature`** | 출력의 **무작위성** 정도 | 0.1처럼 낮추면 같은 명령에 같은 계획이 나온다 |
| **시스템 프롬프트 4단계** | Action(행동 제한) · Map(공간) · Logic(규칙) · Format(형식) | 네 가지를 다 넣어야 계획이 안정적으로 나온다 |
| **`state_to_text`** | 로봇 상태를 **LLM이 읽을 문장**으로 변환 | 숫자 상태를 자연어 맥락으로 바꾸는 다리 |
...
1. `Literal`로 task를 5종으로 제한하면 무엇이 막히나요?
2. `temperature=0.1`을 쓰는 이유는?
3. 스키마 검증이 "실행 전"에 있어야 하는 이유는?
4. 시스템 프롬프트의 4단계를 순서대로 쓰고 각 역할을 한 줄로 설명하세요.

??? success "정답 확인"
    1. LLM이 존재하지 않는 동작을 지어내는 환각. 허용 목록 밖의 값은 파싱 단계에서 거부된다.
    2. 출력의 무작위성을 낮춰 같은 명령에 대해 일관된 계획이 나오게 하기 위해서. 경로가 실행할 때마다 달라지는 것을 막는다.
    3. 잘못된 계획이 로봇에 전달되면 실제 물리 행동으로 이어진다. 검증은 물리 세계에 나가기 전 마지막 방어선이다.
    4. Action=쓸 수 있는 행동 제한, Map=공간·좌표 정보 제공, Logic=판단 규칙 제시, Format=출력 JSON 형식 지정.
...
```mermaid
graph LR
  A[models 스키마] --> B[프롬프트 4단계] --> C[체인 실행] --> D[환각 차단 테스트] --> E[📦 코드+Plan]
```
