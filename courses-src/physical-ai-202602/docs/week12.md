...
### System-1 / System-2
- **정의**: 로봇 지능을 두 계층으로 나눈 구조. System-2는 "무엇을 할지"를 자연어로 받아 플랜(step 시퀀스)을 만드는 **느린 추론·계획 계층**, System-1은 그 플랜을 한 줄씩 실제로 실행하는 **빠른 실행 계층**입니다.
- **역할/왜 중요한가**: 사람의 "생각(느림)"과 "반사 행동(빠름)"을 흉내 낸 분업 구조로, 계획과 실행을 분리하면 각 계층을 독립적으로 개선·재사용할 수 있습니다.
- **맥락·예시**: 본문 FLOW의 `Plan JSON → Executor → Unit Action`에서 System-2가 만든 `Plan JSON`을 System-1 Executor가 받아 실행합니다.

### System-1 Executor (실행기)
- **정의**: 플랜의 각 step을 순서대로 내려가며 실행하는 ROS 2 노드. 매 스텝에서 ① TF로 위치/자세 추적, ② vision 컨텍스트 캐시, ③ guard 판단, ④ 단위 액션 수행을 담당합니다.
- **역할/왜 중요한가**: 실제 로봇 API와 플랜 사이의 "번역·관리자" 역할로, 성공/실패를 판정해 다음 step·retry·replan을 결정합니다.
- **맥락·예시**: 본문에서 `_run_loop()`가 `task` 값에 따라 `exec_move_to()` / `exec_scan()`을 호출하는 부분이 Executor의 핵심 루프입니다.

### 단위 액션 (Unit Action)
- **정의**: 더 이상 쪼개기 어려운 **최소 행동 단위**. `move_to`, `scan`, `track`, `report`/`wait_for_command`, `return_to_home` 등이 있습니다.
- **역할/왜 중요한가**: System-2는 이 블록들을 **조합**해 플랜을 만들고, Executor는 이를 **예측 가능한 방식**으로 실행합니다. 블록 단위로 설계하면 한 액션의 내부 로직을 바꿔도 전체 플랜 실행 구조는 그대로 재사용할 수 있습니다.
- **맥락·예시**: 본문 "단위 액션의 설계 철학" 노트와, 실제 임무가 `move_to → scan` 패턴으로 이어지는 예시 참고.

### `move_to`
- **정의**: 지도(map) 상의 특정 좌표/포즈(`x, y, yaw`)로 로봇을 이동시키는 단위 액션. 내부적으로 Nav2를 사용합니다.
- **역할/왜 중요한가**: 이동 성공 시 `True`(다음 step 진행), 실패 시 `False`(retry 또는 replan)를 반환해 실행 흐름을 제어합니다.
- **맥락·예시**: 본문 `exec_move_to(node, navigator, nav_feedback, goal, replan_rules)` 함수가 실제 이동 로직이며, `goal = {x:2.5, y:1.0, yaw:0.0}` 같은 목표를 받습니다.

### `scan`
- **정의**: 로봇 위치는 그대로 두고 **ATS 카메라만 회전**시키며 주변을 탐지하는 단위 액션. **ALIGN(정면 정렬) → SWEEP(좌우 왕복) → FOUND(발견)** 3단계로 동작합니다.
- **역할/왜 중요한가**: 정해진 지점에서 목표 객체를 찾는 역할을 하며, 발견 시 `/scan_report` 토픽에 FOUND 이벤트만 발행하는 **느슨한 결합** 구조입니다.
- **맥락·예시**: 본문 `exec_scan()`이 `params`(sweep_deg, watch_classes 등)를 받아 ALIGN→SWEEP→FOUND를 한 번에 수행합니다.

### ATS (Active Tracking System)
- **정의**: 카메라를 좌우(yaw)·상하(pitch)로 회전시키는 **능동 추적용 짐벌/팬틸트 카메라 시스템**.
- **역할/왜 중요한가**: 로봇 몸체를 움직이지 않고도 카메라 방향을 바꿔 주변을 탐색하거나 대상을 추종할 수 있게 합니다.
- **맥락·예시**: `scan`의 ALIGN/SWEEP 단계에서 ATS yaw 회전 명령(`yaw_rate_cmd`)을 보내 카메라를 정렬·왕복시킵니다.

### Nav2 / `NavigateToPose`
- **정의**: Nav2(Navigation2)는 ROS 2의 표준 자율주행 스택이고, `NavigateToPose`는 "지정한 포즈까지 이동하라"는 Nav2의 **액션(Action)** 인터페이스입니다.
- **역할/왜 중요한가**: 경로 계획·장애물 회피·이동 실행을 담당하므로, `move_to`는 직접 바퀴를 제어하지 않고 Nav2에 목표만 넘깁니다.
- **맥락·예시**: 본문 `Nav2Navigator` 래퍼가 `NavigateToPose` 액션을 감싸며, 기본 기준 좌표계는 `map`입니다.

### 서비스(Service) vs 액션(Action)
- **정의**: ROS 2의 통신 방식. **서비스**는 Request→Response 한 번으로 끝나는 짧은 작업용, **액션**은 Goal→Feedback(중간 진행)→Result(최종)로 진행되며 **취소도 가능한** 장시간 작업용입니다.
- **역할/왜 중요한가**: 이동처럼 수 초~수십 초 걸리고 중간 피드백·취소가 필요한 작업은 액션이 적합합니다.
- **맥락·예시**: 본문 "여기서 잠깐!" 표 참고 — Spot이 특정 위치까지 걸어가는 작업에 액션을 사용합니다.

### TF (Transform / tf2)
- **정의**: ROS 2에서 여러 좌표계(map, body, cam 등) 사이의 **위치·방향 변환을 시간에 따라 관리**하는 시스템.
- **역할/왜 중요한가**: "지금 로봇이 map 어디에 있는가", "카메라가 몸체 기준 어느 방향을 보는가" 같은 질문에 답해 줍니다.
- **맥락·예시**: `exec_move_to`에서 1초마다 TF로 현재 pose를 조회해 진행 여부를 판단하고, `scan`의 `lookup_yaw_rel()`은 `body → cam` 변환으로 카메라 yaw를 계산합니다.

### 쿼터니언(Quaternion)과 짐벌락(Gimbal Lock)
- **정의**: 쿼터니언은 회전을 4개 값 `(x, y, z, w)`로 표현하는 방식. 짐벌락은 roll/pitch/yaw(오일러각)가 특정 각도에서 한 축의 자유도를 잃어 수학적으로 불안정해지는 현상입니다.
- **역할/왜 중요한가**: 쿼터니언은 짐벌락 없이 회전을 안정적으로 표현하므로 ROS 2는 방향을 항상 쿼터니언으로 주고받습니다.
- **맥락·예시**: 본문 `_yaw_to_quat(yaw)`가 z축 회전만 다뤄 `(0, 0, sin(yaw/2), cos(yaw/2))`로 변환합니다.

### stuck(정체) 판정
- **정의**: 로봇이 목표로 진행하지 못하고 멈춰 있는 상태를 감지하는 로직. `progress_grace_sec`(출발 직후 판정 유예), `hard_stuck_timeout_sec`(최대 무진행 허용 시간), `progress_epsilon_m`(진행으로 인정할 최소 거리 감소량) 세 파라미터로 결정합니다.
- **역할/왜 중요한가**: 영원히 못 가는 목표에 매달리지 않고, 정체를 빨리 감지해 Nav2 goal을 취소하고 retry/replan으로 넘어가게 합니다.
- **맥락·예시**: 본문에서 `(now - accept_t) > grace_sec and (now - last_prog) > hard_stuck`이면 `navigator.cancel()` 후 `False`를 반환합니다.

### PID 제어 / P 제어
- **정의**: 오차를 줄이는 피드백 제어 기법. **PID**는 비례(P)·적분(I)·미분(D) 세 항을 모두 쓰는 정밀 제어, **P 제어**는 비례 항만 쓰는 단순 제어입니다.
- **역할/왜 중요한가**: 정밀하게 멈춰야 하는 동작은 PID, 대략적이고 넓게 움직이면 되는 동작은 P로 충분합니다.
- **맥락·예시**: `scan`의 **ALIGN은 PID**(정면을 안정적으로 정렬, deadband·i_cap·d_lpf 사용), **SWEEP은 단순 P**(넓은 영역 탐색)로 설계됩니다.

### `PlanCommand` / `/system2/plan_cmd`
- **정의**: System-2가 만든 플랜을 Executor에 전달하는 메시지 타입(`ats_msgs/msg/PlanCommand`)과 그 토픽. 플랜 JSON이 `plan_json` 필드에 **문자열**로 들어갑니다.
- **역할/왜 중요한가**: System-2 없이도 이 토픽에 PlanCommand를 직접 한 번만 발행하면 단위 액션을 **단독 테스트**할 수 있습니다.
- **맥락·예시**: 본문 테스트에서 `mission_id: "test_move_001"` 플랜을 `/system2/plan_cmd`로 pub해 `move_to`를 검증합니다.

### FOUND 이벤트 / `/scan_report`
- **정의**: `scan`이 목표 객체(`watch_classes`/`watch_ids`에 매칭)를 발견했을 때 구성하는 메시지(`event:"FOUND"`, class, id, center, bbox, time)와, 그것을 JSON으로 발행하는 토픽.
...
```mermaid
graph LR
  A[move_to 구현] --> B[scan 구현] --> C[plan 테스트] --> D[성공 판정] --> E[📦 코드+로그]
```
