...
### ⚠️ 리눅스만 다시 설치할 때 — 놓치면 되돌릴 수 없는 5가지

멀티부팅 노트북에서 **리눅스만** 새로 까는 작업입니다. Windows는 그대로 두어야 하므로, 아래 다섯 가지를 지키지 않으면 **Windows까지 못 켜게 됩니다.**

| # | 주의점 | 어기면 |
| --- | --- | --- |
| 1 | **BitLocker 상태를 먼저 확인** (`manage-bde -status`) | 재부팅 시 48자리 복구 키 요구 → 키 없으면 Windows 진입 불가 |
| 2 | **EFI 시스템 파티션(100~500MB, FAT32)은 절대 삭제 금지** | 두 OS가 공유하는 부팅 영역 → Windows도 부팅 불가 |
| 3 | 설치 유형은 반드시 **`기타(Something else)`** — "디스크를 지우고 설치" 금지 | 디스크 전체가 지워져 **Windows까지 삭제** |
| 4 | 부트로더는 **파티션이 아니라 디스크 전체**(`/dev/nvme0n1`)에 설치 | `…p3` 처럼 숫자 붙은 파티션에 넣으면 부팅 실패 |
| 5 | Windows **빠른 시작 해제 후 완전 종료** | 설치 프로그램이 Windows를 인식 못 해 듀얼부팅 구성 실패 |

지울 대상은 **기존 리눅스의 `ext4` 파티션과 `linux-swap` 둘뿐**입니다. NTFS(Windows 본체)·복구 파티션·EFI는 건드리지 않습니다.

!!! danger "작업 전 반드시"
    ① 기존 리눅스의 내 파일은 **전부 삭제**됩니다 → 미리 백업
    ② Windows 쪽 자료도 백업 (파티션 사고는 디스크 전체에 영향)
    ③ BitLocker 복구 키 확보
    ④ 확신이 없으면 멈추고 조교·교수에게 문의
...
| --- | --- |
| 사용자 ID | `fm` |
| 비밀번호 | **수업 중 · LMS 공지로 별도 안내** |

!!! warning "비밀번호는 이 페이지에 적지 않습니다"
    이 사이트는 **인터넷에 공개**되어 있어 누구나 볼 수 있습니다. 공용 계정 비밀번호는 LMS 공지나 수업 중 안내로 받으세요. 재설치 후 계정을 새로 만들 때도 **같은 ID(`fm`)** 를 쓰면 이후 실습 안내와 경로가 맞습니다.
