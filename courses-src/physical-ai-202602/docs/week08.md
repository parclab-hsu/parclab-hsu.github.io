...
| 용어 | 한 줄로 외우기 | 헷갈리는 지점 |
| --- | --- | --- |
| **Isaac Sim / Isaac Lab** | 무대(시뮬레이터) / 그 위 강화학습 프레임워크 | 1주차 |
| **Stage · World · Prim** | 장면 데이터 / 시뮬 관리자 / 개체 하나 | 2주차 |
| **RGB vs Depth** | 무엇인지 본다 / 얼마나 먼지 본다 | 2주차 |
| **Annotator → Writer** | 데이터 뽑기 → 내보내기 | 2·3주차 |
| **IMU / LiDAR / Radar** | 내 자세 / 빛 거리 / 전파 거리·속도 | 3주차 |
| **MDP · Action · Observation** | 문제 정의 틀 / 정책 출력 / 정책 입력 | 4주차 |
| **Height Scan** | 발밑 지형을 레이로 관측 | 4주차 |
| **URDF Import** | 설계도(XML) → Isaac Sim USD 변환 | 5주차 |
| **kp / kv** | 끌어당기는 힘 / 흔들림 잡는 저항 | 5주차 |
| **SLAM vs Localization** | 지도 만들며 위치추정 / 있는 지도 위 위치추정 | 6·7주차 |
| **map → odom → body** | 절대 기준 → 단기 추정 → 본체 | 6·7주차 |
| **Nav2 3축** | Planner(경로) · Controller(속도) · BT(행동) | 6주차 |
...
1. (1~2주차) Prim 경로가 틀리면 무슨 일이 생기나요?
2. (3주차) 센서 데이터를 얻는 3단계 파이프라인을 쓰세요.
3. (4주차) Observation과 Action의 차원이 안 맞으면?
4. (5주차) URDF Import 시 Static 옵션을 켜면 어떻게 되나요?
5. (6~7주차) 자율주행이 안 될 때 TF부터 보는 이유는?

??? success "정답 확인"
    1. 해당 객체를 찾지 못해 조작·설정이 실패한다. Prim은 `/World/Cube` 같은 경로가 곧 식별자다.
    2. 센서 생성 → Render Product 생성 → Annotator 등록(필요 시 Writer로 내보내기).
    3. 정책 네트워크의 입출력 크기가 어긋나 학습·추론이 아예 실행되지 않는다.
    4. 로봇이 물리적으로 고정되어 중력·외력에 반응하지 않는다. 관절 구조 확인용으로는 유용하지만 보행 학습에는 부적합.
    5. `map→odom→body` 체인이 끊기면 로봇이 자기 위치를 모르므로 경로 계획·추종이 모두 무의미해진다. 대부분의 증상이 여기서 비롯된다.
