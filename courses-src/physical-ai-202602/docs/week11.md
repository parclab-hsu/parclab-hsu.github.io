...
| 용어 | 한 줄로 외우기 | 헷갈리는 지점 |
| --- | --- | --- |
| **System-2** | 자연어를 받아 **계획(Plan)** 을 만드는 느린 추론층 | "무엇을 할지"를 정한다 |
| **System-1** | 계획을 **한 줄씩 실행**하는 빠른 실행층 | "어떻게 할지"를 수행한다 |
| **단위 액션** | 더 이상 쪼갤 수 없는 **최소 행동 블록** | 1액션 = 1목적. 조합해서 임무를 만든다 |
| **Executor** | 플랜을 순서대로 내려가며 **실행·판정**하는 노드 | TF 추적·비전 캐시·guard 판단을 매 스텝 수행 |
| **Guard 조건** | "지금 이 액션을 해도 되는가" **사전 검사** | 실패를 막는 안전장치 — 통과 못 하면 실행하지 않는다 |
| **True/False 계약** | 액션은 **성공/실패만** 돌려준다는 약속 | True면 다음 step, False면 retry 또는 replan |
| **`queue_status`** | 플랜 실행 **상태값** (idle/running/done 등) | 바깥에서 진행 상황을 읽는 창 |
| **VisionCache** | 정규화된 최신 비전 상태 **저장소** | 액션들이 같은 비전 정보를 공유하게 한다 |
| **`_last_pose`** | 1초마다 갱신되는 **map→body 포즈 캐시** | `ok=False`면 자기 위치를 모른다는 뜻 — 위험 신호 |
| **Lock** | 여러 스레드가 **동시에 건드리는 것을 막는 잠금** | 상태 무결성 보장 |
| **책임 위임** | Nav2·Tracker·DepthBuffer에 **일을 맡기는 설계** | Executor는 조율만 — 내부 구현은 모른다 |
...
1. System-1과 System-2를 사람에 비유하면?
2. 단위 액션을 "더 이상 쪼갤 수 없는 최소 단위"로 만드는 이점은?
3. 액션이 True/False만 반환하도록 계약을 고정한 이유는?
4. `_last_pose["ok"]`가 False일 때 로봇을 그대로 움직이면 왜 위험한가요?

??? success "정답 확인"
    1. System-2는 곰곰이 생각해 계획을 세우는 느린 사고, System-1은 그 계획을 즉각 수행하는 빠른 반사 행동.
    2. 조합으로 새 임무를 만들 수 있고, 한 액션의 내부 로직을 바꿔도 플랜 실행 구조는 그대로 재사용된다.
    3. 실행 흐름 제어(다음 step / retry / replan)를 액션 내부 구현과 무관하게 일관되게 결정하기 위해서.
    4. 자기 위치를 모르는 상태이므로 이동·정렬 명령이 엉뚱한 방향으로 나갈 수 있다. 조기에 감지해 정지·복구해야 한다.
