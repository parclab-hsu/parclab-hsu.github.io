...
| 용어 | 한 줄로 외우기 | 헷갈리는 지점 |
| --- | --- | --- |
| **Isaac Sim** | NVIDIA의 **로봇 시뮬레이터**. 물리·렌더링이 있는 "가상 실험실" | Isaac **Lab**과 혼동 주의 — Sim은 무대, Lab은 그 무대 위 강화학습 프레임워크 |
| **Isaac Lab** | Isaac Sim 위에서 도는 **강화학습 전용 프레임워크** | Lab만 따로 설치해도 Sim이 없으면 못 돈다 |
| **Miniconda / Conda** | 파이썬 **가상환경 관리자**. 프로젝트마다 라이브러리를 따로 담는 상자 | `pip`은 패키지만, `conda`는 파이썬 버전까지 격리한다 |
| **CUDA** | NVIDIA GPU에 **연산을 시키는 도구모음** | 드라이버(하드웨어 구동)와 CUDA(연산 API)는 다른 것. 버전이 서로 맞아야 한다 |
| **nvidia-open / nvidia-driver-XXX** | GPU 드라이버 두 종류 — 오픈소스 / 폐쇄형 | Isaac Sim·CUDA 완전 지원은 폐쇄형(`nvidia-driver-XXX`) 쪽 |
| **Sim-to-Real Gap** | 시뮬에서 되던 게 **실기에서 안 되는 차이** | "시뮬이 틀렸다"가 아니라 마찰·지연·노이즈 등 모델링 안 된 요소 때문 |
...
1. Isaac Sim과 Isaac Lab의 역할을 한 문장씩으로 구분해 보세요.
2. `conda`로 환경을 나누는 이유는? `pip`만 쓰면 무엇이 문제인가요?
3. GPU 드라이버와 CUDA는 무엇이 다른가요?
4. Sim-to-Real Gap이 생기는 대표 원인 두 가지를 드세요.

??? success "정답 확인"
    1. Isaac Sim = 물리·렌더링을 갖춘 로봇 시뮬레이터(무대). Isaac Lab = 그 위에서 강화학습 환경을 구성·학습시키는 프레임워크.
    2. 프로젝트마다 요구 라이브러리·파이썬 버전이 달라 충돌하기 때문. `pip`은 패키지만 격리하고 파이썬 버전은 격리하지 못한다.
    3. 드라이버는 GPU 하드웨어를 OS가 구동하게 하는 소프트웨어, CUDA는 그 GPU에 연산을 시키는 API/툴킷. 둘의 버전 호환이 맞아야 PyTorch·Isaac Sim이 GPU를 쓴다.
    4. 마찰·질량 등 물리 파라미터의 부정확, 센서 노이즈·통신 지연 등 실기에만 있는 요소(그 밖에 지형·접촉 모델 단순화 등).
