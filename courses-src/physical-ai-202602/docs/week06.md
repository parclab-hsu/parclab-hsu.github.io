...
- **정의**: 로봇이 미지의 환경을 돌아다니며 **자신의 위치를 추정(Localization)** 하는 동시에 **주변 지도를 작성(Mapping)** 하는 기술입니다.
- **역할/왜 중요한가**: 미리 만들어진 지도가 없는 환경에서도 로봇이 "내가 어디 있고 주변이 어떻게 생겼는지"를 스스로 알아내야 자율주행이 가능합니다. 위치 추정과 지도 작성은 서로 의존하므로(지도를 알아야 위치를 알고, 위치를 알아야 지도를 그림) "동시에" 푸는 것이 핵심입니다.
- **맥락·예시**: 02강에서 SLAM을 **Front-end(제약 조건 생성)** 와 **Back-end(그래프 최적화)** 두 단계로 나눠 설명했습니다. 본 수업에서는 `slam_toolbox`로 ATS+Spot 시뮬레이션 지도를 만듭니다.

### 오도메트리 (Odometry)
- **정의**: 바퀴 회전량·관절 움직임·IMU 등 로봇 자체 센서로 **이전 위치 대비 얼마나 이동했는지**를 누적 추정한 값입니다.
- **역할/왜 중요한가**: 짧은 시간 동안의 위치 변화를 빠르고 부드럽게 추적할 수 있어 SLAM의 기초 입력이 됩니다. 다만 측정 오차가 계속 쌓여(**drift**) 시간이 길어지면 실제 위치와 어긋납니다.
- **맥락·예시**: 본문에서 `/odom` 토픽으로 제공되며, `odom` 프레임은 "시간이 지나면 오차가 누적되는 단점"이 있다고 설명됩니다. SLAM은 이 누적 오차를 `map → odom` 변환으로 보정합니다.

### 센서 융합 (Sensor Fusion)
- **정의**: LiDAR·카메라·IMU 등 **여러 센서의 정보를 합쳐** 단일 센서보다 정확하고 안정적인 추정값을 만드는 기법입니다.
- **역할/왜 중요한가**: 각 센서는 약점이 있습니다(카메라는 어두우면 약함, LiDAR는 특징 없는 복도에 약함). 서로의 약점을 보완해 위치·지도의 정확도를 높입니다.
- **맥락·예시**: 02강 "센서별 역할"에서 LiDAR는 공간 구조, 카메라는 특징점, IMU는 자세 변화를 담당하며 이를 제약 조건으로 통합한다고 설명합니다.

### TF / 좌표 프레임 (`map` · `odom` · `base_link`/`body`)
- **정의**: ROS 2에서 서로 다른 좌표계(프레임) 사이의 위치·자세 변환 관계를 시간에 따라 기록·제공하는 시스템(`tf2`)과 그 프레임들입니다.
- **역할/왜 중요한가**: 센서값은 각자 다른 좌표계에서 측정되므로, 이를 하나의 기준으로 묶어야 지도와 위치를 일관되게 계산할 수 있습니다. `map → odom → body`로 이어지는 변환 체인이 핵심입니다.
- **맥락·예시**: 04강에서 `map`(움직이지 않는 절대 기준), `odom`(단기 추정, 오차 누적), `body`(로봇 본체 기준)로 정리했고, `ros2 run tf2_tools view_frames`로 프레임 구조를 PDF로 확인합니다.

### slam_toolbox
- **정의**: ROS 2용 2D 그래프 기반 SLAM 패키지로, 온라인 맵핑·루프 클로저·지도 저장/로드를 모두 지원합니다.
- **역할/왜 중요한가**: 별도 구현 없이 런치 파일과 YAML 설정만으로 실시간 지도 작성과 위치 추정을 수행할 수 있어, 본 수업의 지도 작성 단계를 담당합니다.
- **맥락·예시**: 03~04강에서 `online_async_launch.py`(주로 사용)와 `slam_toolbox_params.yaml`로 동작을 구성합니다. 출력으로 `/map`(점유 격자 지도)과 `/tf`(`map → odom` 변환)를 내보냅니다.

### Nav2 (Navigation2)
- **정의**: ROS 2의 표준 자율주행 프레임워크로, ROS 1의 `move_base`를 대체합니다. 경로 계획·이동 제어·행동 결정을 통합한 대형 스택입니다.
- **역할/왜 중요한가**: 목표 지점만 주면 경로를 찾고, 장애물을 피하고, 속도를 다듬어 로봇을 도착시키는 "자율주행 엔진" 역할을 합니다.
- **맥락·예시**: 05강에서 사용자가 `NavigateToPose` 액션으로 목표를 주면 BT Navigator → Planner → Controller로 이어지는 흐름으로 동작합니다.

아래 용어들은 위 핵심 개념을 구성하는 세부 요소입니다.

| 용어 | 설명 |
| --- | --- |
| **Front-end / Back-end** | SLAM의 두 단계. Front-end는 센서 관측으로 **제약 조건(관측 방정식)** 을 만들고, Back-end는 이를 그래프·확률 최적화로 통합해 일관된 궤적·지도를 만든다. |
| **Scan Matching (스캔 매칭)** | 새로 들어온 LiDAR 스캔을 이전 스캔과 비교해 로봇의 이동량(포즈 변화)을 계산하고, 포즈 간 연결(제약)을 생성하는 과정. |
| **Loop Closure (루프 클로저)** | 이전에 방문했던 장소를 다시 인식해 **장거리 제약**을 추가하는 것. 누적된 drift를 한 번에 크게 보정해 지도를 닫힌 형태로 일관되게 만든다. |
| **Graph Optimization (그래프 최적화)** | 누적된 포즈·제약을 수학적으로 함께 풀어 궤적과 지도의 오차를 최소화하는 Back-end 핵심 연산. |
| **Occupancy Grid Map (점유 격자 지도)** | 공간을 일정 크기의 격자(예: `resolution: 0.05` → 5cm)로 나눠 각 칸이 비었는지/막혔는지/모르는지 확률로 표현한 지도. `/map` 토픽으로 제공된다. |
| **Costmap (비용맵)** | 공간을 위험도·접근 비용으로 표기한 지도. **Global Costmap**(`map` 기준, 전역 경로용)과 **Local Costmap**(`odom` 기준, 즉각 회피용)으로 나뉘며 `static`·`obstacle`·`inflation` 레이어로 구성. |
| **Behavior Tree / BT Navigator (행동 트리)** | "어떤 논리로 행동할까"를 트리 구조 XML로 정의. 계획 → 추종 → 장애물 회피 → 재계획 같은 상황별 행동을 조합·관리한다. |
| **Planner / Controller** | Planner는 출발→목표의 **전역 경로(Global Path)** 를 만들고(예: NavFn), Controller는 그 경로를 따라가도록 **속도 명령(`cmd_vel`)** 을 만든다(예: RegulatedPurePursuitController). |
| **Lifecycle Node (라이프사이클 노드)** | 실행 즉시 동작하지 않고 *unconfigured → inactive → active* 상태를 거치는 노드. `map_saver_server`처럼 준비 전 동작하면 안 되는 노드의 시작/정지 시점을 명확히 제어해 안정성을 높인다. |
| **Velocity Smoother (속도 스무더)** | 컨트롤러의 급격한 속도 명령(예: 0→2m/s)을 정해진 주기로 점진적으로 다듬어 급가속/급정지를 막는 노드. `smoothing_frequency`, `max_velocity`, `accel_limit` 등으로 제어. |
...
| 용어 | 한 줄로 외우기 | 헷갈리는 지점 |
| --- | --- | --- |
| **SLAM** | **내 위치 추정 + 지도 작성**을 동시에 | 둘이 서로 의존한다 — 그래서 "동시에" 푼다 |
| **Front-end** | 센서 관측으로 **제약 조건을 만드는** 앞단 | 스캔 매칭이 여기 속한다 |
| **Back-end** | 제약들을 모아 **최적화로 보정**하는 뒷단 | 그래프 최적화·루프 클로저가 여기 |
| **Odometry** | 바퀴·관절·IMU로 **이동량을 누적 추정** | 짧게는 정확, 길어지면 오차 누적(drift) |
| **Sensor Fusion** | 여러 센서를 합쳐 **약점을 상쇄** | LiDAR=구조, 카메라=특징, IMU=자세 |
| **`map` 프레임** | 움직이지 않는 **절대 기준 좌표계** | SLAM이 시작점을 원점으로 스스로 선언한다 |
| **`odom` 프레임** | **단기 추정** 좌표계. 부드럽지만 밀린다 | map→odom 변환이 이 밀림을 보정한다 |
| **`base_link`/`body`** | **로봇 본체** 기준 좌표계 | 실제로 움직이는 것은 이 프레임 |
| **slam_toolbox** | ROS 2용 **2D 그래프 SLAM 패키지** | `/map`과 `map→odom` TF를 내보낸다 |
| **Nav2** | ROS 2 **자율주행 스택** | Planner(경로) + Controller(속도) + BT Navigator(행동) |
| **Costmap** | 공간을 **위험도/비용**으로 칠한 지도 | Global은 `map` 기준, Local은 `odom` 기준 |
...
1. SLAM에서 위치추정과 지도작성을 "동시에" 푸는 이유는?
2. `map`과 `odom` 프레임의 성질을 대비해 설명하세요.
3. Nav2의 세 축과 각 역할을 짝지으세요.
4. Global Costmap과 Local Costmap의 기준 프레임과 용도 차이는?

??? success "정답 확인"
    1. 지도를 알아야 내 위치를 알 수 있고, 내 위치를 알아야 지도를 그릴 수 있어 서로 의존하기 때문.
    2. `map`은 시간이 지나도 움직이지 않는 절대 기준이지만 갱신이 튈 수 있고, `odom`은 연속적이고 부드럽지만 오차가 누적된다. SLAM이 `map→odom` 변환으로 누적 오차를 보정한다.
    3. Planner = 전역 경로 생성, Controller = 경로를 따라가는 속도 명령 생성, BT Navigator = 상황별 행동(회피·재계획) 결정.
    4. Global은 `map` 기준으로 전역 경로 계획에, Local은 `odom` 기준으로 로봇 주변 즉각 회피에 쓴다.
...
```mermaid
graph LR
  A[슬램 실행] --> B[지도 작성] --> C[지도 저장] --> D[Nav2 주행] --> E[📦 지도+영상]
```
