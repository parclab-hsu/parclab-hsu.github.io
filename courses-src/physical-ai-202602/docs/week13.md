...
이번 주차에 등장한 핵심 용어를 초보자 눈높이로 정리합니다.

### 단위 액션 (Unit Action)
- **정의**: 더 이상 쪼개기 어려운 로봇의 최소 행동 단위입니다(`move_to`, `scan`, `track`, `report_and_wait`, `return_to_home`).
- **역할/왜 중요한가**: System-2(상위 계획)는 이 블록들을 조합해 플랜을 만들고, Executor(System-1)는 이를 **예측 가능한 방식**으로 실행합니다. 행동을 표준 블록으로 쪼개야 조합·재사용·테스트가 쉬워집니다.
- **맥락·예시**: part 1은 `move_to`·`scan`, 이번 part 2는 가장 복잡한 `track` 과 시나리오 마무리용 `report_and_wait`·`return_to_home` 를 다룹니다.

### track 3축 통합 제어
- **정의**: 타깃을 화면 중앙에 두고 일정 거리를 유지하며 따라가는 추종 액션. 단순 시야 추적이 아니라 **짐벌(yaw/pitch) + 바디(yaw) + 거리(vx)** 를 동시에 다룹니다.
- **역할/왜 중요한가**: 짐벌만 돌리면 카메라는 타깃을 보지만 로봇 몸체는 엉뚱한 방향을 향해 이동·거리 제어가 불안정해집니다. 세 축을 함께 제어해야 안정적으로 따라갈 수 있습니다.
- **맥락·예시**: 짐벌은 픽셀 오차로 고주파 보정(빠른 응답), 바디 yaw는 TF로 저주파 정렬(느린 응답), 거리는 `range_m` 과 `follow_dist` 비교로 `vx` 를 결정합니다.

### 짐벌 (Gimbal, ATS yaw/pitch)
- **정의**: 카메라(센서)를 yaw(좌우)·pitch(상하)로 회전시키는 ATS 구동부. 픽셀 오차를 입력받아 각속도 명령을 냅니다.
- **역할/왜 중요한가**: 타깃을 **화면 한가운데**로 빠르게 끌어오는 고주파 응답 담당. 몸체보다 가볍고 빠르므로 미세한 오차를 즉시 보정합니다.
- **맥락·예시**: 타깃이 좌측으로 치우치면 yaw를 좌(−), 화면 아래에 있으면 pitch를 위(+)로. 명령은 `tw_ats.angular.z`/`.y` 로 ATS에 Twist 발행됩니다.

### 바디 yaw (wz)
- **정의**: 로봇 몸체(Spot)를 좌우로 회전시키는 각속도 명령(`wz`). 카메라가 보는 방향과 몸 정면을 정렬합니다.
- **역할/왜 중요한가**: "카메라가 보는 방향 = 몸 방향"이 되어야 전진·장애물 회피·거리 유지가 안정적입니다. 짐벌보다 느린 저주파 정렬을 맡습니다.
- **맥락·예시**: `lookup_yaw_rel()` 로 카메라–몸체 yaw 관계를 구하고, **이중 데드밴드**(`yaw_deadband_enter_deg` 4.0° / `yaw_deadband_exit_deg` 6.5°)로 불필요한 흔들림을 막습니다.

### DepthBuffer / range_m
- **정의**: `/depth` 이미지를 캐시하고 타깃 bbox ROI의 **대표 거리값 `range_m`**(미터)을 산출하는 모듈입니다.
- **역할/왜 중요한가**: "화면 중앙"만으로는 전진/후퇴를 결정할 수 없습니다. 실제 거리(`range_m`)가 있어야 `vx`(거리 제어)가 가능합니다.
- **맥락·예시**: depth 표현 통일(`32FC1`/`16UC1`) → vision↔depth 해상도 정렬(리스케일) → 비정상·`NaN`·범위 밖 값 제거 → 중앙값/percentile 선택의 4단계로 튐 값에 강한 거리를 뽑습니다.

### PID 제어 (P·D·I, anti-windup)
- **정의**: 오차에 비례(P), 변화율(D), 누적(I) 항을 합쳐 제어량을 만드는 고전 제어 기법.
- **역할/왜 중요한가**: 짐벌 yaw/pitch 보정의 핵심. I항(누적 오차)이 과도하게 쌓이면 오버슛이 생기는데(**windup**), 오차가 작은 구간에서만 적분을 허용해 이를 방지(**anti-windup**)합니다.
- **맥락·예시**: `if k_yaw_i > 0.0 and allow_i:` 조건으로 windup을 막고, 타깃 ID가 바뀌면 **I항을 초기화**해 이전 타깃의 누적 오차로 인한 오버슛을 막습니다.

### 그 밖의 용어

| 용어 | 설명 |
| --- | --- |
| `follow_dist` | 타깃과 유지하고 싶은 목표 거리(m). `range_m` 과 비교해 멀면 `+vx`, 가까우면 `−vx` 로 전후진 결정. |
| 데드밴드 (Deadband) | 오차가 일정 값 이하면 무시하는 구간. 센서 지터·손떨림 수준의 미세 흔들림에 반응하지 않게 함. |
| LPF (저역통과 필터) | 고주파 잡음을 걸러 부드럽게 만드는 필터. `_yaw_d_lpf`·`_dyaw_lpf` 로 D항·yaw 변화율의 갑작스러운 튐을 억제. |
| Slew 제한 | 명령의 급변을 막는 변화율 제한(`clamp`). `yaw_step`·`yaw_rate_sat` 로 짐벌 명령이 한 번에 튀지 않도록 함. |
| TF (`lookup_yaw_rel`) | `base_link↔camera` 좌표 변환(tf2). 카메라–몸체 yaw 관계 계산에 사용. 조회 실패(`ok_tf`) 시 억지 회전 금지하고 안전 정지. |
| Sticky lock (`_choose_target`) | 같은 타깃 ID를 끈질기게 유지하는 잠금. `primary_id` 우선, 없으면 bbox 면적 최대 객체 선택. ID가 바뀌면 PID I항 초기화. |
| `_control_step()` | 매 틱 실행되는 제어 루프. 비전 읽기→타깃 선택→오차 계산→짐벌→바디→거리 순으로 `vx·wz·range_m` 등을 산출·반환. |
| `_thread_main()` | yaw·거리 조건이 **연속으로** 만족되는 시간(`both_hold`)을 감독. `both_hold >= success_hold_sec` 면 Initial Success(초기 정렬 성공) 판정. |
| `report_and_wait` | 상황을 로그·`/ats_state`·`/system2/report_context` 로 보고하고 System-2 결정을 대기하는 액션. 항상 `True` 반환(시나리오 제어는 상위 로직 담당). |
| `return_to_home` | home 좌표를 `_resolve_home_goal()`(① `params.goal` ② `_home_pose` ③ 파라미터)로 결정하고 `move_to` 를 재사용해 귀환. |
...
| 용어 | 한 줄로 외우기 | 헷갈리는 지점 |
| --- | --- | --- |
| **`track`** | 대상을 **화면 중앙·일정 거리**로 유지하는 액션 | 한 축이 아니라 3축이 동시에 움직인다 |
| **3축 통합 제어** | 짐벌 yaw/pitch + 바디 yaw + 거리(vx) | 각 축이 맡는 오차가 다르다 — 역할 분담이 핵심 |
| **짐벌(ATS) 축** | 카메라를 돌려 **화면 중앙에 맞추기** | 빠르고 가볍다 — 미세 정렬 담당 |
| **바디 yaw (wz)** | 로봇 몸통을 돌려 **짐벌의 한계를 보완** | 짐벌이 끝까지 돌면 몸이 따라 돈다 |
| **거리 제어 (vx)** | 앞뒤로 움직여 **목표 거리 유지** | 너무 가까우면 후진, 멀면 전진 |
| **DepthBuffer** | bbox 영역의 깊이에서 **안정적인 거리값** 뽑기 | 한 점이 아니라 영역 통계를 쓴다 |
| **percentile / 클리닝** | 튀는 깊이값을 **버리고 대푯값** 고르기 | 유리·구멍 때문에 생기는 이상치 제거 |
| **LPF (저역통과 필터)** | 신호의 **떨림을 부드럽게** | 미분항 노이즈를 줄이는 데 쓴다 |
| **slew rate 제한** | 명령이 **한 번에 확 바뀌는 것**을 막기 | 급격한 짐벌 회전 방지 |
| **anti-windup** | 적분항이 **무한정 쌓이는 것**을 막기 | 제한을 안 걸면 복귀가 크게 튄다 |
| **hold 기반 성공 판정** | 한 순간이 아니라 **일정 시간 유지**되어야 성공 | 스쳐 지나간 정렬을 성공으로 오판하지 않게 |
| **`report_and_wait`** | 상황을 **보고하고 운영자 결정을 기다림** | 항상 True를 반환 — 대기 자체가 실패는 아니다 |
| **`return_to_home`** | 사전 정의된 **복귀 지점으로 이동** | 내부적으로 `move_to`를 재사용 |
...
1. track에서 짐벌 축과 바디 축이 하는 일을 구분하세요.
2. bbox의 한 점이 아니라 영역 통계(percentile)로 거리를 뽑는 이유는?
3. 적분항에 anti-windup을 거는 이유는?
4. 성공 판정을 "순간"이 아니라 "일정 시간 유지(hold)"로 하는 이유는?

??? success "정답 확인"
    1. 짐벌(ATS yaw/pitch)은 카메라만 돌려 대상을 화면 중앙에 빠르게 맞추고, 바디 yaw는 짐벌의 회전 한계를 넘어설 때 몸통을 돌려 범위를 넓힌다.
    2. 한 점은 구멍·반사·노이즈로 크게 튈 수 있다. 영역의 대푯값을 쓰면 이상치에 강한 안정적 거리값을 얻는다.
    3. 오차가 오래 남으면 적분항이 무한정 커져, 오차가 해소된 뒤에도 과도한 출력이 남아 크게 오버슈트하기 때문.
    4. 제어 중 우연히 스쳐 지나간 정렬을 성공으로 오판하지 않기 위해서. 안정적으로 유지될 때만 성공으로 본다.

## 📝 13주차 과제

!!! example "과제 13 — 단위 액션 구현 — track (3축 통합 제어)"
    **목표**: 타깃을 화면 중앙·일정 거리로 유지하는 `track`을 짐벌(yaw/pitch)+바디(yaw)+거리(vx) 3축으로 설계한다. DepthBuffer로 거리를 산출한다.

**과제 흐름도**

```mermaid
graph LR
  A[짐벌 정렬] --> B[바디 정렬] --> C[거리 유지] --> D[성공 판정] --> E[📦 코드+구조도]
```

**수행 단계**

1. 짐벌 PID(데드밴드·LPF·slew)로 화면 중앙 정렬
2. 바디 yaw 정렬(TF 기반, 이중 데드밴드)
3. DepthBuffer로 range_m 산출 → follow_dist 유지(vx)
4. 성공 판정(both_hold) 감독 루프 정리

**제출물**

- [ ] track 제어 로직 코드/의사코드
- [ ] 3축 제어 구조도(입력→제어→출력)
- [ ] DepthBuffer ROI 처리 단계 설명
- [ ] (가능 시) 추적 동작 영상
...
| 항목 | 배점 | 기준 |
| --- | --- | --- |
| 짐벌·바디 정렬 | 40 | yaw/pitch·바디 yaw 제어 |
| 거리 유지 | 35 | DepthBuffer·vx |
| 성공판정·문서 | 25 | both_hold·구조도 |
