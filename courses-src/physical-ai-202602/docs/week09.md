...
| 용어 | 한 줄로 외우기 | 헷갈리는 지점 |
| --- | --- | --- |
| **SLAM 모드** | 지도를 **만드는** 중 | 지도가 계속 바뀐다 |
| **Localization 모드** | **저장된 지도** 위에서 위치만 찾기 | 지도는 고정 — 바뀌지 않는다 |
| **AMCL** | 입자(파티클)를 뿌려 **가장 그럴듯한 위치**를 고르기 | 초기 위치를 안 주면 수렴하지 못한다 |
| **초기 위치(2D Pose Estimate)** | AMCL에게 주는 **출발 힌트** | `please set the initial pose` 경고가 이것을 요구하는 것 |
| **.pgm / .yaml** | 지도 **이미지** / 지도 **메타정보**(해상도·원점) | 반드시 한 쌍 — 하나만 있으면 못 쓴다 |
| **점유 격자 색 규칙** | 흰색=빈 공간, 검정=장애물, 회색=미탐색 | GIMP 보정 시 이 3색 규칙을 지켜야 한다 |
| **Loop Closure** | 예전에 온 곳을 **알아보고 오차를 접기** | 누적 drift를 한 번에 크게 보정한다 |
| **QoS (Reliability/Durability)** | 메시지 **전달 보장·보관 정책** | 지도는 Transient Local이라야 늦게 뜬 노드도 받는다 |
| **Costmap 레이어** | Static(고정 지도) / Obstacle(실시간) / Inflation(안전 여유) | Inflation이 좁으면 벽을 스친다 |
| **`max_laser_range`** | LiDAR **최대 신뢰 거리** | 실제 스펙보다 크게 잡으면 허위 벽이 생긴다 |
...
1. SLAM 모드와 Localization 모드의 결정적 차이는?
2. `please set the initial pose` 경고의 의미와 조치는?
3. 지도 토픽에 Transient Local(Durability)을 쓰는 이유는?
4. GIMP로 지도를 수정할 때 반드시 지켜야 할 규칙은?

??? success "정답 확인"
    1. SLAM은 지도를 계속 만들며 갱신하고, Localization은 저장된 지도를 고정해 두고 그 위에서 위치만 추정한다.
    2. AMCL이 초기 위치 추정값을 못 받았다는 뜻. RViz의 2D Pose Estimate로 로봇의 대략적 현재 위치·방향을 찍어 준다.
    3. 지도는 한 번만 발행되므로, 나중에 뜬 노드도 마지막 지도를 받을 수 있도록 보관해 주어야 한다.
    4. 점유 격자의 색 규칙(흰색=빈 공간, 검정=장애물, 회색=미탐색)을 지키고, 안티에일리어싱 없이 raw로 내보내야 한다.
