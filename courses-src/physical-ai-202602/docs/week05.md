...
!!! note "🔬 NVIDIA Isaac 워크샵 연계 (DLI)"
    DLI 워크샵에서는 **URDF Importer**로 UR·Jetbot·Robotiq 자산을 USD로 임포트하고, **articulation**으로 관절 트리를 구성합니다. 자산은 `visual/collision/physics`를 분리한 구조로 관리하여 재사용성과 시뮬 안정성을 높입니다 — 이번 주 ATS URDF Import와 동일한 흐름입니다.
