...
| 용어 | 한 줄로 외우기 | 헷갈리는 지점 |
| --- | --- | --- |
| **TF 트리** | 모든 좌표계를 잇는 **하나의 나무 구조** | 부모가 둘이거나 끊기면 변환이 실패한다 — 트리여야 한다 |
| **Articulation** | 관절로 이어진 **로봇 전체를 하나로** 다루는 단위 | 개별 관절이 아니라 관절 트리 통째로 제어 |
| **ArticulationView** | 여러 로봇을 **한꺼번에 읽고 쓰는** 배치 인터페이스 | 병렬 환경에서 성능을 좌우 |
| **OmniGraph** | 노드를 배선해 **동작을 구성**하는 그래프 | ROS 2 퍼블리시·구독도 여기서 배선 |
| **RenderProduct** | 센서 렌더 출력의 **핸들** | `Render product not valid` 오류는 대개 이게 안 만들어졌을 때 |
| **ROS 2 Bridge** | Isaac Sim ↔ ROS 2 **연결 다리** | 토픽 이름·QoS·시간축이 맞아야 붙는다 |
| **시간축 통일** | 시뮬 시간과 ROS 시간을 **하나로 맞추기** | `use_sim_time`이 어긋나면 TF가 "미래/과거"라 조회 실패 |
| **AMCL** | **입자 필터**로 기존 지도 위 내 위치를 찾기 | SLAM(지도 만들기)이 아니라 Localization(지도 위 찾기) |
| **48차원 관측 / 12차원 액션** | Spot 정책의 **입출력 크기** | 크기가 안 맞으면 정책이 아예 안 돈다 |
...
1. TF가 "트리"여야 하는 이유는?
2. `use_sim_time`이 어긋나면 어떤 증상이 나타나나요?
3. SLAM과 AMCL의 목적을 한 문장씩으로 구분하세요.
4. `Render product not valid` 오류를 만나면 무엇부터 확인하나요?

??? success "정답 확인"
    1. 각 좌표계는 부모가 하나뿐이어야 변환 경로가 유일하게 결정된다. 부모가 둘이거나 끊기면 어떤 경로로 변환할지 정할 수 없어 조회가 실패한다.
    2. TF 조회가 "요청 시각의 변환이 없다"며 실패한다. 시뮬 시간과 ROS 시간이 달라 스캔과 변환의 타임스탬프가 맞지 않기 때문.
    3. SLAM은 지도가 없는 환경에서 지도를 만들며 동시에 위치를 추정한다. AMCL은 이미 있는 지도 위에서 입자 필터로 현재 위치만 추정한다.
    4. 센서에 대한 RenderProduct가 정상 생성되었는지, 그리고 그 뒤에 Annotator가 올바르게 붙었는지.
