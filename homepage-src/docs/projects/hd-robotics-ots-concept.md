...
!!! abstract "설계 방향"
    **정지 측정용 적외선 스테레오 헤드 + 비대칭 6점 강체 마커 + 자동 보정 SW**를 기본 구조로 제안한다. 2~3 m 거리, 600 mm 카메라 기준선, 12 mm 렌즈를 출발점으로 삼고, 16 mm 렌즈와 확대 마커를 비교한다. 실측 센서 데이터가 없는 단계이므로 아래 결과는 설계 민감도 분석이며, OTS 또는 로봇의 달성 성능이 아니다.
...
최종 로봇 목표는 기존 계획의 **위치 0.5 mm, 자세 0.1° 이하**이다. OTS에는 별도의 초기 설계 배분안으로 **TCP 위치 오차 P95 0.15 mm 이하, 자세 오차 P95 0.03° 이하**를 제안한다. 이는 논문 실적이나 확정 검수 규격이 아니며, 아래 합성 데이터 민감도 분석과 실측 오차 예산을 바탕으로 조정할 내부 설계 기준이다.
...
1. **기본 조건의 합성 TCP 위치 P95는 약 0.124 mm**로 초기 OTS 배분안 0.15 mm 아래이지만, 교정·형상·정합 오차에 대한 여유가 충분하다고 볼 수 없다. 표본 최대값도 약 0.249 mm이므로 P95를 최대값으로 해석하면 안 된다.
