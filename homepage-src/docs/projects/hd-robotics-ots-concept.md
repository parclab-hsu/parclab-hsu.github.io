...
![학생용 OTS 아키텍처: 보기, 계산하기, 보정하기, 확인하기](../assets/ots/architecture-student.png)

[아키텍처 이미지 크게 보기·다운로드](../assets/ots/architecture-student.png)

**이 시스템은 로봇의 실제 작업점이 어디에 있는지 외부에서 측정하고, 목표 위치와의 차이를 줄이는 장치입니다.** 그림은 작동 원리를 설명하는 개념도이며, 실제 마커 형상·치수·기준 계측 장비를 확정한 제작 도면이 아닙니다.

| 순서 | 학생용 설명 | 시스템이 하는 일 |
|---|---|---|
| ① 보기 | 두 눈으로 물체를 보는 것처럼 카메라 두 대로 같은 표식을 봅니다. | 로봇을 멈추고 마커를 동시에 촬영합니다. |
| ② 계산하기 | 두 영상에서 표식이 보이는 위치의 차이로 거리를 구합니다. | 각 마커 점의 3차원 좌표와 강체 자세를 복원한 뒤 TCP 위치로 변환합니다. |
| ③ 보정하기 | 로봇이 가려던 위치와 실제 위치의 차이를 여러 자세에서 모읍니다. | 관절값과 측정값을 기존 알고리즘에 넣어 보정값을 구하고 적용합니다. |
| ④ 확인하기 | 보정 후 실제로 더 정확해졌는지 다시 확인합니다. | 새 검증 자세로 움직인 뒤 독립된 기준 계측기로 평가합니다. |

### 꼭 알아둘 용어

- **마커:** 카메라가 찾기 쉬운 위치 표식입니다. 로봇에 단단히 붙여야 표식의 움직임으로 로봇의 움직임을 알 수 있습니다.
- **TCP:** 공구 끝처럼 실제로 작업하는 점입니다. 마커 중심과 다르므로 두 위치 사이의 관계를 교정해야 합니다.
- **위치와 자세:** 위치는 “어디에 있는가”, 자세는 “어느 방향을 향하는가”입니다.
