...
| **HILS 실기 — 경로 지연·스탬프 (2026-09-16)** | 명령 왕복 p50 **14.0~16.4 ms**(p95 20.9), 피드백 중계 p50 13.2~14.5 ms, `/rover/joint_states` 50.01 Hz. 보드 스탬프는 µs 해상도(간격 0.019918 s), `/odom` 스탬프−수신 3 ms. **「스탬프가 도착보다 덜 흔들린다」는 판정 보류** — 방향은 4회 일치하나 차가 0.02~0.32 ms 로 불안정 |
