...
!!! success "현재 상태 (2026-08-22) — 3계층 폐루프가 실기에서 성립"
    ...
    **아직 확인되지 않은 것** — 월면토 마찰을 명시 바인딩했으나 **슬립이 마찰로 설명되지
    ...
    상세는 [SRS](l-project-srs.md) v1.32 · [SDP](l-project-sdp.md) v2.08 개정 이력을 참조하십시오.
...
**게걸음(crab) 주행** — 네 바퀴가 **모두 같은 각으로** 조향해 차체 방향을 유지한 채 사선으로
이동하는 기동입니다. 4WS 로버만 할 수 있고 비홀로노믹 자율 주행 스택은 명령하지 않으므로,
사실상 수동 주행에서만 쓰이는 능력입니다. 조향 운용 정책이 코너별 비대칭(−30~+60°)이라 네 축이
같이 낼 수 있는 범위는 **±30°** 이고, 90° 조향이 필요한 **순수 옆걸음은 하지 않습니다**.
...
| 조이스틱 수동 주행 (합성 입력) | **5 PASS / 0 FAIL** (2026-09-16 재확인) — 자율 주행 선점, 게걸음 시 네 축 조향 **30.0°**(조향 정책의 공통 구간 `crab_limit` 을 노드에서 읽어 대조), 데드맨 해제 시 정지 유지, 반납 후 자율 복귀 |
...
| Isaac Sim 연동 계약 리허설 | **27 PASS / 0 FAIL** (2026-09-16 재실행) — 정식 USD에 12축 조인트명 전부 실재, Action Graph 배선, 명령·피드백 양방향 변환, 역순·잡조인트 피드백에서 12조인트 복원, 조인트 누락 내성, split 모드 명령 분할, 구동축 토크 경로, 폐루프 `/odom` 0.500·0.200. 조향 운용 정책 반영 — **게걸음 네 축 +30.0° 동일**이고, **순수 옆걸음(90°)은 정책 밖이라 네 축이 −60/+60/+60/−60 로 갈리는 것**까지 검사로 확인합니다. **Isaac 자리는 대역 노드이며 물리·접촉·슬립·GPU는 범위 밖** |
