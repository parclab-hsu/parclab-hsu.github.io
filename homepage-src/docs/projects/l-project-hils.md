...
| Isaac Sim 연동 계약 리허설 | **15 PASS / 0 FAIL** (2026-08-04 기준) — 정식 USD에 12축 조인트명 전부 실재, 명령·피드백 양방향 변환, 역순·잡조인트 피드백에서 12조인트 복원(60.0 Hz), 폐루프 `/odom` 0.500·0.200. **Isaac 자리는 대역 노드이며 물리·접촉·슬립·GPU는 범위 밖**. 게걸음 항목은 조향 운용 정책 도입(2026-09-16)으로 기대값이 90° → 30° 로 바뀌어 **재실행 대기** — 이 시험은 라이브 브리지가 떠 있으면 오염을 피해 스스로 중단합니다 |
