...
현재 정책 범위 밖입니다(SRS FR-006·FR-081). 아래 과거 시험표의 90° 결과는 정책 도입 전 기록입니다.
...
| 조이스틱 수동 주행 (합성 입력) | **5 PASS / 0 FAIL** — 자율 주행 선점, crab steering 시 네 축 조향 90.0°, 데드맨 해제 시 정지 유지, 반납 후 자율 복귀 |
...
| Isaac Sim 연동 계약 리허설 | **15 PASS / 0 FAIL** — 정식 USD에 12축 조인트명 전부 실재, 명령·피드백 양방향 변환, 역순·잡조인트 피드백에서 12조인트 복원(60.0 Hz), 폐루프 `/odom` 0.500·0.200, crab steering 네 축 +90.0°. **Isaac 자리는 대역 노드이며 물리·접촉·슬립·GPU는 범위 밖** |
