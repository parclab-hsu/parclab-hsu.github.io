...
!!! success "현재 상태 (2026-08-22) — 3계층 폐루프가 실기에서 성립"
    ...
    **아직 확인되지 않은 것** — 월면토 마찰을 명시 바인딩했으나 **슬립이 마찰로 설명되지
    ...
    상세는 [SRS](l-project-srs.md) v1.34 · [SDP](l-project-sdp.md) v2.14 개정 이력을 참조하십시오.

    **2026-09-16 갱신** — 조향에 **운용 정책 한계**(코너별 거울 −30~+60°)를 두고, 선회 포락선에
    조향 조건을 더했습니다. 보드 스탬프는 µs 해상도로 올렸습니다. 실기에서 축 클램프·제자리 회전·
    포락선 절삭·사선 이동을 확인했습니다. 대가는 **연속 선회 최소 선회반경 1.81 m** 와 **사선 ±30°**
    이며, 제자리 회전은 그대로 됩니다.
...
flowchart TB
    ...
    ARB -- "/rover/cmd_vel (중재된 v, ω) — 의도만" --> AG[micro_ros_agent]

    subgraph B["HILS 보드 — STM32H7 (FreeRTOS + micro-ROS) · 경성 실시간"]
        MCU[rover_mcu<br/>µs 해상도 샘플 스탬프] --> KIN[4WS 기구학<br/>v, ω → 12축<br/>선회 포락선<br/>휠 속도·횡가속·조향 정책]
...
- **12축 계약(v3)**: 구동 4축(속도, PVM) + 조향 4축(PPM — 조인트 한계 ±90°, **운용 정책은 코너별 −30~+60°**) + 서스펜션 4축(±0.35 rad = ±20.05°, PPM)
- **4WS 기구학**: 스워브 정/역기구학(강체 최소자승) — **제자리 회전**(조향 ∓55.30°)과 사선(crab) 주행 지원. 조향 운용 정책 때문에 연속 선회에는 **최소 선회반경 1.81 m** 가 있고, 사선 이동은 네 축 공통 구간인 **±30°** 까지입니다
...
flowchart TB
    ...
    TELE -- "/cmd_vel_joy" --> RC
    NAV -- "/cmd_vel" --> RC
    TELE -- "/rover/suspension_cmd" --> MCU

    RC -- "/rover/cmd_vel (중재된 v, ω)" --> MCU
...
자율 주행(Nav2)과 별개로 **조이스틱 수동 주행**을 지원합니다.
**왼쪽 스틱은 방향, 오른쪽 스틱은 throttle**을 조작하며, X 버튼으로 동위상·역위상 조향을
전환합니다. 버튼으로 서스펜션 자세 preset(level/lift/drop)을 선택합니다(SRS FR-079·080).

**Crab steering**은 네 바퀴를 같은 각도로 조향해 차체 자세를 유지하며 이동하는 방식입니다.
현재 운용 정책에서는 공통 조향각이 **±30°**로 제한됩니다. 90° 조향이 필요한 순수 횡이동은
현재 정책 범위 밖입니다(SRS FR-006·FR-081).

| 항목 | 설계 |
|---|---|
| **명령 중재** | 별도 mux 노드 없이 `rover_control` 한 곳에서 처리 — 명령 중재는 host가, real 모드 기구학은 보드가 담당합니다. 조이스틱 명령이 1초 내에 들어오면 자율 주행 명령을 버립니다. |
...
| **두절 대응** | 노드 중단에 따른 명령 timeout은 기존 watchdog(0.5초)이 처리합니다. 무선 패드 링크 단절은 autorepeat로 감춰질 수 있어 별도 payload 정체 감지(FR-076)를 사용합니다. |
...
(`tools/test_teleop.py` — 5항목 전부 통과). 다만 이 시험의 범위는 **노드 로직(중재·데드맨·crab steering·
...
아래 결과는 저장소에 기록된 **2026-09-16 기준** 시험 실적입니다. 소프트웨어 시험 결과와
실기 검증 상태를 구분해 표시합니다.
...
| 조이스틱 수동 주행 (합성 입력) | **5 PASS / 0 FAIL** (2026-09-16 재확인) — 자율 주행 선점, crab steering 시 네 축 조향 **30.0°**(조향 정책의 공통 구간 `crab_limit` 을 노드에서 읽어 대조), 데드맨 해제 시 정지 유지, 반납 후 자율 복귀 |
...
| Isaac Sim 연동 계약 리허설 | **27 PASS / 0 FAIL** (2026-09-16 재실행) — 정식 USD에 12축 조인트명 전부 실재, Action Graph 배선, 명령·피드백 양방향 변환, 역순·잡조인트 피드백에서 12조인트 복원, 조인트 누락 내성, split 모드 명령 분할, 구동축 토크 경로, 폐루프 `/odom` 0.500·0.200. 조향 운용 정책 반영 — **crab steering 네 축 +30.0° 동일**이고, **순수 옆걸음(90°)은 정책 밖이라 네 축이 −60/+60/+60/−60 로 갈리는 것**까지 검사로 확인합니다. **Isaac 자리는 대역 노드이며 물리·접촉·슬립·GPU는 범위 밖** |
| 통합 회귀 시험 (`run_all_tests.sh`) | 위 시험 **7종 일괄 통과** (약 110초) — 인터프리터 자동 선택·잔존 노드 정리 포함 |
| 펌웨어 (FreeRTOS + micro-ROS + 12축 CANopen 마스터) | 2026-09-16 Arm GNU 13.3.Rel1 빌드 통과 — FLASH 469,980 B / 894 KB(51%), 이미지 472,028 B. **보드 플래시·실기 동작 확인 완료** |
| **HILS 실기 — 조향 운용 정책 (2026-09-16)** | **확인** — 축 클램프(네 축 45° 지령 → FL·RR 29.97°, FR·RL 44.98°), 제자리 회전(조향 ∓55.30° · 구동 ±2.42 rad/s, 잘림 없음), 사선 이동 30°(네 축 동일), 주행 2.37 m 후 정지·자세 복원 |
| **HILS 실기 — 선회 포락선 (2026-09-16)** | **확인** — v 0.3 · wz 0.8 지령에서 조향 ±30.0°, 구동 1.06/2.00 rad/s. 요레이트가 그대로였다면 2.84/4.36 이어야 하므로 보드가 wz 를 약 0.17(= v/1.81, 최소 선회반경)로 낮춘 것이 확인됩니다 |
| **HILS 실기 — 경로 지연·스탬프 (2026-09-16)** | 명령 왕복 p50 **14.0~16.4 ms**(p95 20.9), 피드백 중계 p50 13.2~14.5 ms, `/rover/joint_states` 50.01 Hz. 보드 스탬프는 µs 해상도(간격 0.019918 s), `/odom` 스탬프−수신 3 ms. **「스탬프가 도착보다 덜 흔들린다」는 지표에서 보류합니다** — µs 해상도로 올리고 카메라 4종 **215.7 MB/s** 부하까지 걸어도 차이가 0.02 ms 이하입니다. 그 부하가 **보드 경로에 닿지 않기** 때문입니다(같은 구간에서 `/sim/joint_states` 도착 흔들림은 10배 악화되는데 보드 경로는 불변 — 보드 피드백은 XRCE/UDP 로 에이전트를 거칩니다). 판정하려면 보드 경로에 직접 부하를 주어야 합니다 |
