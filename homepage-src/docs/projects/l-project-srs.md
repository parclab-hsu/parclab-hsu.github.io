...
- 시뮬 PC 플랜트(Isaac Sim + OmniLRS)의 달 지형 변형·바퀴 자국 — 렌더 전용, 물리 반영 제외(3.9절)
...
- 저장소 루트 `DEFORMATION.md`(지형 변형 엔진 분석·검증, §5a~§5f) · `DEFORMATION-HIRES-PLAN.md`(고해상도 자국 계획·P0 측정) — 3.9절 근거
- `third_party/omnilrs/README.md` — OmniLRS 고정 커밋·패치 0001~0009 목록과 사유
- Kamohara et al., "Modelling of Terrain Deformation by a Grouser Wheel for Lunar Rover Simulation", ISTVS 2024 (arXiv 2408.13468) — OmniLRS 변형 엔진 원 논문
- Lunar Sourcebook (Heiken, Vaniman, French, 1991) — 월면토 Bekker 기준값(Soil B)
...
| CSCI-12 omnilrs 플랜트 패치 | `third_party/omnilrs/`(고정 커밋·패치·오버레이), `tools/isaac/` | Python / Isaac Sim 5.0 + OmniLRS | 시뮬 PC 달 지형 플랜트 — 상류 결함 수정, 하중 입력, 렌더 전용 변형, 고해상도 바퀴 자국(패치 0001~0009) |
...
| ID | 요구사항 | V | 상태 |
|---|---|---|---|
| SRS-FR-001 | `/cmd_vel`(geometry_msgs/Twist)을 구독하여 4WS(swerve) 역기구학으로 12축 명령(구동 rad/s ×4 + 조향 rad ×4 + 서스펜션 rad ×4)을 산출한다. 조향각은 **코너별 운용 정책 한계**(FR-006) 안에서 정한다 — 같은 방향을 내는 두 각(δ, δ∓π) 중 범위에 드는 쪽을 쓰고 δ∓π 를 쓰면 휠 속도 부호를 반전한다. 둘 다 밖이면 가까운 경계로 자른다(그 전에 선회 포락선 FR-081 이 요레이트를 낮춰 막는다). **linear.y(crab)를 지원**한다, **linear.y(crab)를 지원**한다(자율 주행 소스는 0, 수동 주행에서만 사용). **기구학 소유자는 하나다** — HILS 보드가 붙는 real 모드에서는 **보드가 기구학을 돈다**: `rover_control` 은 조이스틱·자율 주행 중재가 끝난 twist(v, ω, v_y)만 `/rover/cmd_vel` 로 내고 **축 지령은 한 건도 내지 않으며**, 보드가 같은 기구학(`kinematics.c`, 파이썬 정본과 대조)으로 12축을 만든다. sim 모드(보드 없이 시뮬레이터 직결)는 `rover_control` 이 12축을 낸다. launch 인자 `kinematics_owner`(기본 `auto`: real→board · sim→host, 명시값 우선) | T | 검증완료 (host 경로) · **board 경로는 상위 계약만 검증**(`tools/test_kinematics_owner.py` 10 PASS, 보드 기구학은 호스트 대조 `test_firmware_kinematics.py`) — 보드 실기 적용은 펌웨어 재플래시·Jetson 갱신 후 |
...
| SRS-FR-073 | 데드맨 해제 시 우선권을 유지하여 정지 상태를 지키고, 자율 주행 복귀는 release 버튼(Start, `button_release`=7)으로 명시적으로 반납할 때만 이루어진다 (손을 떼자마자 자율 주행이 재개되는 것을 방지). 반납 시 `rover_teleop` 은 `/cmd_vel_joy` **발행 자체를 멈추고**, `rover_control` 은 FR-072 타임아웃 만료 후 `/cmd_vel` 을 다시 받는다 | T | **검증완료 (2026-08-22 실기 실측)** — 반납 전 `/cmd_vel` 무시(로그 `cmd_vel ignored - joystick has priority`), Start 발행 후 `/cmd_vel_joy` 중단·`/cmd_vel` 0.5 m/s → `/rover/axes_cmd` 2.347. **2026-09-11 격리 재확인** — 도메인 분리 후 `test_teleop.py` 5 PASS / 0 FAIL |
...
### 3.9 시뮬레이션 플랜트 — 지형 변형·바퀴 자국 (CSCI-12)

대상은 시뮬 PC 의 Isaac Sim + OmniLRS 플랜트다. 실물 로버와 무관한 **시뮬레이션 요구사항**이며, 이 절의
'검증완료'는 **Isaac 실측 기준**이다(실물 월면토 시험 자료가 아니다). 근거·수치는 저장소 루트
`DEFORMATION.md` §5a~§5f 와 `DEFORMATION-HIRES-PLAN.md`, 원자료는 `results/`.

| ID | 요구사항 | V | 상태 |
|---|---|---|---|
| SRS-FR-083 | 플랜트는 휠이 지나간 지형에 **하중에 따른 자국**을 남긴다. 하중은 PhysX 접지력이 아니라 **서스펜션 조인트 토크**에서 얻는다(정지·안착 시 접지력이 0 으로 읽혀 깊이가 하중이 아니라 충격을 따르기 때문 — D-39). 기준하중(자중 ÷ 4 = 182.2 N)에서 침하 약 20 mm, 그 밖에서는 **하중^(2/3)**(강성륜 Bekker, n = 1) | T | **검증완료 (Isaac 실측)** — 기준하중 0.0201 m(엔진 오프라인), 364 N 에서 1.587 배, 주행 중 평균 깊이 24~27 mm (패치 0004·0008·0009) |
| SRS-FR-084 | 지형 변형은 **물리 스텝을 멈추게 해서는 안 된다.** 변형은 렌더 메시에만 쓰고 충돌 메시는 건드리지 않는다 — 충돌 메시에 쓰면 PhysX 재쿠킹 동안 물리 스텝이 건너뛰어진다(D-47). **그 결과 자국은 보이기만 하고 휠이 밟지 않는다** — 자국 위 재주행·슬립–침하 연성은 물리에 반영되지 않는다 | T | **검증완료 (Isaac 실측)** — 주행 5 s 창 스탬프 기준 멈춤 **0/9**(수정 전 8/9, 패치 0006). 이후 모든 실측에서 멈춤 없음 |
| SRS-FR-085 | 휠 근처 자국은 **1.5 cm 이하** 해상도로 그리고, **휠 메시에서 잰 단면**(그라우저·림·가장자리 플랜지)을 휠 **회전각**에 맞춰 찍는다 — 그라우저 자국 간격은 슬립 s 에 따라 `2πR/N·(1−s)` 가 된다. 지나온 자국은 주행이 끝날 때까지 남는다 | T | **검증완료 (Isaac 실측·오프라인)** — 표본 0.0149 m, 그라우저 주기 55.8~56.4 mm(휠 기준 55.9), 오프라인 슬립 0.1·0.3 에서 이론 대비 0.4 % 이내, 타일 누락·이음매 오차 0 (패치 0009) |
| SRS-FR-086 | 플랜트 지형 소스는 **OmniLRS 고정 커밋 + 저장소 패치·오버레이**로 재현할 수 있어야 하며, 설치본이 그와 같은지 검사할 수 있어야 한다 | I | 검증완료 — `third_party/omnilrs/setup-omnilrs.sh --check` (패치 0001~0009 · 오버레이 일치) |

> **이 절이 요구하지 않는 것.** 자국이 충돌에 반영되는 것(휠이 자국을 밟는 것), 월면토 계수의 시험
> 근거, 둔덕·벽 경사의 정량 근거는 요구하지 않는다 — 셋 다 현재 근거가 없거나(둔덕 비율·임계각은 문헌
> 추정) 차년도 과제(차륜–월면토 상호작용 모델) 범위다. **이 플랜트의 주행·슬립 수치를 설계 입력으로 쓰지
> 않는다**는 6장 원칙은 그대로다.
...
| SRS-NF-007 | 지형 변형(바퀴 자국)을 켠 상태에서 **주행 중 플랜트 RTF 는 끈 상태의 90 % 이상**이어야 한다(시뮬 PC 기준). RTF 는 **헤더 스탬프로 5 s 창마다** 잰다 — 수신 주기로 재면 스탬프가 멈춘 채 토픽이 나오는 상태를 정상으로 본다 | T | **검증완료 (Isaac 실측)** — 끈 상태 0.497 대비 켠 상태 0.480~0.500(**97~100 %**). 종전 0.05 m 지형 전체 재기록 방식은 0.21~0.24(44 %) |
| SRS-NF-008 | 바퀴 자국 형상은 문헌 범위에 든다 — 평균 깊이 15~30 mm(Bekker 21.6 mm · Lunokhod 21~24 mm), 폭 = 휠 폭 ± 표본 1 칸, 그라우저 주기 = 휠 기준 × (1 − s) ± 3 % | A | **검증완료 (Isaac 실측)** — 깊이 24~27 mm, 폭 0.164 m(휠 0.159), 주기 55.8~56.4 mm. 그라우저 자국 진폭 9~10 mm 는 단륜 시험(6.09 mm)보다 크다 — 요구사항에는 넣지 않았다 |
...
- 플랜트의 **물리 특성**(링크 질량·관성·중력·구동 드라이브)은 위 기구 제원과 별개로
  ...
  달 지형(OmniLRS lunaryard, IG-001 2f)과 월면토 마찰(D-24 ②)은 반영했으나, **바퀴 자국은 렌더
  전용이고(SRS-FR-084) 토양 계수에 시험 근거가 없으므로** 이 플랜트의 주행·슬립 수치를 Nav2
  파라미터 근거로 쓰지 않는다 *(2026-09-14 정정 — 종전 「달 지형·마찰 미반영」은 D-24 ② 조치 뒤로 낡은 서술이었다)*
- **플랜트 지형 소스는 OmniLRS 고정 커밋 + 저장소 패치**다(`third_party/omnilrs/PINNED`, 패치 0001~0009,
  오버레이 `lunaryard_lproject.yaml`). 상류를 그대로 받아 쓰면 변형 엔진 결함(`y_deform_offset`·지연 단위)과
  물리 멈춤(D-47)이 되살아난다. 설치본은 `setup-omnilrs.sh --check` 로 대조하며(SRS-FR-086), 버전을 올릴 때는
  IG-001 확인 항목을 다시 수행한다
