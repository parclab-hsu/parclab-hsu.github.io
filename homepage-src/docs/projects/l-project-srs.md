...
| SRS-FR-076 | **패드 링크 두절**(절전·무선 단절·케이블 분리)을 감지해 구동축을 정지시킨다 | T | **미구현 — 안전 공백** |
...
> 링크 생존을 명령 흐름과 **별개 신호로** 판정해야 한다(예: `/joy` 헤더 타임스탬프 정체 감시,
> 또는 joy 장치 파일 존재 확인). 설계는 미착수다.
>
> 현재는 운용 규칙으로만 막고 있다 — OP-002 2.4절(구동계 연결 시 유선 사용, 비상 정지 상비,
> **토픽이 흐르는 것을 조종이 살아 있다는 근거로 쓰지 않기**). 실물·에뮬레이터를 붙이기 전에
> FR-076 을 닫아야 한다.
...
- 로버 제원: 휠 반경 0.213 m·트랙 1.26 m·축거 1.50 m (VIPER v4 USD 실측 — AN-001).
