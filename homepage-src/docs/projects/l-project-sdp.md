...
| B3 | 조향/서스 기어비·카운트 상수 확정 | 실물 미구매로 실측 불가 — 기구 설계 제원 확정 시 상수 갱신 **4곳**(drive.h·axes.py·`hils_rover_control/config/rover_params.yaml`·emu_core.py), 에뮬레이터는 동일 상수로 정합 유지. yaml을 빠뜨리면 코드만 바뀌고 실행 노드는 옛 값으로 돈다(D-17) |
