...
| 1.56 | 2026-08-25 | **변형 엔진 성능 실측 — 주행 시 30 → 26.5 Hz(−12 %).** "켜면 굉장히 느리다" 는 관찰을 네 조건(변형 ON/OFF × 정지/주행)으로 측정했다. 스텝당 **+1.8 ms(정지) ~ +4.4 ms(주행)**. 원인은 변형 계산이 아니라 **갱신 방식**이다 — 실제로 바뀌는 격자는 40칸인데 매 스텝 **256만 정점 전체를 USD 에 다시 쓴다**(29.3 MB × 30 Hz ≈ 880 MB/s). 콜라이더 재생성은 아니다. **가장 중요한 확인: 현행 설정에서는 이 비용을 전부 치르고 얻는 것이 없다** — 변형이 0 인데도(§3) `deform()` 이 깊이와 무관하게 항상 정점 전체를 다시 쓰기 때문이다. 산출물 `results/deformation-perf-20260825/`(원자료 JSON·그림 생성 스크립트·측정 조건), 그림 `docs/images/deformation/deformation-perf.png`, 도구 `tools/isaac/measure_sim_rate.py`. **저장소 설정은 `enable: False` 그대로**이며 ON 측정은 임시 환경 설정으로 했다 |
| 1.57 | 2026-08-25 | **D-37 — 명령원이 사라져도 Isaac 은 마지막 속도 목표대로 계속 달린다.** 변형 엔진 측정을 끝내고 절차대로 보드를 EPOS 로 내리자 `/sim/axes_cmd` 가 멎었는데, ArticulationController 는 직전 지령 2.0 rad/s 를 유지해 **로버가 무인으로 계속 달려 크레이터에 빠졌다**(실측 슬립 100 %). SRS FR-076 의 sim 쪽 짝이 통째로 비어 있었다. `sim_isaac_bridge.py` 에 **명령 워치독**(기본 0.5 s)을 신설했고 크레이터 안에서 실제로 휠이 서는 것까지 확인했다. **슬립 조사 도구 신설** — `wheel_feedback` 노드가 휠별 omega·torque·v_wheel·v_ref·slip 을 `/rover/wheel_feedback` 로 낸다(지상 진실이 없으면 **NaN**, 0 으로 채우지 않는다). 자체시험 22 PASS. **자산 센서 확장** — 후방 카메라 신설(전방 스펙 복사, 요 180°)과 IMU 를 지붕(0.579 m)에서 **무게중심(0.3153 m)**으로 재배치, `make_omnilrs_robot_usd.py` 가 생성 시 배치하고 **10항목을 되열어 검산**한다. 생성기의 Isaac 의존을 오도메트리 경로로 한정해 **시뮬레이터가 떠 있어도 센서 배치를 돌릴 수 있게** 했다. **GUI 탭 2종 신설** — Wheels(속도·토크·슬립, v_wheel/v_ref 를 겹쳐 그린다)와 Rover State(인공수평의·방위·IMU·모드). 회귀 기구학 74 · 휠 피드백 22 · URDF 계약 · 에뮬레이터 20 전부 통과 |
| 1.58 | 2026-08-25 | **강화학습 저장소를 분리 신설** — [`parclab-hsu/l-project-rl`](https://github.com/parclab-hsu/l-project-rl)(비공개). 자세 제어와 크레이터 탈출 모션을 다룬다. 학습은 실험 사이클이 빠르고 산출물 성격이 달라 본체와 같은 저장소에 두면 본체 이력이 실험 커밋에 묻힌다. **12축 계약은 본체 `axes.py` 가 정본**이고 학습 저장소에 사본을 두지 않는다 — 사본 관리는 D-33·D-34·D-36 으로 세 번 실패했다. 출발점은 D-37 의 실측(크레이터에서 슬립 100 %, 자세는 정상)이다. **착수 전 제약을 README 에 먼저 명시했다** — D-31 미해결(원인을 모르는 채 학습하면 그 원인을 보상으로 학습한다) · 변형 엔진 꺼져 있음 · 실기 검증 경로 없음 |
| 1.59 | 2026-08-25 | **D-31 해결 — 「슬립 49 %」는 슬립이 아니라 `1 − RTF` 였다. 그리고 그 자리에서 D-38 이 나왔다.** 속도를 40 배 흔들어 소인하니 슬립이 **속도와 무관**했고, 같은 노드가 내는 **포즈와 트위스트가 정확히 2 배 어긋났다**(0.4987). 원인은 설정 두 개의 불일치다 — PhysX 는 `physics/default_physics.yaml` 의 `dt: 0.016666` 으로 적분하는데 실시간 페이싱은 `environment` 의 `physics_dt: 0.0333` 으로 걸려, 루프가 30 회/초 돌며 매회 16.67 ms 씩만 진행한다 → **심 시간이 벽시계의 0.4994 배**. `measure_slip.py` 가 경로를 벽시계로 나누고 있었다. **심 시간 기준으로 다시 재니 슬립 −0.011 ~ +0.001, 유효 굴림 반경 비 1.0097.** 침하·차체 끌림 후보는 배제됐다. **D-38 은 미해결로 남긴다** — 네 조건을 실측한 결과 이 하드웨어에서는 접지 정합성을 깨지 않고 실시간을 못 낸다(물리 1/30 은 RTF 0.999 지만 ω=2 에서 슬립 0.637 로 접지가 깨지고, 헤드리스로도 45 Hz 가 천장이라 렌더링이 병목이 아니다). **현행 유지하되 측정을 전부 심 시간 기준으로 바꾸고 GUI 에 RTF 를 상시 노출한다.** 신설 `tools/isaac/sweep_slip_vs_speed.py`, 산출물 `results/d31-slip-sweep-20260825/`. **D-24 ② 마찰 A/B 결과는 유효하다** — 애초에 물리량을 재고 있지 않았으므로 마찰이 안 듣는 것이 정상이었다 |
| 1.60 | 2026-08-25 | **SRS FR-076 / 백로그 B6 완료 — 남은 안전 공백을 닫았다.** D-37(sim 쪽 명령 래치)과 **같은 실패 모양이 실기 경로에도 있었다** — `autorepeat_rate` 때문에 패드 링크가 죽어도 `/cmd_vel_joy` 가 끊기지 않아 FR-005 워치독이 안 걸린다. 판정 신호를 실측으로 골랐다: 헤더 타임스탬프는 autorepeat 중에도 갱신되어 **쓸 수 없고**, 페이로드는 111 표본 내내 bit-identical 이라 **쓸 수 있다**. 정지는 **주행 지령이 나가고 있을 때만** 걸고 입력 변화 시 자동 해제한다. SRS v1.20 |
...
| B6 | **패드 링크 두절 감지 → 구동축 정지 (SRS-FR-076)** | **완료 (2026-08-25).** `rover_teleop` 이 `/joy` 페이로드가 마지막으로 바뀐 시각을 들고 있다가, `pad_link_timeout`(2.0 s) 동안 한 비트도 안 바뀐 채 주행 지령이 나가면 구동을 세운다. 입력이 바뀌면 자동 해제. **후보였던 「헤더 타임스탬프 정체 감시」는 실측으로 기각**했다 — autorepeat 중에도 스탬프는 갱신된다. 회귀 `tools/test_pad_link_watchdog.py`(정상→두절→복구 3단계) |
