...
=== "네트워크 · 스토리지"
    ...
    - **10G Ethernet 백본** — 노드 간 데이터셋·체크포인트 이동, WebRTC 스트리밍 트래픽 수용
    - **NAS 30~40TB (RAID)** — 학생 홈디렉터리(NFS), 공용 데이터셋, 스냅샷 백업
    - **UPS** — 서버·NAS 정전 보호
    - 전력 상시 3kW 이하 · 일반 냉방으로 충분 · 12U 랙 1개 — **기존 실습실에 그대로 수용, 별도 시설 공사 불요**
...
| 자원 관리 | 조 단위 고정 배정 + MIG 예약제 (1차) → Slurm (2차) | 30명 규모엔 고정 배정이 단순·확실 |
...
| 단계 | 시점 | 내용 |
|---|---|---|
| Phase 1 | 예산 확정 후 ~3개월 | HSU-PAC 구축: 발주 → 설치 → 플랫폼 구성 → 동시 30접속 파일럿 → 정식 운영 |
