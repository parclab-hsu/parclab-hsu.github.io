...
확정 역할과 Work Package 의 대응입니다. **읽는 순서는 확정 역할이 먼저입니다.**
...
확정 R&R 이후 접근 방식이 **측정 기반 물리 보정과 검증 중심 시스템 설계**으로 정리되면서,
...
!!! note "무엇이 실적이고 무엇이 신규인지"
    **생성 도구(Cosmos 등 WFM) 연계는 본 과제에서 새로 구축하는 범위이며 현재 수행실적이
    아닙니다.** 확보한 것은 그 아래 계층 — 물리 시뮬레이션을 직접 구동·계측하고,
    변형·접촉 모델을 이식해 보고, ROS 2 로 환경을 실시간 제어해 본 이력입니다.

    **생성 도구는 수단입니다.** 제안의 중심은 대표 제조 작업 1~2개를 대상으로
    실패를 재현하고 복구 데이터로 바꾸는 순환이며, 어떤 도구로 만들었든 생성결과는
    Physics Twin 검증을 통과해야 학습·평가에 들어갑니다.
...
!!! tip "핵심은 수집량이 아니라 실패상태 복원입니다 (2026-08-31 범위 구체화)"
    **실물로 실패를 대량 재현하지 않습니다.** 5 % 규모에서 성립하지 않을뿐더러 안전·비용
    부담도 큽니다. 대신 가상환경에서 **실패 직전의 로봇 관절각·물체 위치·접촉 상태를 저장**해
    두고, 그 상태로 되돌아가 다른 복구 조작을 여러 번 시도합니다.

    **하나의 실패에서 여러 복구 데이터**를 얻는 구조이며, 이것이 학생 트랙의 핵심 기법입니다.
    생성된 복구 동작의 충돌·비정상 동작은 자동으로 걸러냅니다.
