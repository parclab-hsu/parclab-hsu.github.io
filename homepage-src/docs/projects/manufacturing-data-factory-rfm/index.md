...
!!! success "한성대학교의 대표 성과 — 무엇으로 평가받을 것인가"
    ...
    **③ Manufacturing Ontology·Edge Case Intelligence**
    ...
    위 세 가지는 **개발 목표 항목으로 그대로 유지**합니다. 2026-08-31 자 범위 구체화로
    각각을 범용 플랫폼으로 만들지 않고 **대표 제조 작업 1~2개**를 대상으로 구체화했습니다 —
    상세는 「연차별 개발 산출물·핵심 KPI·성과지표」 절과 [연차별 목표](roadmap.md) 를 보십시오.
...
**연구개발비는 4년간 총 17.0억으로 확정되었습니다** — 한성대학교 **16.0억**,
모빌테크 **1.0억**. 모빌테크는 한성대 수행범위 안의 **공동개발기관**입니다.
...
| 주요 역할 | 비중 | 금액 | 근거 |
|---|---:|---:|---|
| ① Real-to-Sim-to-Real 고도화 | 36 % | **5.8억** | 물성시험·보정·실물 대조가 이 항목에 몰림 |
| ② Zero-shot Transfer 도메인 적응 | 24 % | **3.8억** | 증강 조건 구성과 전이 평가 |
| ③ 엣지 케이스 시뮬레이션 | 25 % | **4.0억** | 실패 재현·복구 데이터 생성과 검증 |
| ④ 학술 연구·기술 자문 | 15 % | **2.4억** | 논문·특허·표준 제안과 자문 |
| **합계** | **100 %** | **16.0억** | |

**모빌테크 1.0억**은 대표 작업 공간의 **정밀 3D Asset 취득과 Simulation-ready 변환**에
씁니다 — Pilot Scan · 기준 Asset · 좌표·Semantic·LOD · 현장 Meta. 1~3차년도에 집중되며,
4차년도는 순환 완주·재검증이라 Asset 신규 취득이 없습니다.
...
아래는 **확정 통보 이전**에 수행범위 기준으로 쌓아 올린 준비안입니다. **현재 유효한 것은
위의 5 % 확정안**이며, 이 표는 「범위를 줄이면 금액이 따라 준다」를 보이는 근거로만 남깁니다.
