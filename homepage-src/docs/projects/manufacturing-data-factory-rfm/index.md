...
| # | KPI | 목표 | 측정 방법 |
|:--:|---|---|---|
| 4 | 주요 실패유형별 복구 데이터 확보율 | **≥ 80 %** | 사전 정의된 실패유형 중 유효한 복구 시나리오·데이터가 확보된 비율 |
| 5 | 엣지 케이스 데이터 검수 완료율 | **≥ 90 %** | 생성 데이터 중 검수 절차가 완료된 비율 |
...
| # | KPI | 목표 | 측정 방법 |
|:--:|---|---|---|
| 7-a | **Zero-shot** — 미학습 물체·배치·물성조건 성능 유지율 | **≥ 70 %** | 추가 학습 없이 미학습 조건에서 기준환경 대비 작업성공률 |
| 7-b | **경량 적응·Post-training 후** 성능 유지율 | **≥ 90 %** | 소량 적응 학습 뒤 같은 조건에서 측정한 값 — 7-a 와 별도로 보고 |
...
| 차년도 | 개발 단계 | 대표 결과물 |
|---|---|---|
| **1차 (9개월)** | 기준 시스템 | 대표 작업·실패유형 확정 · WFM–Physics Twin Reference Architecture · Domain Gap 프로토콜 v1 |
| **2차 (10개월)** | 증강 엔진 | Physics Consistency Evaluator v1 · 제조 Domain Variation Library v1 · Edge Case Compiler v1 |
| **3차 (12개월)** | 전이 검증 | Transfer Decision Manager · Multi-stage Validation Gate · Edge/Recovery 증강 Dataset |
| **4차 (12개월)** | 표준화 | Closed-loop RoboOps Toolchain · Cross-domain Physical AI Benchmark · 기술 백서 |
