...
<figure markdown>
  ![한성대학교 고유 Physical AI Data Factory 아키텍처 — OmniLRS 파생 Deformation Engine, Manufacturing Ontology, 학생 Teleoperation, Ontology 기반 Edge Case Intelligence, Versioned Episode Factory와 HSU-PAC Shared Testbed](../../assets/manufacturing-rfm-system-architecture.png){ loading=lazy }
...
| 기준 | 과제 전체 | 본 패키지 26.0억의 비중 | 비고 |
|---|---:|---:|---|
| 공고문 표 (지원규모 7,500백만원) | 75.0억 | 약 34.7% | 세부과제 단위로 해석한 경우 |
| 협의 공유값 (총 약 340억, 정부지원 75억) | 340.0억 | 약 7.6% | 컨소시엄 총액으로 해석한 경우 |

두 해석의 차이가 커서 **본 패키지의 적정 규모는 총액 기준이 확정된 뒤에만 판단할 수 있습니다.**
확정 시 26.0억을 그대로 두는 것이 아니라, 조정범위(22.5~30.5억) 안에서 과제 전체 배분과
한성대·모빌테크의 최종 책임범위에 맞춰 재산정합니다.

!!! note "협약 형태에 따른 전제"
    본 표는 모빌테크 공동개발분(6.5억)을 한 패키지로 묶어 제시한 것입니다. 모빌테크가 별도
    **공동연구개발기관**으로 협약하면 이 6.5억은 모빌테크 기관 예산으로 분리되고, 한성대 패키지는
    19.5억 규모가 됩니다. 위탁·용역 형태면 한성대 예산 내 외주 용역비로 계상됩니다.

    후자의 경우 **3,000만원 이상 외주 용역은 협약 시 연구개발계획서에 내역과 금액을 명시**해야 하고,
    핵심공정 기술개발은 외주 대상이 될 수 없다는 제약이 있으므로(공통 운영요령), 모빌테크가
    Digital Twin 구축을 주도하는 본 구조에서는 **공동연구개발기관 형태가 적합합니다.**
...
인건비 7.0억에 대응하는 한성대 참여 인력 구성입니다. 최종 인원과 참여율은 확정 예산과
학사일정에 맞춰 산정합니다.

| 구분 | 인원(잠정) | 주 담당 Work Package |
|---|---|---|
| 연구책임자 | 1 | 전체 총괄, HSU-1 Interface, HSU-6 RFM 연계 협의 |
| 참여교수 | 1~2 | HSU-2 물리모델, HSU-4 Ontology |
| 박사후연구원 | 1 | HSU-2 Deformation Engine, 파라미터 식별·보정 |
| 석사과정 연구원 | 4~6 | HSU-3 Simulation, HSU-4 Edge Case, HSU-5 Dataset |
