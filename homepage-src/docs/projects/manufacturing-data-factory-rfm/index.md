...
!!! warning "학생 참여는 노동이 아니라 교육과정이다"
    학생 Teleoperation은 **교과·연구 프로그램 안에서 수행하는 실습**이며, 데이터 생산량을 학점이나
    평가에 연동하지 않습니다. 참여는 자발적이고 언제든 철회할 수 있으며, 철회해도 교육과정상
    불이익이 없도록 대체 과제를 둡니다. 조작자 식별정보는 가명화하고, 수집 목적·보관기간·활용범위를
    사전 고지합니다. 사람의 행동 데이터를 수집·분석하는 범위에 해당하면 **IRB 심의 필요 여부를
    착수 전에 확인**합니다.
...
!!! warning "예산 기준과 범위"
    ...
    공고 정합성」의 수치 차이 참조), 총사업비·기관별 정부지원금·현금·현물·간접비 배분을 의미하는
    확정예산이 아닙니다.

    500평 상생공간의 건축·인테리어, 컨소시엄 공용 대형 GPU Cluster, RFM 주관기관의 모델개발비,
    타 기관 Robot HW 구매비는 포함하지 않습니다.

### 총액 해석에 따라 두 안을 준비합니다

사업규모가 확정되지 않았고 두 해석의 차이가 4배 이상이므로, **하나의 숫자로는 어느 쪽에서도
성립하지 않습니다.** 수행범위를 달리한 두 안을 함께 둡니다.

현재 공유된 참여기관은 11곳입니다 — 모빌테크 · 한성대 · 컨피그 · 투모로로보틱스 · 리얼월드 ·
플라잎 · 로보티즈 · 로보로스 · 홀리데이로보틱스 · 블루로빈 · 로봇융합연구원(KIRO).

| 총액 해석 | 기관 평균 | **기준안 26.0억** | **축소안 14.5억** | 한성대 단독분 |
|---|---:|---:|---:|---:|
| 공고문 표 — 75억 | 약 6.8억 | 34.7% (과다) | **19.3% (적정)** | 11.0억 = 14.7% |
| 협의 공유값 — 340억 | 약 30.9억 | **7.6% (적정)** | 4.3% (과소) | 11.0억 = 3.2% |

!!! danger "이 항목이 예산안 전체의 성립 조건입니다"
    **75억으로 확정되면 기준안 26.0억은 통과하기 어렵습니다.** 대학 한 곳과 중소기업 한 곳이
    세부과제 총액의 3분의 1을 가져가는 구성이 되기 때문입니다. 그 경우 축소안으로 전환하며,
    줄어드는 것은 인건비 단가가 아니라 **수행범위**입니다.

    반대로 340억이 총액이면 축소안은 담당 범위에 비해 과소해 Deformation Engine 의 소재군 확장과
    참여기업 공동 Testbed 운영이 어려워집니다. **총액 확정이 [OPEN-ITEMS](https://github.com/parclab-hsu/rfm-hansung-rr/blob/main/OPEN-ITEMS.md) 2번인
    이유가 여기 있습니다.**

### 기준안 — 26.0억 (총액 약 340억 전제)
...
### 축소안 — 14.5억 (세부과제 75억 전제)

**대표 성과 세 축은 그대로 두고 적용 범위를 좁힙니다.** 엔진·Ontology·학생 Data Engine 중 어느
하나를 빼면 나머지가 성립하지 않으므로, 축소는 대상 범위와 반복 횟수에서 합니다.
...
| 모빌테크 공동개발 | 3.5 | −3.0 |
| 연구시설·장비 | 2.0 | −2.0 |
| Cloud·Software·Data | 1.0 | −1.0 |
| 연구재료·물성시험 | 1.2 | −0.8 |
| 현장실증·연구활동 | 0.8 | −0.7 |
| 간접비 등 | 1.8 | −1.2 |
| **합계** | **14.5** | **−11.5** |

**무엇이 줄어드는가 — 축소안에서 달라지는 목표**

| 항목 | 기준안 | 축소안 |
|---|---|---|
| Digital Twin 대상 | 제조 Line 단위 | **단일 Cell 단위** |
| Deformation Engine 소재군 | v3 시점 4개 이상 | **2개 이상**, v3 이력·마모는 개념검증 수준 |
| 제조 Use Case | 다중 Use Case | **Pilot 1건 + 확장 1건** |
| Teleoperation 운용 | 5개 조 상시 | **3개 조**, 학기 단위 운용 |
| 참여기업 Shared Testbed | 상시 운영 | **분기 단위 Sprint** |
| Sim-to-Real 반복 검증 | 재학습–재검증 2회 이상 | **1회** |

축소안에서는 KPI 중 *재현 가능 소재군 수* 와 *Recovery Coverage* 의 목표치를 함께 낮춰야 합니다.
**측정 방법은 바꾸지 않습니다** — 목표만 조정하고 지표 정의는 유지합니다.
...
**기준안** 인건비 7.0억에 대응하는 구성입니다. 축소안(4.2억)에서는 참여교수 1명, 석사과정
연구원 3명 규모로 줄이고 학부연구생 순환 폭을 좁힙니다. 최종 인원과 참여율은 확정 예산과
학사일정에 맞춰 산정합니다.

| 구분 | 인원(잠정) | 주 담당 Work Package |
|---|---|---|
| 연구책임자 | 1 | 전체 총괄, HSU-1 Interface, HSU-6 RFM 연계 협의 |
| 참여교수 | 1~2 (축소안 1) | HSU-2 물리모델, HSU-4 Ontology |
| 박사후연구원 | 1 | HSU-2 Deformation Engine, 파라미터 식별·보정 |
| 석사과정 연구원 | 4~6 (축소안 3) | HSU-3 Simulation, HSU-4 Edge Case, HSU-5 Dataset |
...
!!! note "연구근접지원인력은 조건부 의무다"
    ...
    본 패키지의 차년도 규모(기준안 6.0~8.5억 · 축소안 2.5~4.5억, 모빌테크분 포함)에서 한성대 몫이 이 기준을 넘는지는
...
| 사업 | **총액이 75억으로 확정될 경우** — 기준안 26.0억은 세부과제 총액의 34.7%로 통과 불가 | 예산 전면 재작성 | 수행범위를 좁힌 **축소안 14.5억**을 미리 준비. 대표 성과 세 축은 유지하고 대상 범위·반복 횟수를 줄이는 방식 |
