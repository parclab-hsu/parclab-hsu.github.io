...
|:--:|---|---|---|---|---|
| 1 | **대표 작업의 접촉력 재현오차** | **≤ 20 %** | **미국** · Columbia Univ.·SceniX·Google DeepMind — 연성체 Digital Twin의 물성 보정 공개, 동일 접촉력 NRMSE는 미공개 | 개별 물성보정 사례는 있으나 동일 F/T 곡선 NRMSE 공개치 미확인 | 동일한 물체·자세·속도에서 실제 힘 센서값과 가상환경값을 비교 |
| 2 | **대표 소재의 잔류변형 재현오차** | **≤ 25 %** | **미국** · Columbia Univ.·SceniX·Google DeepMind — 영상 기반 연성체 물리 보정 공개, 동일 잔류변형 정규화오차는 미공개 | 개별 연성체 시뮬레이션 중심이며 반복하중 잔류변형 공개치 미확인 | 선정한 소재와 반복하중 조건에서 작업 전후의 실제 형상과 가상 형상을 비교 |
| 3 | **실제–가상 작업 성공률 차이** | **≤ 20 %p** | **미국** · Columbia Univ.·SceniX·Google DeepMind — 실제–가상 성공률 상관 **r = 0.901~0.944**<br><small>상관계수로서 절대 %p 차이와는 다른 지표</small> | 동일 초기상태·반복수로 짝지은 Sim–Real 성공률 차이 공개치 미확인 | 같은 작업·조건에서 가상환경과 실제 로봇의 성공률 차이 |
| 4 | **주요 실패유형별 복구 데이터 확보율** | **≥ 80 %** | **중국·캐나다** · Current Robotics·Tsinghua·Peking·Toronto — Hi-WM rollback·branching 후 실제 성공률 **+37.9 %p**<br><small>실패유형 coverage는 미공개</small> | 성공 시연 중심이며 Failure Taxonomy별 Recovery Coverage 공개치 미확인 | 사전에 정한 주요 실패유형 중 유효한 복구 시나리오와 데이터가 확보된 비율 |
| 5 | **엣지 케이스 데이터 검수 완료율** | **≥ 90 %** | **중국** · Shanghai AI Lab 연구팀 — SIM1의 궤적 품질 필터링 공개, 동일 검수 완료율은 미공개 | 기관별 내부 검수 중심이며 생성 전체 대비 검수 완료율 공개치 미확인 | 생성 데이터 중 검수가 완료된 비율 |
| 6 | **실제 주요 실패의 가상환경 재현율** | **≥ 70 %** | **미국** · Columbia Univ.·SceniX·Google DeepMind — 실제–가상 정책 성능 상관 **r > 0.9**<br><small>실패 재현율과는 다른 지표</small> | 고장주입·HILS 개별 검증 중심이며 실제 실패목록 대비 재현율 공개치 미확인 | 실제로 발생한 주요 실패를 가상환경에서 다시 만들어 낸 비율 |
| 7 | **학습하지 않은 물체·배치·물성조건 성능 유지율** | **≥ 70 %** | **중국** · Shanghai Jiao Tong Univ.·Horizon Robotics·Style3D — SimWeaver 5개 변형 작업 Zero-shot 실물 성공률 **91.30 %**<br><small>기준환경 대비 유지율과는 다른 지표</small> | 개별 Sim-to-Real 실증은 있으나 미학습 물체·배치·물성 유지율 공개치 미확인 | 미학습 조건에서 기준환경 대비 작업성공률 |
| 8 | **엣지 케이스·복구 데이터 적용 후 실제 성공률 향상** | **+ 10 %p 이상** | **중국·캐나다** · Current Robotics·Tsinghua·Peking·Toronto — Hi-WM 실제 성공률 **+37.9 %p**<br>**중국** · Shanghai AI Lab 연구팀 — SIM1 일반화 성능 **+50 %** | Edge/Recovery 재학습 전후 동일 실물시험의 성공률 개선 공개치 미확인 | 생성 데이터로 재학습한 모델의 실제 로봇 성공률 변화 |
