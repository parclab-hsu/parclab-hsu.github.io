...
**완료 판정** — 대표 작업 1~2개와 주요 실패유형 목록이 **컨소시엄 승인**을 받고,
실제–가상 비교 평가절차가 측정 항목·조건·판정 기준을 갖춘 문서로 확정됩니다.
동일 입력조건에 대한 생성·검증 이력이 재현됩니다.
...
시험 대비 **재현오차 NRMSE ≤ 20 %** 를 달성합니다. 물리적으로 불가능한 생성 결과를
**검출률 ≥ 85 %** 로 걸러내고, 수집환경에서 시연·실패·복구 데이터가 정해진
규격(Scene–Action–State–Event)으로 축적됩니다.
...
**검수 완료율 ≥ 90 %** 를 달성하고, 데이터셋이 정상·실패·복구·가상 생성으로 구분되어
**학습용·평가용 용도가 표시된 채** 공개됩니다.
...
**총 17.0억 중 한성대학교 13.0억 · 모빌테크 4.0억** 으로 배분합니다.
모빌테크는 **한성대 수행범위 안의 공동개발기관**이며, **차년도별 1.0억 균등**입니다.
