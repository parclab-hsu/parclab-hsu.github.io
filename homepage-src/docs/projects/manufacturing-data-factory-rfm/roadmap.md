...
| 핵심기술 | 1차 (9개월) | 2차 (10개월) | 3차 (12개월) | 4차 (12개월) |
|---|---|---|---|---|
| **① Manufacturing Deformation Engine**<br><small>대표 작업의 물리 보정</small> | 실제–가상 비교 평가절차 | **물리 가상환경 구축** · 변형·물성 보정 기술 v1 · 실측 물성값 | 실제–가상 성능차이·미학습조건 평가 | RFM 재학습·**실물 로봇 재검증** |
| **② Human Data Engine**<br><small>실패상태 복원과 복구 증강</small> | 수집 대상 실패유형 선정 | **시연·실패·복구 데이터 수집환경** | 생성 데이터 자동 검수 · 데이터셋 용도별 공개 | **실패상태 복원 및 복구 행동 증강** |
| **③ Ontology·Edge Case Intelligence**<br><small>RFM 취약조건 탐색</small> | **작업–상태–실패–복구 관계 구조 v1** · 데이터 연계규격 | 실패 가능조건 추출기 v1 | **RFM 취약조건 및 엣지 케이스 생성기** | 실제 실패의 가상환경 재현 |
| **공통 · ④ 학술·자문** | 대표 작업 1~2개 확정 | — | RFM 연계도구·평가기준 | 데이터·평가절차 표준화 · 통합 실증 |
...
|---|---|---|---|
| **1차년도**<br>9개월<br>3.1억 | 대표 작업과 실패유형을 확정하고, **실제–가상 비교의 측정 항목·조건·판정 기준**을 문서로 고정 | 대표 제조 작업과 실패유형 선정 · 실환경–가상환경–RFM 데이터 연계규격 설계 · 작업–상태–실패–복구 관계 구조 정의 · 실제–가상 비교 평가절차 확정 | **대표 제조 작업 1~2개와 실패유형 선정**<br>실환경·가상환경·RFM 데이터 연계규격<br>작업·상태·실패·복구 관계 구조 v1<br>**실제–가상 비교 평가절차** |
| **2차년도**<br>10개월<br>4.4억 | 대표 작업의 **물성을 실측해 가상환경에 반영**하고, 원격조작 데이터 **수집환경을 가동** | 대표 작업의 물리 가상환경 구축 · 실제 센서·영상 기반 마찰·접촉·변형값 추정과 보정 · 대표 소재 물성 실측 · 원격조작 수집환경 구축 · 실패 가능조건 추출 | 대표 작업 물리 가상환경<br>**제조 변형·물성 보정 기술 v1**<br>대표 소재의 실측 물성값<br>**시연·실패·복구 데이터 수집환경**<br>실패 가능조건 추출기 v1 |
| **3차년도**<br>12개월<br>4.9억 | **RFM 취약조건을 정량 특정**하고, 해당 조건 데이터를 생성해 **품질 기준으로 자동 선별**·용도별 공개 | RFM 취약조건 탐색 · 실패 경계조건 기반 엣지 케이스 생성 · 생성 데이터 자동 검수 · RFM 연계도구와 평가기준 수립 · 미학습 조건 성능 평가 | **정상·실패·복구·가상 생성 데이터셋 1차 공개**<br>**RFM 취약조건 및 엣지 케이스 생성기**<br>생성 데이터 자동 검수 기능<br>RFM 연계도구 및 평가기준<br>실제–가상 성능차이·미학습조건 평가 |
| **4차년도**<br>12개월<br>4.6억 | 실패 재현–복구 생성–재학습–**실물 재검증 순환을 1회 이상 완주** | 실제 실패의 가상환경 재현 · 실패 직전 상태 저장·복원과 복구 행동 증강 · RFM 재학습과 실물 재검증 · 데이터·평가절차 표준화 · 통합 실증 | **실제 실패의 가상환경 재현**<br>**실패상태 복원 및 복구 행동 증강**<br>**RFM 재학습·실제 로봇 재검증**<br>데이터·평가절차 표준화<br>통합 실증 및 활용 가이드 |
...
> **목표** — 확정된 대표 작업의 **물성을 실측해 가상환경에 반영**하고, 원격조작 기반
> 시연·실패·복구 데이터 **수집환경을 가동**한다.
...
시험 대비 **재현오차 NRMSE ≤ 20 %** 를 달성합니다. 수집환경에서 시연·실패·복구 데이터가
정해진 규격(Scene–Action–State–Event)으로 축적됩니다.

### 3차년도 (12개월) — 취약조건 중심 데이터 생성 및 RFM 평가

> **목표** — **RFM 의 취약조건을 정량적으로 특정**하고, 해당 조건의 데이터를 생성해
...
**완료 판정** — RFM 취약조건이 수치로 특정되고, 생성 데이터의 **검수 완료율 ≥ 90 %** 를
달성합니다. 데이터셋이 정상·실패·복구·가상 생성으로 구분되고 **학습용·평가용 용도가
표시되어** 공개됩니다. 미학습 조건 성능 유지율이 측정됩니다.

### 4차년도 (12개월) — 실패·복구 데이터 순환체계 및 최종 실증

> **목표** — 실제 실패를 가상환경에서 재현해 복구 데이터를 생성하고, 재학습한 모델을
> **실물 로봇으로 재검증하는 순환을 1회 이상 완주**한다.
...
제시됩니다. 제3자가 운용 가이드라인만으로 기준 시나리오를 재현할 수 있습니다.
